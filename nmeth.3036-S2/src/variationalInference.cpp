...
double updateResponsibilities(mylib::Array *img,
                              vector<GaussianMixtureModel *> &vecGM,
                              responsibilities &r, double thrSignal) {
  ...
  kdtreeGaussian.balanceTree(
      vecGM);  // this function rearranges the order of elements in vecGM
  ...
  // reset all the values to zero: we don't need to reset memory (slow). Just
  // reset counter
  ...
  for (unsigned int kk = 0; kk < vecGM.size(); kk++) {
    if (vecGM[kk]->isDead() == false) {
      expectedLogDetCovariance[vecGM[kk]->id] =
          vecGM[kk]->expectedLogDetCovariance();
      expectedLogResponsivity[vecGM[kk]->id] =
          vecGM[kk]->expectedLogResponsivity(totalAlpha);
    } else {
  ...
  for (unsigned long long int n = 0; n < r.N; n++) {
    if (imgData[n] > thrSignal)  // this pixel is considered signal
    {
      ...
      for (int ii = 0; ii < dimsImage; ii++)
        cc[ii] = (float)ccAux[ii];  // transform index 2 coordinates
      ...
      kdtreeGaussian.findNearest(vecGM, cc, maxGaussiansPerVoxel, 1e32,
                                 &radius);
      ...
      while (!kdtreeGaussian.nearest_queue.empty()) {
        ...
        {
                cout<<"ERROR at updateResponsibilities: sparse matrix is not in
        triplet form"<<endl;
                exit(2);
        }
        */
  ...
  */
  //---------------------------------
  /*
  //normalize all the rows. r stores values of consecutive n->I don't need to
  allocate memory: THIS IS NOT TRUE AFTER SPLITTING THE GAUSSIANS!!!
  ...
  for(int ii=0;ii<r.R_nk->nz;ii++)
  {
          ...
          {
                  if((ii-pIni)==1) r.R_nk->x[pIni]=1.0;//only one possibility
  for this element
  ...
  */
  // normalize all the rows TODO r stores values of consecutive n->I don't need
  // to allocate memory
  ...
  for (int ii = 0; ii < r.R_nk->nz; ii++) {
    if (rowNorm[r.R_nk->i[ii]] > 0) r.R_nk->x[ii] /= rowNorm[r.R_nk->i[ii]];
  }
  delete[] rowNorm;
...
//@warning Peak memory use is size(img)*(4+4+8+8) bytes
void updateResponsibilitiesWithParticles(mylib::Array *img,
                                         vector<GaussianMixtureModel *> &vecGM,
                                         responsibilities &r,
                                         double thrSignal) {
  // update all the centers for each Gaussian to calculate tree apropiately
  ...
  for (unsigned int kk = 0; kk < vecGM.size(); kk++) {
    ...
    expectedLogDetCovariance[vecGM[kk]->id] =
        vecGM[kk]->muLambdaExpectation(&logDeterminant, NULL);
...
//==========================================================================================================
void updateGaussianParameters(mylib::Array *img,
                              vector<GaussianMixtureModel *> &vecGM,
                              responsibilities &r) {
  // TODO make it work for any type of data
  mylib::float32 *imgData = (mylib::float32 *)(img->data);
  ...
  vector<Matrix<double, dimsImage, 1> > x_k(
      K, Matrix<double, dimsImage, 1>::Zero());
  vector<Matrix<double, dimsImage, dimsImage> > S_k(
      K, Matrix<double, dimsImage, dimsImage>::Zero());
  ...
  for (unsigned int kk = 0; kk < K; kk++)
    if (vecGM[kk]->N_k > 0.0) S_k[kk] /= vecGM[kk]->N_k;
  ...
  for (unsigned int kk = 0; kk < K; kk++) {
    if (vecGM[kk]->alpha_k / alphaTotal < minPi_kForDead &&
        vecGM[kk]->isDead() == false && vecGM[kk]->fixed == false) {
...
//==========================================================================================================
void updateGaussianParametersWithParticles(
    mylib::Array *img, vector<GaussianMixtureModel *> &vecGM,
    responsibilities &r) {
  // update all the centers for each Gaussian to calculate tree apropiately
  ...
  mylib::CDF *resample;
  meanPrecisionSample auxResampleVec[numParticles];  // TODO: implement
                                                     // resampling without
                                                     // having to duplicate
                                                     // vector of samples
  ...
  Wishartdev *wishartCDF = new Wishartdev(dimsImage + 10.0, auxSigma,
                                          seed++);  // bogus initialization
  Multinormaldev *multinormalCDF = new Multinormaldev(
      vecGM[0]->m_k, auxSigma, seed++);  // bogus initialization
  Wishartdev *wishartPosterior = new Wishartdev(
      dimsImage + 10.0, auxSigma, seed++);  // bogus initialization
  Multinormaldev *multinormalPosterior = new Multinormaldev(
      vecGM[0]->m_k, auxSigma, seed++);  // bogus initialization
  for (unsigned int kk = 0; kk < K; kk++) {
    GM = vecGM[kk];
    ...
    kdtreeGaussian.findNearest(
        vecGM, cc, maxNeighborsMRF + 1, maxDistThrMRF,
        &radius);  // TODO: based neighbors on voronois tesselation

    neighGM.clear();
    while (!kdtreeGaussian.nearest_queue.empty()) {
      // save neighbors in the MRF graph
      if (kdtreeGaussian.nearest_queue.top() !=
          GM)  // to avoid including ourselves
        neighGM.push_back(kdtreeGaussian.nearest_queue.top());
      kdtreeGaussian.nearest_queue.pop();
    }

    auxSigma = GM->W_k * GM->nu_k / meanPrecisionSample::nuProposal;
    wishartCDF->resetParameters(meanPrecisionSample::nuProposal,
                                auxSigma);  // so expected value is GM->w_k
    // sampling importance resampling.
    // We use predetermined parameters for the proposal distribution to make
    // sure it is laxed enough to try different options
    sumWeights = 0.0;
    for (int ii = 0; ii < numParticles; ii++) {
      ...
      auxSigma =
          (GM->muLambdaSamples[ii].lambda_k * meanPrecisionSample::betaProposal)
              .inverse();
      // cout<<auxSigma<<endl;
      ...
      for (unsigned int nn = 0; nn < neighGM.size(); nn++) {
        ...
        auxMRF += neighGM[nn]->muLambdaExpectation(&pairwiseMRF,
                                                   &(GM->muLambdaSamples[ii]));
      }
      if (isNaN(auxMRF))
        GM->muLambdaSamples[ii].w = 0.0;  // for proposal that are forbidden
                                          // (collision between two particles)
      else  // I need to include the ratio between posterior and proposal
            // distribution as part of teh importance sampling
      {
        // eval probability from posterior
        auxPosterior =
            wishartPosterior->evalLog(GM->muLambdaSamples[ii].lambda_k) *
            multinormalPosterior->eval(GM->muLambdaSamples[ii].mu_k);
        ;
        // eval probability from proposal
        auxProposal = wishartCDF->evalLog(GM->muLambdaSamples[ii].lambda_k) *
                      multinormalCDF->eval(GM->muLambdaSamples[ii].mu_k);
        GM->muLambdaSamples[ii].w =
            exp(auxMRF + auxPosterior - auxProposal);  // since the proposal is
                                                       // the EM without MRF
                                                       // priors, the ratio in
    ...
    stringstream itoa;
    itoa << kk;
    GM->writeParticlesForMatlab(
        "/Users/amatf/TrackingNuclei/tmp/debugParticlesProposal" + itoa.str() +
        ".txt");
    //------------------------------------------------------------------------------
    if (sumWeights < 1e-12) {
      cout << "ERROR: Sum weights=" << sumWeights << endl;
      cout << "It probably means that the proposal is too tight and all the "
              "proposed shapes intersect with other nuclei"
           << endl;
      exit(10);
    }
    ...
    memcpy(auxResampleVec, GM->muLambdaSamples,
           sizeof(meanPrecisionSample) * numParticles);
    int pp = 0;
    for (int ii = 0; ii < numParticles; ii++) {
...
double calculateLogLikelihood(mylib::Array *img,
                              vector<GaussianMixtureModel *> &vecGM,
                              responsibilities &r) {
  ...
  Matrix<double, dimsImage, dimsImage> auxSigma;
  for (unsigned int ii = 0; ii < vecGM.size(); ii++) {
    // vecPi_k[ii]=vecGM[ii]->N_k/N;  //This is fo rthe standard EM without
    // priors
    vecPi_k[ii] =
        vecGM[ii]->alpha_k / totalAlpha;  // check equation (10.69) from Bishop
    if (vecGM[ii]->W_k(dimsImage - 1, dimsImage - 1) <
        1e-8)  // it means we have a 2D ellipsoid->just regularize W with any
               // value (we will ignore it in the putput anyway)
    {
      vecGM[ii]->W_k(dimsImage - 1, dimsImage - 1) =
          0.5 * (vecGM[ii]->W_k(0, 0) + vecGM[ii]->W_k(1, 1));
    }
  ...
  for (int ii = 0; ii < r.R_nk->nz; ii++) {
    ...
    if (n != nOld)  // speed up since r.R_nk->i is sorted
    {
      ...
      for (int jj = 0; jj < dimsImage; jj++)
        x_n(jj) = (double)(ccAux[jj]);  // transform index 2 coordinates
      mylib::Free_Array(coord);
      nOld = n;
    }
  ...
  /*
  ofstream
  outLikelihood("/Users/amatf/TrackingNuclei/tmp/GMEMtracking3D_1311858269_CPU/debugLikelihoodPerPixel.m");
  ...
  */
  ...
  for (unsigned int kk = 0; kk < vecGaussian.size(); kk++)
    delete vecGaussian[kk];
  vecGaussian.clear();

  return logLikelihood;
}

double calculateLogLikelihoodWithParticles(
    mylib::Array *img, vector<GaussianMixtureModel *> &vecGM,
    responsibilities &r) {
  // TODO
  return 0.0;
...
GaussianMixtureModel *splitGaussian(vector<GaussianMixtureModel *> &vecGM,
                                    responsibilities &r, int pos) {
  if (pos >= (int)vecGM.size() || pos < 0) {
    cout << "ERROR: at splitGaussian(vector<GaussianMixtureModel*> &vecGM, "
            "responsibilities &r,unsigned int plos) trying to split Gaussian "
            "with pos larger than number of Gaussians"
         << endl;
    exit(2);
  }
...
void deleteGaussian(vector<GaussianMixtureModel *> &vecGM, responsibilities &r,
                    int pos) {
  if (pos >= (int)vecGM.size() || pos < 0) {
    cout << "ERROR: at deleteGaussian(vector<GaussianMixtureModel*> &vecGM, "
            "responsibilities &r,unsigned int plos) trying to split Gaussian "
            "with pos larger than number of Gaussians"
         << endl;
    exit(2);
...
//=======================================================================================================================
void calculateLocalKullbackDiversity(mylib::Array *img,
                                     vector<GaussianMixtureModel *> &vecGM,
  ...
  for (unsigned int ii = 0; ii < vecGM.size(); ii++) {
    ...
    if (vecGM[ii]->id != (int)ii) {
      cout << "ERROR: at calculateLocalKullbackDiversity we need "
              "vecGM[kk]->id==kk "
           << endl;  // crucial for line (*)
      exit(2);
    }
  }
  ...
  for (int ii = 0; ii < r.R_nk->nz; ii++) {
    ...
    if (aux > 0.0)
      vecGM[k]->splitScore +=
          aux * (log(aux) - vecGaussian[k]->evalLog(x_n));  // line (*)
  }
...
//=======================================================================================================================
double calculatePairwiseLocalKullbackDiversity(mylib::Array *img,
                                               GaussianMixtureModel *GM1,
                                               GaussianMixtureModel *GM2,
                                               const responsibilities &r) {
  // precompute normal distributions
  Matrix<double, dimsImage, dimsImage> auxSigma;
  ...
  for (int ii = 0; ii < r.R_nk->nz; ii++) {
    ...
    if (k == GM1->id || k == GM2->id) {
      ...
      if (n == nOld)  // we have found both for this n
      {
        ...
        if (auxR > 0.0)
          splitScore += auxR * log(auxR / (pi1 * Gaussian1->eval(x_n) +
                                           pi2 * Gaussian2->eval(x_n)));
  ...
  if (auxR > 0.0)  // process last step
  {
    ...
    splitScore +=
        auxR *
        log(auxR / (pi1 * Gaussian1->eval(x_n) + pi2 * Gaussian2->eval(x_n)));
  }
...
double calculateModelSelectionPenalty(mylib::Array *img,
                                      vector<GaussianMixtureModel *> &vecGM,
                                      responsibilities &r, double numSamples) {
  ...
  penalty2 = GaussianMixtureModel::dof * log(numSamples / (12.0 * totalAlpha)) +
             log(numSamples / 12.0) + GaussianMixtureModel::dof + 1;
...
//log(K!)
double calculateModelSelectionPenalty(mylib::Array *img,
vector<GaussianMixtureModel*> &vecGM, responsibilities &r,double numSamples)
{
        int Knz=0;//number of mixtures with alpha_k>0
        double penalty=0.0;
...
//===========================================================
void copy2GMEM_CUDA(GaussianMixtureModel *GM,
                    GaussianMixtureModelCUDA *GMCUDAtemp) {
...
//===========================================================
void copyFromGMEM_CUDA(GaussianMixtureModelCUDA *GMCUDAtemp,
                       GaussianMixtureModel *GM) {
...
//==================================================
// TODO: I should use r_nk (responsibities) to calculate a weighted mean of the
// flow for each cell
int applyFlowPredictionToNuclei(mylib::Array *imgFlow,
  ...
  if (imgFlow->type != mylib::FLOAT32_TYPE) {
    cout << "ERROR: at applyFlowPredictionToNuclei: flowArray expected to be "
            "FLOAT32"
         << endl;
    return 1;
  }
  if (imgFlow->ndims != 4) {
    cout << "ERROR: at applyFlowPredictionToNuclei: code is not ready for "
            "other than 3D"
         << endl;
    return 2;
  }
  ...
  if (imgFlowMask != NULL) {
    if (imgFlowMask->type != mylib::UINT8_TYPE) {
      cout << "ERROR: at applyFlowPredictionToNuclei: imgFlowMask expected to "
              "be UINT8"
           << endl;
      return 3;
    }
    if (imgFlowMask->ndims + 1 != imgFlow->ndims) {
      cout << "ERROR: at applyFlowPredictionToNuclei: imgFlowMask and imgFlow "
              "have different n-dimensions"
           << endl;
      return 5;
    }
    ...
    for (int aa = 0; aa < imgFlowMask->ndims; aa++) {
      if (imgFlow->dims[aa] != imgFlowMask->dims[aa]) {
        cout << "ERROR: at applyFlowPredictionToNuclei: imgFlowMask and "
                "imgFlow have different sizes"
             << endl;
        return 4;
      }
    }
  ...
  if (imgFlowMask != NULL) imgFlowMaskPtr = (mylib::uint8 *)(imgFlowMask->data);
  ...
  {
          cout<<"ERROR: at mainTestOpticalFlow: flow array file
  "<<filenameOut<<" could not be written"<<endl;
          return 3;
  }
  ...
  */
  ...
  for (unsigned int kk = 0; kk < vecGM.size(); kk++) {
    ...
    for (int ii = 0; ii < dimsImage; ii++) {
      ...
      for (long long int zz = p[2] - winSize[2]; zz <= p[2] + winSize[2];
           zz++) {
        for (long long int yy = p[1] - winSize[1]; yy <= p[1] + winSize[1];
             yy++) {
          for (long long int xx = p[0] - winSize[0]; xx <= p[0] + winSize[0];
               xx++) {
            pos = xx +
                  imgFlow->dims[0] *
                      (yy + imgFlow->dims[1] * (zz + imgFlow->dims[2] * ii));
      ...
      if (isForwardFlow == true) {
        // update flow
        vecGM[kk]->m_k(ii) -= flow;  // negative because we calculate flow from
                                     // t->t+1 (forward in time)
        vecGM[kk]->m_o(ii) -= flow;
      } else {
        // update flow
        vecGM[kk]->m_k(ii) += flow;  // positive because we calculate flow from
                                     // t+1->t (backwards in time)
        vecGM[kk]->m_o(ii) += flow;
      }
    }
  }
