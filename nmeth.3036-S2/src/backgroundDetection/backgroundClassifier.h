...
class backgroundDetectionFeatures {
 public:
  ...
  /*
  \brief starting at the the given root it calculates a set of fatures. It uses
  teporalWindowSize to calculate the features

  \return			Greater than zero if features could not be
  calculated
  */
  int calculateFeatures(TreeNode<ChildrenTypeLineage>* root, int devCUDA);

  // support functions
  static bool isCorrectConfidenceLevel(TreeNode<ChildrenTypeLineage>* root,
                                       int confidenceLevel);  // whether we can
                                                              // use this
                                                              // lineage for
                                                              // training (looks
                                                              // at confidence
 ...
 protected:
 private:
  static int teporalWindowSize;  // number of time points includes to calculate
                                 // features
};
...
inline long long int backgroundDetectionFeatures::getNumFeatures() {
  return 35;  // make sure this matches with
              // backgroundDetectionFeatures::calculateFeatures (there is an
              // assert)
}
