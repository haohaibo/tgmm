 ...
 * use a Cholesky
        * decomposition to determine whether a system of equations has a
 * solution.
  *
  * \sa MatrixBase::ldlt(), class LLT
  */
/* THIS PART OF THE DOX IS CURRENTLY DISABLED BECAUSE INACCURATE BECAUSE OF BUG
...
struct ldlt_inplace<Lower> {
  ...
  static bool unblocked(MatrixType& mat, TranspositionType& transpositions,
    ...
    if (size <= 1) {
      ...
      if (sign) *sign = real(mat.coeff(0, 0)) > 0 ? 1 : -1;
      return true;
    }
    ...
    for (Index k = 0; k < size; ++k) {
      ...
      biggest_in_corner = mat.diagonal().tail(size - k).cwiseAbs().maxCoeff(
          &index_of_biggest_in_corner);
      index_of_biggest_in_corner += k;

      if (k == 0) {
        // The biggest overall is the point of reference to which further
        // diagonals
        ...
        if (sign)
          *sign = real(mat.diagonal().coeff(index_of_biggest_in_corner)) > 0
                      ? 1
                      : -1;
      }
...
struct LDLT_Traits<MatrixType, Lower> {
  ...
  typedef TriangularView<typename MatrixType::AdjointReturnType, UnitUpper>
      MatrixU;
...
struct LDLT_Traits<MatrixType, Upper> {
  typedef TriangularView<typename MatrixType::AdjointReturnType, UnitLower>
      MatrixL;
