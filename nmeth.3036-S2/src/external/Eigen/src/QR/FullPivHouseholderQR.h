...
struct solve_retval<FullPivHouseholderQR<_MatrixType>, Rhs>
  ...
  void evalTo(Dest& dst) const {
    ...
    if (!dec().isSurjective()) {
      ...
      RealScalar biggest_in_upper_part_of_c =
          c.topRows(dec().rank()).cwiseAbs().maxCoeff();
      RealScalar biggest_in_lower_part_of_c =
          c.bottomRows(rows - dec().rank()).cwiseAbs().maxCoeff();
      // FIXME brain dead
      const RealScalar m_precision =
          NumTraits<Scalar>::epsilon() * std::min(rows, cols);
      // this internal:: prefix is needed by at least gcc 3.4 and ICC
      if (!internal::isMuchSmallerThan(biggest_in_lower_part_of_c,
                                       biggest_in_upper_part_of_c, m_precision))
        return;
    }
    dec()
        .matrixQR()
...
template <typename MatrixType>
typename FullPivHouseholderQR<MatrixType>::MatrixQType
FullPivHouseholderQR<MatrixType>::matrixQ() const {
  ...
  for (Index k = size - 1; k >= 0; k--) {
    res.block(k, k, rows - k, rows - k)
        .applyHouseholderOnTheLeft(m_qr.col(k).tail(rows - k - 1),
                                   internal::conj(m_hCoeffs.coeff(k)),
                                   &temp.coeffRef(k));
