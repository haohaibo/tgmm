...
class ColPivHouseholderQR {
 public:
  ...
  typedef typename internal::plain_row_type<MatrixType, RealScalar>::type
      RealRowVectorType;
  typedef
      typename HouseholderSequence<MatrixType, HCoeffsType>::ConjugateReturnType
...
ColPivHouseholderQR<MatrixType>& ColPivHouseholderQR<MatrixType>::compute(
  ...
  RealScalar threshold_helper = m_colSqNorms.maxCoeff() *
                                internal::abs2(NumTraits<Scalar>::epsilon()) /
                                rows;
  ...
  for (Index k = 0; k < m_nonzero_pivots; ++k)
    m_colsPermutation.applyTranspositionOnTheRight(
        k, m_colsTranspositions.coeff(k));
...
struct solve_retval<ColPivHouseholderQR<_MatrixType>, Rhs>
  ...
  void evalTo(Dest& dst) const {
    ...
    const int cols = dec().cols(), nonzero_pivots = dec().nonzeroPivots();
