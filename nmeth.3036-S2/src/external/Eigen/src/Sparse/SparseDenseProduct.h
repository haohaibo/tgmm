 ...
 public:
  ...
  void scaleAndAddTo(Dest& dest, Scalar alpha) const {
    ...
    for (Index j = 0; j < m_lhs.outerSize(); ++j) {
      typename Rhs::Scalar rhs_j =
          alpha * m_rhs.coeff(LhsIsRowMajor ? 0 : j, 0);
  ...
  void scaleAndAddTo(Dest& dest, Scalar alpha) const {
    ...
    for (Index j = 0; j < m_rhs.outerSize(); ++j)
      for (RhsInnerIterator i(m_rhs, j); i; ++i)
        dest.col(RhsIsRowMajor ? i.index() : j) +=
            (alpha * i.value()) * m_lhs.col(RhsIsRowMajor ? j : i.index());
  }
