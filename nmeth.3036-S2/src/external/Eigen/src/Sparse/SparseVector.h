...
class SparseVector
 ...
 public:
   ...
   * is large.
    */
  inline Scalar& coeffRef(Index i) { return m_data.atWithInsertion(i); }
 ...
 public:
  ...
  inline void startVec(Index outer) { eigen_assert(outer == 0); }
  ...
  inline SparseVector(const MatrixBase<OtherDerived>& other) : m_size(0) {
    *this = other.derived();
  }
  ...
  inline SparseVector(const SparseVector& other) : m_size(0) {
    *this = other.derived();
  }
  ...
  //       const bool needToTranspose = (Flags & RowMajorBit) !=
  ...
  //         // Since each coeff of the rhs has to be evaluated twice, let's
  //         evauluate it if needed
  ...
  //           for (typename _OtherCopy::InnerIterator it(otherCopy, j); it;
  //           ++it)
