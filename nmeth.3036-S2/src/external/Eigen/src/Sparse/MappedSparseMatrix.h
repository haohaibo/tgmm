...
class MappedSparseMatrix<Scalar, _Flags, _Index>::InnerIterator {
 public:
  ...
  InnerIterator(const Flagged<MappedSparseMatrix, Added, Removed>& mat,
                Index outer)
      : m_matrix(mat._expression()),
        m_id(m_matrix._outerIndexPtr()[outer]),
        m_start(m_id),
        m_end(m_matrix._outerIndexPtr()[outer + 1]) {}
