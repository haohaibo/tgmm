...
class SparseSelfAdjointView
 ...
 public:
  ...
  SparseSymmetricPermutationProduct<_MatrixTypeNested, UpLo> twistedBy(
      const PermutationMatrix<Dynamic>& perm) const {
 ...
 public:
  ...
  void scaleAndAddTo(Dest& dest, Scalar alpha) const {
    ...
    for (Index j = 0; j < m_lhs.outerSize(); ++j) {
      ...
      Block<Dest, 1, Dest::ColsAtCompileTime> dest_j(
          dest.row(LhsIsRowMajor ? j : 0));
...
void permute_symm_to_fullsymm(
  ...
  for (Index j = 0; j < size; ++j) count[j] = dest._outerIndexPtr()[j];
...
void permute_symm_to_symm(const MatrixType& mat,
  ...
  for (Index j = 0; j < size; ++j) {
    ...
    for (typename MatrixType::InnerIterator it(mat, j); it; ++it) {
      Index i = it.index();
      if ((SrcUpLo == Lower && i < j) || (SrcUpLo == Upper && i > j)) continue;
  ...
  for (Index j = 0; j < size; ++j) {
    ...
    for (typename MatrixType::InnerIterator it(mat, j); it; ++it) {
      ...
      dest._innerIndexPtr()[k] =
          DstUpLo == Lower ? std::max(ip, jp) : std::min(ip, jp);
