...
void computeProductBlockingSizes(std::ptrdiff_t& k, std::ptrdiff_t& m,
                                 std::ptrdiff_t& n) {
  // Explanations:
...
inline void computeProductBlockingSizes(std::ptrdiff_t& k, std::ptrdiff_t& m,
                                        std::ptrdiff_t& n) {
...
struct gebp_madd_selector {
  EIGEN_STRONG_INLINE EIGEN_ALWAYS_INLINE_ATTRIB static void run(const CJ& cj,
                                                                 A& a, B& b,
                                                                 C& c,
                                                                 T& /*t*/) {
    c = cj.pmadd(a, b, c);
...
struct gemm_pack_lhs {
  void operator()(Scalar* blockA, const Scalar* EIGEN_RESTRICT _lhs,
                  Index lhsStride, Index depth, Index rows, Index stride = 0,
                  Index offset = 0) {
    ...
    for (Index i = 0; i < peeled_mc; i += Pack1) {
      ...
      for (Index k = 0; k < depth; k++)
        for (Index w = 0; w < Pack1; w++) blockA[count++] = cj(lhs(i + w, k));
...
struct gemm_pack_rhs<Scalar, Index, nr, ColMajor, Conjugate, PanelMode> {
  ...
  enum { PacketSize = packet_traits<Scalar>::size };
  void operator()(Scalar* blockB, const Scalar* rhs, Index rhsStride,
