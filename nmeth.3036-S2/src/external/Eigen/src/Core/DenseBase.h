...
#endif  // not EIGEN_PARSED_BY_DOXYGEN
{
 ...
 public:
  ...
  // Note: The "DenseBase::" prefixes are added to help MSVC9 to match these
  // declarations with the later implementations.
  SegmentReturnType segment(Index start, Index size);
  typename DenseBase::ConstSegmentReturnType segment(Index start,
                                                     Index size) const;
  ...
  template <int Size>
  typename ConstFixedSegmentReturnType<Size>::Type head() const;

  template <int Size>
  typename FixedSegmentReturnType<Size>::Type tail();
  template <int Size>
  typename ConstFixedSegmentReturnType<Size>::Type tail() const;

  template <int Size>
  typename FixedSegmentReturnType<Size>::Type segment(Index start);
  template <int Size>
  typename ConstFixedSegmentReturnType<Size>::Type segment(Index start) const;
   ...
   * this function just returns
    ...
    */
  EIGEN_STRONG_INLINE const typename internal::eval<Derived>::type eval()
      const {
  ...
  inline const Select<Derived, typename ElseDerived::ConstantReturnType,
                      ElseDerived>
  select(typename ElseDerived::Scalar thenScalar,
         const DenseBase<ElseDerived>& elseMatrix) const;
  ...
  const Replicate<Derived, Dynamic, Dynamic> replicate(Index rowFacor,
