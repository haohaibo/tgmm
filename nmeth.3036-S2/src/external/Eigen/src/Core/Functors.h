...
struct scalar_real_ref_op {
  ...
  typedef typename NumTraits<Scalar>::Real result_type;
  EIGEN_STRONG_INLINE result_type& operator()(const Scalar& a) const {
    return real_ref(*const_cast<Scalar*>(&a));
  }
};
template <typename Scalar>
...
struct scalar_imag_ref_op {
  ...
  typedef typename NumTraits<Scalar>::Real result_type;
  EIGEN_STRONG_INLINE result_type& operator()(const Scalar& a) const {
    return imag_ref(*const_cast<Scalar*>(&a));
  }
};
template <typename Scalar>
...
struct scalar_quotient1_impl {
  ...
  EIGEN_STRONG_INLINE scalar_quotient1_impl(const Scalar& other)
      : m_other(static_cast<Scalar>(1) / other) {}
  EIGEN_STRONG_INLINE Scalar operator()(const Scalar& a) const {
    return a * m_other;
  }
  EIGEN_STRONG_INLINE const Packet packetOp(const Packet& a) const {
...
struct linspaced_op_impl<Scalar, true> {
  ...
  EIGEN_STRONG_INLINE const Packet packetOp(Index i) const {
    return internal::padd(
        m_lowPacket, pmul(m_stepPacket, padd(pset1<Packet>(i), m_interPacket)));
  }
...
// in CwiseBinaryOp, we require the Lhs and Rhs to have the same scalar type,
// except for multiplication
// where we only require them to have the same _real_ scalar type so one may
// multiply, say, float by complex<float>.
// FIXME move this to functor_traits adding a functor_default
template <typename Functor>
...
template <typename LhsScalar, typename RhsScalar>
struct functor_allows_mixing_real_and_complex<
    scalar_product_op<LhsScalar, RhsScalar> > {
  enum { ret = 1 };
};
template <typename LhsScalar, typename RhsScalar>
struct functor_allows_mixing_real_and_complex<
    scalar_conj_product_op<LhsScalar, RhsScalar> > {
  enum { ret = 1 };
};
