...
inline void* handmade_aligned_malloc(size_t size) {
  ...
  void* aligned = reinterpret_cast<void*>(
      (reinterpret_cast<size_t>(original) & ~(size_t(15))) + 16);
 ...
 * have 16 bytes alignment.
  * On allocation error, the returned pointer is null, and if exceptions are
...
#ifdef EIGEN_EXCEPTIONS
  if (result == 0 && new_size != 0) throw std::bad_alloc();
#endif
  return result;
}
 ...
 * guaranteed to have 16 bytes alignment.
  * On allocation error, the returned pointer is undefined, but if exceptions
 * are enabled then a std::bad_alloc is thrown.
...
/** \internal
  * Allocates an aligned buffer of SIZE bytes on the stack if SIZE is smaller
 * than
  * EIGEN_STACK_ALLOCATION_LIMIT, and if stack allocation is supported by the
 * platform
  * (currently, this is Linux only). Otherwise the memory is allocated on the
 * heap.
...
#define ei_aligned_stack_alloc(SIZE)     \
  (SIZE <= EIGEN_STACK_ALLOCATION_LIMIT) \
      ? alloca(SIZE)                     \
      : Eigen::internal::aligned_malloc(SIZE)
#define ei_aligned_stack_free(PTR, SIZE) \
  if (SIZE > EIGEN_STACK_ALLOCATION_LIMIT) Eigen::internal::aligned_free(PTR)
#elif defined(_MSC_VER)
#define ei_aligned_stack_alloc(SIZE)     \
  (SIZE <= EIGEN_STACK_ALLOCATION_LIMIT) \
      ? _alloca(SIZE)                    \
...
#define ei_aligned_stack_new(TYPE, SIZE)                                    \
  Eigen::internal::construct_elements_of_array(                             \
      reinterpret_cast<TYPE*>(ei_aligned_stack_alloc(sizeof(TYPE) * SIZE)), \
      SIZE)
#define ei_aligned_stack_delete(TYPE, PTR, SIZE)                  \
  do {                                                            \
    Eigen::internal::destruct_elements_of_array<TYPE>(PTR, SIZE); \
...
#define EIGEN_MAKE_ALIGNED_OPERATOR_NEW_IF_VECTORIZABLE_FIXED_SIZE(Scalar, \
                                                                   Size)   \
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW_IF(((Size) != Eigen::Dynamic) &&         \
                                     ((sizeof(Scalar) * (Size)) % 16 == 0))
...
inline bool cpuid_is_vendor(int abcd[4], const char* vendor) {
  return abcd[1] == ((int*)(vendor))[0] && abcd[3] == ((int*)(vendor))[1] &&
         abcd[2] == ((int*)(vendor))[2];
}
...
inline void queryCacheSizes(int& l1, int& l2, int& l3) {
  ...
  if (cpuid_is_vendor(abcd, "GenuineIntel"))
    queryCacheSizes_intel(l1, l2, l3, max_std_funcs);
           ...
           cpuid_is_vendor(abcd, "AMDisbetter!"))
