 ...
 * pivoting: the matrix A
  * is decomposed as A = PLUQ where L is unit-lower-triangular, U is
 * upper-triangular, and P and Q
  * are permutation matrices. This is a rank-revealing LU decomposition. The
...
class FullPivLU {
 public:
   ...
   * formula for
    ...
    */
  FullPivLU& setThreshold(Default_t) { m_usePrescribedThreshold = false; }
  ...
  inline Index rank() const {
    ...
    RealScalar premultiplied_threshold =
        internal::abs(m_maxpivot) * threshold();
    Index result = 0;
...
FullPivLU<MatrixType>& FullPivLU<MatrixType>::compute(
  ...
  for (Index k = 0; k < size; ++k) {
    ...
    // when k==0, biggest_in_corner is the biggest coeff absolute value in the
    // original matrix
    if (k == 0) cutoff = biggest_in_corner * NumTraits<Scalar>::epsilon();

    // if the pivot (hence the corner) is "zero", terminate to avoid generating
    // nan/inf values.
    // Notice that using an exact comparison (biggest_in_corner==0) here, as
    // Golub-van Loan do in
    // their pseudo-code, results in numerical instability! The cutoff here has
    // been validated
