...
const float colormap::hsv18[54] = {
    1.0000, 0,      0,      1.0000, 0.3333, 0,      1.0000, 0.6667,
    0,      1.0000, 1.0000, 0,      0.6667, 1.0000, 0,      0.3333,
    1.0000, 0,      0,      1.0000, 0,      0,      1.0000, 0.3333,
    0,      1.0000, 0.6667, 0,      1.0000, 1.0000, 0,      0.6667,
    1.0000, 0,      0.3333, 1.0000, 0,      0,      1.0000, 0.3333,
    0,      1.0000, 0.6667, 0,      1.0000, 1.0000, 0,      1.0000,
    1.0000, 0,      0.6667, 1.0000, 0,      0.3333};  // 18x3=54;

responsibilities::responsibilities(unsigned long long int N_,
                                   unsigned long long int K_,
                                   long long int nzmax) {
  N = N_;
  K = K_;
  ...
  if (R_nk == NULL) {
    cout << "ERROR: at responsibilities::responsibilities(unsigned long long "
            "int N_,unsigned long long int K_) allocating sparse matrix "
         << endl;
  }
}
...
responsibilities::~responsibilities() {
  R_nk =
      cs_spfree(R_nk);  // returns NULL. Already protected in case R-nk is null
}
...
//===========================================================
void responsibilities::writeOutSegmentationMaskAndImageBlend(
    mylib::Array *img, string fileout, const vector<colormap> &colMap) {
  const double alpha = 0.65;  // blend weight
  const double alpha_c = 1.0 - alpha;
  const double uint8 = 255;
  const double minR_nkValue =
      0.1;  // minimum probability to be accepted as a pixel
  ...
  mylib::Array *blend = mylib::Make_Array(mylib::RGB_KIND, mylib::UINT8_TYPE,
                                          img->ndims, img->dims);
      ...
      if (!vecKr.empty())  // create the blend for current pixel
      {
        ...
        if (vecKr[maxPos].second > minR_nkValue) {
          if (vecKr[maxPos].first >= (int)colMap.size()) {
            cout << "ERROR: at writeOutSegmentationMaskAndImageBlend. Colormap "
                    "is not long enough for all the clusters"
                 << endl;
            exit(2);
          }
          blend_RData[nOld] +=
              (mylib::uint8)(uint8 * alpha_c * colMap[vecKr[maxPos].first].r);
          blend_GData[nOld] +=
              (mylib::uint8)(uint8 * alpha_c * colMap[vecKr[maxPos].first].g);
          blend_BData[nOld] +=
              (mylib::uint8)(uint8 * alpha_c * colMap[vecKr[maxPos].first].b);
        }
      }
  ...
  if (mylib::Write_Image((char *)fileout.c_str(), blend,
                         mylib::DONT_PRESS))  // write file with no compressions
  {
    cout << "ERROR: at writeOutSegmentationMaskAndImageBlend file was not "
            "written properly"
         << endl;
    exit(2);
  }
...
void writeOutSegmentationMaskAndImageBlend(mylib::Array *img, string fileout,
                                           const vector<colormap> &colMap,
                                           long long int *nPos,
                                           int *kAssignment, int *colorVec,
