...
//we return the volume of an ellipsoid of radius 2*sigma in each dimension
const double GaussianMixtureModel::ellipsoidVolumeConstant=
(4.0/3.0)*3.14159265358979310*pow(2.0,dimsImage);
//we return the volume of an ellipsoid of radius 2*sigma in each dimension
const double GaussianMixtureModel::ellipseAreaConstant=3.14159265358979310*pow(2.0,dimsImage);
//Kalman Filter transition matrix
MatrixState GaussianMixtureModel::psi=MatrixState::Identity();
//Kalman Filter state to observations
MatrixState2Obs GaussianMixtureModel::Mobs=MatrixState2Obs::Zero();
//Kalman Filter uncertainty in motion model
const MatrixState GaussianMixtureModel::Q=MatrixState::Identity();


const double meanPrecisionSample::nuProposal = 10.0;
//if we were in one dimension sigma=sigmaEllipsoid/sqrt(betaProposal). 
//So with 3.0 uncertainty of mean is all inside the cell size
const double meanPrecisionSample::betaProposal = 3.0*3.0;
...
GaussianMixtureModel::GaussianMixtureModel(XMLNode &xml,int position)
{
    ...
    for(int ii=0;ii<dimsImage;ii++)
        Mobs(ii,ii)=1.0;
    //we assume no speed for initial frame
    mu_KF=VectorState::Zero();
    for(int ii=0;ii<dimsImage;ii++)
        mu_KF(ii)=m_k(ii);
    P_KF=MatrixState::Identity();
...
ostream& GaussianMixtureModel::writeXML(ostream& os)
{
    ...
    os<<"id=\""<<id<<"\" lineage=\""<<lineageId<<"\" parent=\""<<parentId
...
double GaussianMixtureModel::distEllipsoid(meanPrecisionSample &p1,meanPrecisionSample &p2)
{
    ...
    //centers fo ball within the sphere
    Matrix<double,dimsImage,1> g2=p2.mu_k;


    //variables needed for the iterative algorithm
    double t1=0.0,t2=0.0;
    Matrix<double,dimsImage,1> a;//auxiliar to solve quadratic equation
    Matrix<double,dimsImage,1> b;//auxiliar to solve quadratic equation
    ...
    while(numIter<maxIter)
    {
        ...
        if( isnanF(theta1) == true ) 
            theta1=0.0;//due to numerical imprecision when dot product is very close to 1.0
        g2=p2.lambda_k*(x2-p2.mu_k);
        theta2=acos(((x1-x2).normalized()).dot(g2.normalized()));
        if(isnanF(theta2) == true ) 
            theta2=0.0;
...
void GaussianMixtureModel::splitGaussian(GaussianMixtureModel *GM,int id_)
{
    ...
    if(sigmaMax<1e-3)
        sigmaMax=1e-3;//you don't want to shoot too far
    double sigmaMax2=1.0/sqrt(sigmaMax);

    //maximum change direction
    Matrix<double,dimsImage,1> v=sigma1eig.eigenvectors().col(pos);
    //I can use the max coeff because I have previously adjusted for scale
    double sigmaMin=sigma1eig.eigenvalues().maxCoeff();

    //reset values
    //we want to generate larger Gaussians because it is easier to compress than to expand
    W_k=(sigmaMin/nu_k)*Matrix<double,dimsImage,dimsImage>::Identity();
    //we need to compensate for scale
    for(int ii=0;ii<dimsImage;ii++)
        W_k(ii,ii)*=(scale[ii]*scale[ii]);
...
void GaussianMixtureModel::splitGaussian(GaussianMixtureModel *GM,int id_,float* img,int imSize[dimsImage])
{
    ...
    int maxIter=100;
    //squared to avoid computing so many sqrt
    double minDistLocalMaxima2=6.0*6.0;
    //how many sigmas away we use to setup the box
    ...
    //order from mylib::Array (... (xn-1*dims[n-2] + xn-2)*dims[n-3] + ...)*dims[0] + x0.
    //For mylib dims[0] is x
    long long int pos=0;
    long long int count=-1;
    //precompute Gaussians constants on exp.
    //we do not need for it to decay slower: it avoids the outsiders influence
    sigma1*=(-0.5);
        ...
        for(int yy=yMin;yy<=yMax;yy++)
        {
            ...
            for(int xx=xMin;xx<=xMax;xx++)
            {
                ...
                if(aux>auxB1 || aux>auxB2)
                {
                    ...
                    //very close to C1
                    if((auxCol-auxC1).cwiseProduct(auxScale).squaredNorm()<minDistLocalMaxima2)
                    {
                        if(aux>auxB1)
                        {
                            auxB1=aux;
                    ...
                    }else if((auxCol-auxC2).cwiseProduct(auxScale).squaredNorm()<minDistLocalMaxima2){
                            //very close to C2
                        if(aux>auxB2)
                        {
                            auxB2=aux;
    ...
    //delete unnecessary memory: not really needed since we access them using 
    //for loops(we just restrict the counter)
    numElemBox=count+1;
    ...
    do{
        ...
        //cerr<<"Iter="<<iter<<";totalW1="<<totalW1<<";totalW2="<<totalW2
        //<<";totalDist="<<totalDist<<";m_kOrig="<<mkOrig.transpose()<<
        //";m1="<<m_k.transpose()<<";m2="<<GM->m_k.transpose()<<endl;
    ...
    if(totalW1<1e-8)//empty cluster
    {
        ...
        if( W_k(dimsImage-1,dimsImage-1) < 1e-8 )
        {
    ...
    if(totalW2<1e-8)//empty cluster
    {
        ...
        //it means we have a 2D ellipsoid->just regularize W with any value
        //(we will ignore it in the putput anyway) 
        if( GM->W_k(dimsImage-1,dimsImage-1) < 1e-8 )
        {
    ...
    if(isnanF(GM->W_k(0)))
    {
        ...
        cout<<"Id="<<id<<" "<<id_<<";NumIters="<<iter<<";totalW1="<<totalW1
            <<";totalW2="<<totalW2<<";totalDist="<<totalDist<<";m_kOrig="
            <<mkOrig.transpose()<<";m1="<<m_k.transpose()<<";m2="<<GM->m_k.transpose()
            <<";nu_k="<<GM->nu_k<<endl;
        //exit(4);
...
void GaussianMixtureModel::updatePriors(double betaPercentageOfN_k,double nuPercentageOfN_k,double alphaPercentage, double alphaTotal)
{
    ...
    beta_o=max(betaPercentageOfN_k*N_k,1e-5);
    //degrees of freedom have to be greater or equal to dimsImage
    nu_o=max(nuPercentageOfN_k*N_k,(double)dimsImage);
    W_o=W_k*nu_k/nu_o;
    if(alphaTotal<0.0)
        alpha_o=alpha_o;//this value does not need to be altered
    else
        //we only kill very poor mixtures. Multiplier has to be<1.0 , 
        //otherwise even with N_k=0 Gauassians will still survive
        alpha_o=alphaTotal*minPi_kForDead*alphaPercentage;
}
//==============================================================================
void GaussianMixtureModel::updatePriorsAfterSplitProposal()
{
    m_o=m_k;
    //it can never be zero   
    beta_o=max(0.5*N_k,1e-5);
    //degrees of freedom have to be greater or equal to dimsImage
    nu_o=max(0.5*N_k,(double)dimsImage);
    W_o=W_k*nu_k/nu_o;
...
double GaussianMixtureModel::volumeGaussian()
{
    ...
    switch(dimsImage)
    {
        case 3:
            //vol=(4/3)Pi*a*b*c
            return ellipsoidVolumeConstant/sqrt(sigma1eig.eigenvalues().prod());
            break;
        case 2:
            //area=Pi*a*b
            return ellipseAreaConstant/sqrt(sigma1eig.eigenvalues().prod());
            break;
        default:
...
void GaussianMixtureModel::informationUpdateKalman()
{
    if(dimsImage>4)
    {
        //calculate the inverse efficiently using the fact that 
        //covariance matrices are symmetric
        //gain2.ldlt().solveInPlace(gain);
