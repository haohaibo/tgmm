...
const int GaussianMixtureModel::dof =
    dimsImage + dimsImage * (dimsImage + 1) / 2;
const double GaussianMixtureModel::ellipsoidVolumeConstant =
    (4.0 / 3.0) * 3.14159265358979310 *
    pow(2.0, dimsImage);  // we return the volume of an ellipsoid of radius
                          // 2*sigma in each dimension
const double GaussianMixtureModel::ellipseAreaConstant =
    3.14159265358979310 * pow(2.0, dimsImage);  // we return the volume of an
                                                // ellipsoid of radius 2*sigma
                                                // in each dimension

MatrixState GaussianMixtureModel::psi =
    MatrixState::Identity();  // Kalman Filter transition matrix
MatrixState2Obs GaussianMixtureModel::Mobs =
    MatrixState2Obs::Zero();  // Kalman Filter state to observations
const MatrixState GaussianMixtureModel::Q =
    MatrixState::Identity();  // Kalman Filter uncertainty in motion model

const double meanPrecisionSample::nuProposal = 10.0;
const double meanPrecisionSample::betaProposal =
    3.0 * 3.0;  // if we were in one dimension
                // sigma=sigmaEllipsoid/sqrt(betaProposal). So with 3.0
                // uncertainty of mean is all inside the cell size
...
GaussianMixtureModel::GaussianMixtureModel(const GaussianMixtureModel &p) {
  ...
  memcpy(muLambdaSamples, p.muLambdaSamples,
...
//===========================================================
GaussianMixtureModel &GaussianMixtureModel::operator=(
    const GaussianMixtureModel &p) {
  if (this != &p) {
    m_k = p.m_k;
    beta_k = p.beta_k;
    W_k = p.W_k;
...
float GaussianMixtureModel::distBlobs(const float *cc) {
  ...
  for (int ii = 0; ii < dimsImage; ii++)
    dd += scale[ii] * scale[ii] * (center[ii] - cc[ii]) * (center[ii] - cc[ii]);
...
//@warning Vector of muLambdaSamples is not recorded to avoid generating long
// files
...
ostream &GaussianMixtureModel::writeXML(ostream &os) {
  ...
  ;
  os << "id=\"" << id << "\" lineage=\"" << lineageId << "\" parent=\""
     << parentId << "\" dims=\"" << dimsImage << "\" splitScore=\""
     << splitScore << "\"";
  ...
  os << "\" W=\"";
  for (int ii = 0; ii < dimsImage; ii++)
    for (int jj = 0; jj < dimsImage; jj++) os << W_k(ii, jj) << " ";
  os << "\"" << endl;

  // write priors values
  os << "nuPrior=\"" << nu_o << "\" betaPrior=\"" << beta_o
     << "\" alphaPrior=\"" << alpha_o << "\" distMRFPrior=\"" << sigmaDist_o
     << "\"";
  os << " mPrior=\"";
  for (int ii = 0; ii < dimsImage; ii++) os << m_o(ii) << " ";
  os << "\" WPrior=\"";
  for (int ii = 0; ii < dimsImage; ii++)
    for (int jj = 0; jj < dimsImage; jj++) os << W_o(ii, jj) << " ";
...
//=============================================================
bool GaussianMixtureModelPtrComp(GaussianMixtureModel *a,
                                 GaussianMixtureModel *b) {
  return (a->id < b->id);
}
...
//===================================================================
double GaussianMixtureModel::distEllipsoid(meanPrecisionSample &p1,
                                           meanPrecisionSample &p2) {
  if (&p1 == &p2) {
    return 0.0;
  };
  ...
  SelfAdjointEigenSolver<Matrix<double, dimsImage, dimsImage> > sigma1eig(
      p1.lambda_k);
  SelfAdjointEigenSolver<Matrix<double, dimsImage, dimsImage> > sigma2eig(
      p2.lambda_k);
  ...
  if ((p1.mu_k.transpose() * p2.lambda_k * p1.mu_k)(0) <= 1.0) {
    return 0.0;
  };
  if ((p2.mu_k.transpose() * p1.lambda_k * p2.mu_k)(0) <= 1.0) {
    return 0.0;
  };

  // longer check based on algorithm described in
  //	[1] A. Lin and S. Han, ?On the Distance between Two Ellipsoids,? SIAM
  // Journal on Optimization, vol. 13, pp. 298?308, May. 2002.

  Matrix<double, dimsImage, 1> g1 = p1.mu_k;
  Matrix<double, dimsImage, 1> g2 =
      p2.mu_k;  // centers fo ball within the sphere
  ...
  while (numIter < maxIter) {
    ...
    if (isnanF(theta1) == true)
      theta1 = 0.0;  // due to numarical imprecision when dot product is very
                     // close to 1.0
    ...
    // cout<<numIter<<" "<<theta1<<" "<<theta2<<" "<<fabs(theta1-theta2)<<"
    // "<<(x2-x1).norm()<<endl;
  }

  cout << "ERROR: at GaussianMixtureModel::distEllipsoid reached maximum "
          "number iterations wihtout convergence"
       << endl;
...
void GaussianMixtureModel::writeParticlesForMatlab(string outFilename) {
  cout << "DEBUGGING: GaussianMixtureModel::writeParticlesForMatlab in "
       << outFilename << endl;
  ofstream out(outFilename.c_str());

  // first line is the expected value for each parameter in the proposal
  // distribution
  ...
  for (int kk = 0; kk < numParticles; kk++) {
    for (int ii = 0; ii < dimsImage; ii++)
      out << muLambdaSamples[kk].mu_k(ii) << " ";
    for (int ii = 0; ii < dimsImage * dimsImage; ii++)
      out << muLambdaSamples[kk].lambda_k(ii) << " ";
...
void GaussianMixtureModel::splitGaussian(GaussianMixtureModel *GM, int id_) {
  ...
  Matrix<double, dimsImage, 1> v =
      sigma1eig.eigenvectors().col(pos);  // maximum change direction
  double sigmaMin =
      sigma1eig.eigenvalues().maxCoeff();  // I can use the max coeff because I
                                           // have previously adjusted for scale

  // reset values
  W_k = (sigmaMin / nu_k) *
        Matrix<double, dimsImage, dimsImage>::Identity();  // we want to
                                                           // generate larger
                                                           // Gaussians because
                                                           // it is easier to
                                                           // compress than to
  ...
  // regularize W (just in case): not needed because the new W_k is guaranteed
...
void GaussianMixtureModel::splitGaussian(GaussianMixtureModel *GM, int id_,
                                         float *img, int imSize[dimsImage]) {
  int maxIter = 100;
  double minDistLocalMaxima2 =
      6.0 * 6.0;            // squared to avoid computing so many sqrt
  ...
  SelfAdjointEigenSolver<Matrix<double, dimsImage, dimsImage> > sigma1eig(
      sigma1);
  int xMax = 0, xMin = imSize[0] - 1, yMax = 0, yMin = imSize[1] - 1, zMax = 0,
      zMin = imSize[2] - 1;
  for (int ii = 0; ii < dimsImage; ii++) {
  ...
  yMin = max((int)0, yMin);
  zMin = max((int)0, zMin);
  xMax = min(imSize[0] - 1, xMax);
  yMax = min(imSize[1] - 1, yMax);
  zMax = min(imSize[2] - 1, zMax);

  long long int numElemBox =
      (xMax - xMin + 1) * (yMax - yMin + 1) * (zMax - zMin + 1);

  // build weights
  Matrix<double, Dynamic, Dynamic> xn(dimsImage, numElemBox);
  VectorXd wn(numElemBox);
  // order from mylib::Array (... (xn-1*dims[n-2] + xn-2)*dims[n-3] +
  // ...)*dims[0] + x0. For mylib dims[0] is x
  long long int pos = 0;
  long long int count = -1;

  sigma1 *= (-0.5);  // precompute Gaussians constants on exp.//we do not need
                     // for it to decay slower: it avoids the outsiders
                     // influence

  auxC1 = -1e32 * Matrix<double, dimsImage, 1>::Ones();
  auxC2 =
      -1e32 * Matrix<double, dimsImage, 1>::Ones();  // to help initialization
  ...
  for (int zz = zMin; zz <= zMax; zz++) {
    for (int yy = yMin; yy <= yMax; yy++) {
      ...
      for (int xx = xMin; xx <= xMax; xx++) {
        ...
        if (aux < -25)
          aux = 1.3888e-11;  // exp(-25)=1.3888e-11
        else
          aux = img[pos] * (exp(aux));
        if (aux < 0) {
          cout << "ERROR at GaussianMixtureModel::splitGaussian: weight can "
                  "not be negative!"
               << endl;
          exit(3);
        }
        ...
        if (aux > auxB1 || aux > auxB2) {
          ...
          if ((auxCol - auxC1).cwiseProduct(auxScale).squaredNorm() <
              minDistLocalMaxima2)  // very close to C1
          {
            if (aux > auxB1) {
              auxB1 = aux;
              auxC1 = auxCol;
            }
          } else if ((auxCol - auxC2).cwiseProduct(auxScale).squaredNorm() <
                     minDistLocalMaxima2) {  // very close to C2
            if (aux > auxB2) {
              auxB2 = aux;
              auxC2 = auxCol;
            }
  ...
  // delete unnecessary memory: not really needed since we access them using for
  // loops( wejust restrict teh counter)
  numElemBox = count + 1;
  ...
  GM->m_k = auxC2;
  // auxC2=m_k;//auxC2 contains the original m_k: just in case we want to verify
  // proposal
  m_k = auxC1;
  ...
  do {
    ...
    auxC1 = Matrix<double, dimsImage, 1>::Zero();
    totalW1 = 0.0;
    auxC2 = Matrix<double, dimsImage, 1>::Zero();
    totalW2 = 0.0;
    totalDist = 0.0;
  ...
  W_k = Matrix<double, dimsImage, dimsImage>::Zero();
  totalW1 = 0.0;
  GM->W_k = Matrix<double, dimsImage, dimsImage>::Zero();
  totalW2 = 0.0;
  for (long long int ii = 0; ii < numElemBox; ii++) {
    auxCol = xn.col(ii);
    d1 = (auxCol - m_k).squaredNorm();
    ...
    if (d1 < d2)  // assign to d1
    {
      ...
      totalW2 += wnPtr[ii];
      GM->W_k +=
          wnPtr[ii] * (auxCol - GM->m_k) * (auxCol - GM->m_k).transpose();
    }
  }
  if (totalW1 < 1e-8)  // empty cluster
  {
    ...
    if (W_k(dimsImage - 1, dimsImage - 1) <
  ...
  if (totalW2 < 1e-8)  // empty cluster
  {
    ...
    GM->W_k /= totalW2;
    if (GM->W_k(dimsImage - 1, dimsImage - 1) <
        1e-8)  // it means we have a 2D ellipsoid->just regularize W with any
               // value (we will ignore it in the putput anyway)
    {
      GM->W_k(dimsImage - 1, dimsImage - 1) =
          0.5 * (GM->W_k(0, 0) + GM->W_k(1, 1));
    }
    GM->W_k = GM->W_k.inverse() / GM->nu_k;
  }

  if (isnanF(GM->W_k(0))) {
    cout << "WARNING at GaussianMixtureModel::splitGaussian: precision matrix "
            "GM->W_k is nan"
         << endl;
    cout << "Id=" << id << " " << id_ << ";NumIters=" << iter
         << ";totalW1=" << totalW1 << ";totalW2=" << totalW2
         << ";totalDist=" << totalDist << ";m_kOrig=" << mkOrig.transpose()
         << ";m1=" << m_k.transpose() << ";m2=" << GM->m_k.transpose()
         << ";nu_k=" << GM->nu_k << endl;
    // exit(4);
    ...
    for (int ii = 0; ii < dimsImage; ii++)
      GM->W_k(ii, ii) =
          regularizePrecisionMatrixConstants::lambdaMax * scale[ii] * scale[ii];
  }
  if (isnanF(W_k(0))) {
    cout << "WARNING at GaussianMixtureModel::splitGaussian: precision matrix "
            "W_k is nan"
...
void GaussianMixtureModel::splitGaussian(GaussianMixtureModel *GM,int id_)
{
        ...
        //compute eigenvalue decomposition
        SelfAdjointEigenSolver<Matrix<double,dimsImage,dimsImage> >
sigma1eig(W_k*nu_k);

        int pos;
        double sigmaMax=sigma1eig.eigenvalues().minCoeff(&pos);
        Matrix<double,dimsImage,1> v=sigma1eig.eigenvectors().col(pos);//maximum
change direction
...
//==============================================================================
void GaussianMixtureModel::updatePriors(double betaPercentageOfN_k,
                                        double nuPercentageOfN_k,
                                        double alphaPercentage,
                                        double alphaTotal) {
  m_o = m_k;
  beta_o = max(betaPercentageOfN_k * N_k, 1e-5);  // it can never be zero
  nu_o = max(nuPercentageOfN_k * N_k, (double)dimsImage);  // degrees of freedom
                                                           // have to be greater
                                                           // or equal to
                                                           // dimsImage
  W_o = W_k * nu_k / nu_o;
  if (alphaTotal < 0.0)
    ...
    alpha_o = alphaTotal * minPi_kForDead *
              alphaPercentage;  // we only kill very poor mixtures. Multiplier
                                // has to be<1.0 , otherwise even with N_k=0
                                // Gsuassians will still survive
}
//==============================================================================
void GaussianMixtureModel::updatePriorsAfterSplitProposal() {
  m_o = m_k;
  beta_o = max(0.5 * N_k, 1e-5);             // it can never be zero
  nu_o = max(0.5 * N_k, (double)dimsImage);  // degrees of freedom have to be
                                             // greater or equal to dimsImage
  W_o = W_k * nu_k / nu_o;

  // do not touch alpha:it should be OK
  // alpha_o=alphaTotal*minPi_kForDead*0.8;//we only kill very poor mixtures.
  // Multiplier has to be<1.0 , otherwise even with N_k=0 Gsuassians will still
  // survive
}
//==========================================================================
...
double GaussianMixtureModel::volumeGaussian() {
  // compute eigenvalue decomposition
  SelfAdjointEigenSolver<Matrix<double, dimsImage, dimsImage> > sigma1eig(W_k *
                                                                          nu_k);
  switch (dimsImage) {
    case 3:
      return ellipsoidVolumeConstant /
             sqrt(sigma1eig.eigenvalues().prod());  // vol=(4/3)Pi*a*b*c
      break;
    case 2:
      return ellipseAreaConstant /
             sqrt(sigma1eig.eigenvalues().prod());  // area=Pi*a*b
      break;
    default:
      cout << "ERROR: ellipsid volume is not prepared for this number of "
              "dimensions"
           << endl;
      exit(3);
      return 0.0;
...
void GaussianMixtureModel::informationUpdateKalman() {
  if (dimsImage > 4) {
    // gain2.ldlt().solveInPlace(gain);//calculate the inverse efficiently using
    // the fact that covariance matrices are symmetric
    cout << "ERROR: code is not ready for more than an observation vector of 4 "
            "dimensions. Change it to use cholesky decomposition"
         << endl;
    exit(2);
  }
  ...
  MatrixObs gain2 =
      ((gain * Mobs.transpose()) + (W_k.inverse() / (beta_k * nu_k))).inverse();
