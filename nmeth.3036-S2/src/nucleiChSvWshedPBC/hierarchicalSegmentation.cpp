...
nodeHierarchicalSegmentation& nodeHierarchicalSegmentation::operator=(
    const nodeHierarchicalSegmentation& p) {
  if (this != &p) {
    thrTau = p.thrTau;
    svPtr = p.svPtr;
  }
...
nodeHierarchicalSegmentation::nodeHierarchicalSegmentation(
    istream& is,
    supervoxel* basicRegionsVec)  // create supervoxel from biunary file
{
...
void nodeHierarchicalSegmentation::writeToBinary(
    ostream& os)  // write ot binary file
{
...
hierarchicalSegmentation::hierarchicalSegmentation(
    unsigned int numBasicRegions_, int TM) {
  numBasicRegions = numBasicRegions_;
  basicRegionsVec = new supervoxel[numBasicRegions];
...
hierarchicalSegmentation::hierarchicalSegmentation(
    const hierarchicalSegmentation& p) {
...
hierarchicalSegmentation::hierarchicalSegmentation(
    istream& is)  // create supervoxel from biunary file
{
  ...
  for (int ii = 1; ii < numNodes; ii++) {
    if (parentIdx[ii] < 0) {
      cout << "ERROR: at hierarchicalSegmentation: binary file seems "
              "corrupted. Node "
    ...
    if (nodes[parentIdx[ii]]->left == NULL) {
      ...
      cout << "ERROR: at hierarchicalSegmentation: node cannot have more "
              "children. Node "
           << ii << " is also root" << endl;
      exit(5);
    }
...
void hierarchicalSegmentation::writeToBinary(
    ostream& os)  // write ot binary file
{
...
int hierarchicalSegmentation::segmentationNodesAtTau(imgVoxelType tau) {
  ...
  // explore the tree and keep all the nodes that are below tau for teh first
  // time
  queue<TreeNode<nodeHierarchicalSegmentation>*> q;
  q.push(dendrogram.pointer_mainRoot());
  ...
  while (q.empty() == false) {
    ...
    if (auxNode->getNumChildren() == 0) {
      ...
      if (auxNode->data.thrTau <=
          tau)  // we select this region and we do not look at descendants
      {
        currentSegmentationNodes.push_back(auxNode);
...
int hierarchicalSegmentation::segmentationAtTau(imgVoxelType tau) {
  ...
  /*
  //reserve allways a little bit more space in case we add some splits
  afterwards (we do not want to dynamically reallocate too much if we are
  keeping pointers)
  float extra = (float)(currentSegmentationNodes.size()) * 0.61803;//golden
  ratio
  extra = std::min( extra, 100.0f );
  extra = std::max ( extra, 10000.0f);
  currentSegmentatioSupervoxel.reserve( currentSegmentationNodes.size() +
  (size_t)(extra) );
  */
  ...
  int count = 0;
  for (vector<TreeNode<nodeHierarchicalSegmentation> *>::iterator
           iter = currentSegmentationNodes.begin();
       iter != currentSegmentationNodes.end(); ++iter, ++count) {
...
//===============================================================================
int hierarchicalSegmentation::supervoxelAtTreeNode(
    TreeNode<nodeHierarchicalSegmentation>* hsNode, supervoxel& sv) {
...
//===============================================================================
size_t hierarchicalSegmentation::supervoxelAtTreeNodeOnlySize(
...
int hierarchicalSegmentation::debugCheckDendrogramCoherence() {
  cout << "DEBUGGING: "
          "hierarchicalSegmentation::debugCheckDendrogramCoherence()==========="
          "================"
       << endl;
  // traverse tree to set id
  queue<TreeNode<nodeHierarchicalSegmentation>*> q;
  ...
  while (!q.empty()) {
    ...
    if (aux->left != NULL) {
      ...
      if (aux->left->data.thrTau > aux->data.thrTau) {
        cout << "ERROR: at debugCheckDendrogramCoherence: left son has larger "
                "tauThr than parent. Parent*"
             << aux->left->parent << ";aux*=" << aux << ";count=" << count
             << endl;
        return 3;
      }

      if (aux->left->parent != aux) {
        cout << "ERROR: at debugCheckDendrogramCoherence: parent left son do "
                "not agree. Parent*"
             << aux->left->parent << ";aux*=" << aux << ";count=" << count
             << endl;
        return 3;
    ...
    if (aux->right != NULL) {
      ...
      if (aux->right->data.thrTau > aux->data.thrTau) {
        cout << "ERROR: at debugCheckDendrogramCoherence: right son has larger "
                "tauThr than parent. Parent*"
             << aux->right->parent << ";aux*=" << aux << ";count=" << count
             << endl;
        return 3;
      }

      if (aux->right->parent != aux) {
        cout << "ERROR: at debugCheckDendrogramCoherence: parent right son do "
                "not agree"
             << endl;
        return 4;
      }
    ...
    if (aux->getNumChildren() == 1) {
      cout << "WARNING: at debugCheckDendrogramCoherence: node has only one "
              "child; count="
           << count << endl;
    }
    ...
    if (aux->getNumChildren() == 0)  // this should be a basic region
    {
      if (aux->data.svPtr == NULL) {
        cout << "ERROR: at debugCheckDendrogramCoherence: basic region has no "
                "pointer to supervoxel"
             << endl;
        return 4;
      }
      ...
      if (found == false) {
        cout << "ERROR: at debugCheckDendrogramCoherence: basic region has "
                "pointer to supervoxel but it is not in the list of "
                "basicRegionsVEc"
             << endl;
        return 4;
      }
      ...
      if (aux->data.svPtr != NULL) {
        cout << "WARNING: at debugCheckDendrogramCoherence: none basic region "
                "has pointer to supervoxel"
             << endl;
      }
    }
...
//=====================================================================================
void hierarchicalSegmentation::findVeryPersistantBasicRegions(
    vector<supervoxel*>& svVec) {
  svVec.clear();
  ...
  while (!q.empty()) {
    ...
    if (aux->getNumChildren() == 0 && aux->data.svPtr != NULL) {
      if (aux->parent->data.thrTau >= maxTau) svVec.push_back(aux->data.svPtr);
    } else {
      if (aux->left != NULL) {
        q.push(aux->left);
      }
...
void hierarchicalSegmentation::findRealisticNodes(unsigned int minSize) {
  ...
  // explore the tree using breadth first and keep all the nodes that are below
  // maxTau and the ones that have a minSize. More details in notebook February
  // 14th 2013
  queue<TreeNode<nodeHierarchicalSegmentation>*> q;
  q.push(dendrogram.pointer_mainRoot());
  TreeNode<nodeHierarchicalSegmentation>* auxNode;
...
void hierarchicalSegmentation::cleanHierarchyWithTrimming(unsigned int minSize,
                                                          unsigned int maxSize,
                                                          int devCUDA) {
  // preset parameters
  float thrJduplicates = 0.1;  // to remove duplicates after trimming
  float maxPrctile =
      0.4;  // at the most keeping maxPrctile of teh pixels in supervoxel
  int conn3Dtrim = 6;
  int conn3DIsNeigh = 74;
  int64 boundarySizeSv[dimsImage];
  int64* neighOffsetTrim =
      supervoxel::buildNeighboorhoodConnectivity(conn3Dtrim, boundarySizeSv);
  ...
  cout << "A: Number of realistic nodes with maxTau " << maxTau
       << " and minSize " << minSize << "...";
  ...
  for (vector<supervoxel>::iterator iter = currentSegmentatioSupervoxel.begin();
       iter != currentSegmentatioSupervoxel.end(); ++iter) {
    iter->trimSupervoxel<imgTypeC>(maxSize, maxPrctile, conn3Dtrim,
                                   neighOffsetTrim);
  }

  //----------------------------------------------------------------------------------
  // remove parents in which the two children are not touching after the
  // trimming
  cout << "Removing parents in which children do not neighbor after trimming"
       << endl;
  ...
  for (int ii = currentSegmentationNodes.size() - 1; ii >= 0;
       ii--)  // we start from bottom to top (breadth first, so leaves are at
              // the end) so we can remove quickly all the ancestors
  {
    if (currentSegmentationNodes[ii]->getNumChildren() == 0) continue;

    if (find(eraseIdx.begin(), eraseIdx.end(), ii) !=
        eraseIdx.end())  // node already marked for deletion
      continue;

    ch1 = currentSegmentationNodes[ii]->left;
    ch2 = currentSegmentationNodes[ii]->right;

    // check if all of them are present: it was constructed using breadth first
    // -> I do not need to search all the vector
    it = find(currentSegmentationNodes.begin() + ii,
              currentSegmentationNodes.end(), ch1);
    if (it == currentSegmentationNodes.end())  // not found
      continue;
    else
      ch1Idx = it - currentSegmentationNodes.begin();
    it = find(currentSegmentationNodes.begin() + ii,
              currentSegmentationNodes.end(), ch2);
    if (it == currentSegmentationNodes.end())  // not found
      continue;
    else
    ...
    if (currentSegmentatioSupervoxel[ch1Idx].isNeighboring(
            currentSegmentatioSupervoxel[ch2Idx], conn3DIsNeigh,
            neighOffsetIsNeigh) == false) {
      eraseIdx.push_back(ii);
      // remove all the ancestors of ii
      auxNode = currentSegmentationNodes[ii];
      while (auxNode->parent != NULL) {
        auxNode = auxNode->parent;
        it = find(currentSegmentationNodes.begin(),
                  currentSegmentationNodes.begin() + ii,
                  auxNode);  // breadth first search
  ...
  cout << "Erasing " << eraseIdx.size()
       << " parent nodes with not touching childrend out of "
       << currentSegmentationNodes.size() << " nodes..." << endl;
  ;
  ...
  for (int ii = eraseIdx.size() - 1; ii >= 0; ii--, posSwap--) {
    ...
    currentSegmentatioSupervoxel[eraseIdx[ii]] =
        currentSegmentatioSupervoxel[posSwap];
  ...
  currentSegmentationNodes.resize(currentSegmentationNodes.size() -
                                  eraseIdx.size());
  currentSegmentatioSupervoxel.resize(currentSegmentatioSupervoxel.size() -
                                      eraseIdx.size());

  //----------------------------------------------------------------------------------
  // remove the ones that after trimming are too similar (keep the one with
  // lowest tau)
  ...
  for (vector<supervoxel>::iterator iter = currentSegmentatioSupervoxel.begin();
       iter != currentSegmentatioSupervoxel.end(); ++iter) {
    iter->weightedCentroid<imgTypeC>();  // here we only need centroid for the
                                         // nearest neighbors
  }

  eraseIdx.push_back(0);  // just to enter teh for loop
  while (eraseIdx.empty() ==
         false)  // some elements could have more than maxKNN to erase
  {
    vector<vector<vector<supervoxel>::iterator> > nearestNeighborVec;
    int errK = supervoxel::nearestNeighbors(
        currentSegmentatioSupervoxel, currentSegmentatioSupervoxel,
        supervoxel::getmaxKNNCUDA(), 5.0, devCUDA,
        nearestNeighborVec);  // only elements very near by
    if (errK > 0) exit(errK);
    float auxJ;
    int neighIdx;
    eraseIdx.clear();
    for (size_t ii = 0; ii < nearestNeighborVec.size(); ii++) {
      ...
      for (size_t jj = 0; jj < nearestNeighborVec[ii].size(); jj++) {
        if (nearestNeighborVec[ii][jj] ==
            (currentSegmentatioSupervoxel.begin() + ii))
          continue;  // make sure we are not looking at ourselves
        auxJ = currentSegmentatioSupervoxel[ii].JaccardDistance(
            *(nearestNeighborVec[ii][jj]));
        JvecDuplicates.push_back(auxJ);
        if (auxJ < thrJduplicates) {
          neighIdx =
              nearestNeighborVec[ii][jj] - currentSegmentatioSupervoxel.begin();
          // mark for erasing
          if (currentSegmentationNodes[ii]->data.thrTau >
              currentSegmentationNodes[neighIdx]
                  ->data.thrTau)  // erase the one with highest tau
          {
            eraseIdx.push_back(ii);
    ...
    cout << "Erasing " << eraseIdx.size()
         << " parent nodes that are the same out of "
         << currentSegmentationNodes.size() << " nodes..." << endl;
    // keep unique elements in order so they are easy to erase
    sort(eraseIdx.begin(), eraseIdx.end());
    ...
    for (int ii = eraseIdx.size() - 1; ii >= 0; ii--, posSwap--) {
      ...
      currentSegmentatioSupervoxel[posSwap] = auxSv;
      auxNode = currentSegmentationNodes[eraseIdx[ii]];
      currentSegmentationNodes[eraseIdx[ii]] =
          currentSegmentationNodes[posSwap];
      currentSegmentationNodes[posSwap] = auxNode;
    }
    currentSegmentationNodes.resize(currentSegmentationNodes.size() -
                                    eraseIdx.size());
...
void hierarchicalSegmentation::cleanHierarchy() {
  ...
  if (dendrogram.pointer_mainRoot() == NULL || numBasicRegions <= 0) return;
  ...
  // we look for the nodes that are below maxTau for teh first time and add them
  // as new roots
  ...
  /* In reality, since maxTau is a variable part of the class, we do not need to
  lose this information. We can just propagate it trhough the code for merge /
  split. Anything above maxTau is like Infinity
  ...
  while ( q.empty() == false )
  {
          ...
          if( auxNode->data.thrTau < maxTau )//we select this region and we do
  not look at descendants
          {
  ...
  */
  ...
  while (q.empty() == false) {
    ...
    chL = auxNode->left;
    chR = auxNode->right;
    if (chL != NULL) q.push_back(chL);

    if (chR != NULL) q.push_back(chR);

    if (chR == NULL && chL == NULL) {
      if (auxNode->data.svPtr != NULL &&
          auxNode->data.svPtr->tempWildcard < 0.0f)
    ...
    if (auxNode->data.svPtr->JaccardDistance(*(chLsv)) < thrJduplicates) {
      ...
      if (q.back() == chL)
        ...
        par = q.back();
        q.pop_back();
        if (q.back() == chL) q.pop_back();
        q.push_back(par);
      }

      if (chLsv->tempWildcard > 0.0f)  // basic region about ot be
                                       // deleted->auxNode now is basic region
                                       // with the same components as chL
      {
    ...
    } else if (auxNode->data.svPtr->JaccardDistance(*(chRsv)) <
               thrJduplicates) {
      countJR++;
      // set chL in new root
      root = new TreeNode<nodeHierarchicalSegmentation>();
      ...
      if (q.back() == chR)
        ...
        par = q.back();
        q.pop_back();
        if (q.back() == chR) q.pop_back();
        q.push_back(par);
      }

      if (chRsv->tempWildcard > 0.0f)  // basic region about ot be
                                       // deleted->auxNode now is basic region
                                       // with the same components as chR
      {
    ...
    } else if (chLsv->isNeighboring(*(chRsv), conn3DIsNeigh,
                                    neighOffsetIsNeigh) ==
               false) {  // rule 2: separate both children if they do not touch
                         // after trimming
      countNeigh++;
      root = new TreeNode<nodeHierarchicalSegmentation>();
      root->parent = NULL;  // new dendrogram root
    ...
    // release memory
    if (auxNode->data.svPtr->tempWildcard < 0.0f &&
        auxNodeReinserted == false) {
      delete auxNode->data
          .svPtr;  // we are doing top to bottom->parent is not needed anymore
      auxNode->data.svPtr = NULL;
    }
    if (chL != NULL && chL->data.svPtr->tempWildcard > 0.0f) delete chLsv;
    if (chR != NULL && chR->data.svPtr->tempWildcard > 0.0f) delete chRsv;
  }
  dendrogram.SetMainRootToNULL();
  dendrogram.SetMainRoot(oldRoot);
  ...
  while (q.empty() == false) {
    ...
    if (auxNode->getNumChildren() == 1) {
      ...
      if (chL->getNumChildren() ==
          0)  // basic region->auxNode becomes basic region
      {
        auxNode->data.svPtr = chL->data.svPtr;
        if (auxNode->left != NULL)
          auxNode->left = NULL;
        ...
        auxNode->left = chL->left;
        if (chL->left != NULL) chL->left->parent = auxNode;
        auxNode->right = chL->right;
        if (chL->right != NULL) chL->right->parent = auxNode;
        delete chL;
      }
    }

    if (auxNode->left != NULL) q.push_back(auxNode->left);
    if (auxNode->right != NULL) q.push_back(auxNode->right);
  }

  // release memory
  delete[] neighOffsetIsNeigh;

  cout << "countJR = " << countJR << "; countJL = " << countJL
       << "; countNeigh = " << countNeigh << " after exploring "
       << countExploredNodes << endl;

  if (debugCheckDendrogramCoherence() > 0) {
    cout << "========WARNING: hierarchicalSegmentation::cleanHierarchy: still "
            "has some minor issues. Look Notebook April 13th "
            "2013==========================="
         << endl;
  }
}
...
//=======================================================================
//\param[in]  eraseIdx: it has to be SORTED (ASCENDING) UNIQUE set of indexes to
// erase
void hierarchicalSegmentation::eraseSupervoxelFromCurrentSegmentation(
    const vector<int>& eraseIdx) {
  if (eraseIdx.empty() == true) return;
  if (eraseIdx.back() >= currentSegmentatioSupervoxel.size()) {
    cout << "ERROR: at "
            "hierarchicalSegmentation::eraseSupervoxelFromCurrentSegmentation: "
            "last eraseIdx[ii] is larger than currentSegmentatioSupervoxel "
            "size (out of range)"
         << endl;
    exit(3);
  }
...
template <class imgTypeC>
float hierarchicalSegmentation::suggestMerge(
    TreeNode<nodeHierarchicalSegmentation>* root, supervoxel& rootSv,
    TreeNode<nodeHierarchicalSegmentation>** rootMerge, supervoxel& rootMergeSv,
    int debugRecursion) {
  ...
  *rootMerge = root->parent;
  ...
  if (rootSv.JaccardDistance(rootMergeSv) < thrJduplicates) {
    ...
    cout<<"DEBUGGING: suggestMerge: about to call recursion. debugRecursion =
    "<<debugRecursion<<". Total memory used = "<<pmc.WorkingSetSize /
    pow(2.0,20) <<"MB"<<endl;
    */
    //===========================================
    return suggestMerge<imgTypeC>(rootAux, rootSvAux, rootMerge, rootMergeSv,
                                  debugRecursion + 1);
  }
  ...
  if (fabs(((float)rootMergeSv.PixelIdxList.size()) -
           ((float)sibilingSv.PixelIdxList.size()) -
           ((float)rootSv.PixelIdxList.size())) /
  ...
  // note: I do not return the sibiling since I need to find all the current
  // segmentation elements that are descendants of the rootMerge node. Use
  // function findDescendantsInCurrentSegmentation(...)
  return rootMergeSv.mergePriorityFunction<imgTypeC>(
      rootSv, sibilingSv);  // probability of the merge
}

//==================================================================================
// suggestSplit and suggestMerge might not be reversible if daughters do not
// touch. But that is normal since if they do not touch, it is a sure split.
// Check Notebook April 17th 2013 for more details.
template <class imgTypeC>
float hierarchicalSegmentation::suggestSplit(
    TreeNode<nodeHierarchicalSegmentation>* root, supervoxel& rootSv,
  ...
  // simple checks
  if (root->getNumChildren() != 2) return 0.0f;

  // check Jaccard difference
  rootSplit[0] = root->left;
  rootSplit[1] = root->right;
  for (int aa = 0; aa < 2; aa++) {
    supervoxelAtTreeNode(rootSplit[aa], rootSplitSv[aa]);  // generate
                                                           // supervoxel
  ...
  /*
  //check that the sizes match
  if( fabs( ((float)rootSv.PixelIdxList.size()) -
  ((float)rootSplitSv[0].PixelIdxList.size()) -
  ((float)rootSplitSv[1].PixelIdxList.size()) ) / ( (float)
  rootSv.PixelIdxList.size() ) > 0.1 )
  {
          //we cannot merge
          rootMergeSv.PixelIdxList.clear();
          *rootMerge = NULL;
  ...
  */
  ...
  return (1.0f -
          rootSv.mergePriorityFunction<imgTypeC>(
              rootSplitSv[0], rootSplitSv[1]));  // 1 - probability of the merge
                                                 // = probability of split
}

//===================================================================================
int hierarchicalSegmentation::debugTestMergeSplitSuggestions(imgVoxelType tau) {
  cout << "DEBUG: hierarchicalSegmentation::debugTestMergeSplitSuggestions: "
          "EXPECTING SUPERVOXELS IN UINT16 DATA!!!!"
       << endl;
  ...
  for (size_t ii = 0; ii < currentSegmentatioSupervoxel.size(); ii++) {
    ...
    score = suggestMerge<unsigned short int>(currentSegmentationNodes[ii],
                                             currentSegmentatioSupervoxel[ii],
                                             &rootMerge, rootMergeSv);

    if (rootMerge != NULL && score > 0) {
      // find split and make sure one of the splits is the original root
      suggestSplit<unsigned short int>(rootMerge, rootMergeSv, rootSplit,
                                       rootSplitSv);

      if (rootSplit[0] != currentSegmentationNodes[ii] &&
          (rootSplit[1] != currentSegmentationNodes[ii])) {
        ...
        if (isJequivalent == false)
          cout << "WARNING: at node " << ii
               << " of current segmentation split/merge are not reversible"
               << endl;
      }

      // save suggestion in lineage to isually check
      cout << "RESULT: at node " << ii
           << " of current segmentation split/merge is reversible" << endl;
    }
  }
...
//=======================================================================================
void hierarchicalSegmentation::findDescendantsInCurrentSegmentation(
...
int hierarchicalSegmentation::debugNumberOfNodesBelowTauMax() {
  ...
  if (dendrogram.pointer_mainRoot() == NULL) return nn;
...
void hierarchicalSegmentation::debugHierarchyDepth(string filename) {
  cout << "DEBUG::hierarchicalSegmentation::debugHierarchyDepth: writing file "
          "to "
       << filename << endl;
  ...
  while (q.empty() == false) {
    ...
    if (auxNode->data.thrTau < maxTau)  // first of its kind->check depth
    {
      ...
      while (qN.empty() == false) {
        auxNode = qN.front();
        qN.pop();
        if (auxNode->left != NULL) qN.push(auxNode->left);
        if (auxNode->right != NULL) qN.push(auxNode->right);
        numN++;
      }
...
//========================================================================
void hierarchicalSegmentation::debugEstimateDeltaZsupervoxel(imgVoxelType tau,
                                                             string filename) {
  cout << "DEBUGGING:hierarchicalSegmentation::debugEstimateDeltaZsupervoxel: "
          "writing results to "
       << filename << endl;
  ...
  for (size_t ii = 0; ii < currentSegmentatioSupervoxel.size(); ii++) {
    ...
    uint64 pos =
        currentSegmentatioSupervoxel[ii].PixelIdxList[(int)(alpha * sizeSv)];
    supervoxel::getCoordinates(pos, coord);
    zBottom = coord[dimsImage - 1];

    pos = currentSegmentatioSupervoxel[ii]
              .PixelIdxList[(int)((1.0f - alpha) * sizeSv)];
    supervoxel::getCoordinates(pos, coord);
...
//========================================================================
void hierarchicalSegmentation::resetNodeIdDendrogram(
...
template void hierarchicalSegmentation::cleanHierarchyWithTrimming<float>(
    unsigned int minSize, unsigned int maxSize, int devCUDA);
template void
hierarchicalSegmentation::cleanHierarchyWithTrimming<unsigned short int>(
    unsigned int minSize, unsigned int maxSize, int devCUDA);
template void hierarchicalSegmentation::cleanHierarchyWithTrimming<
    unsigned char>(unsigned int minSize, unsigned int maxSize, int devCUDA);
...
template float hierarchicalSegmentation::suggestMerge<float>(
    TreeNode<nodeHierarchicalSegmentation>* root, supervoxel& rootSv,
    TreeNode<nodeHierarchicalSegmentation>** rootMerge, supervoxel& rootMergeSv,
    int debugRecursion);
template float hierarchicalSegmentation::suggestMerge<unsigned short int>(
    TreeNode<nodeHierarchicalSegmentation>* root, supervoxel& rootSv,
    TreeNode<nodeHierarchicalSegmentation>** rootMerge, supervoxel& rootMergeSv,
    int debugRecursion);
template float hierarchicalSegmentation::suggestMerge<unsigned char>(
    TreeNode<nodeHierarchicalSegmentation>* root, supervoxel& rootSv,
    TreeNode<nodeHierarchicalSegmentation>** rootMerge, supervoxel& rootMergeSv,
    int debugRecursion);

template float hierarchicalSegmentation::suggestSplit<float>(
    TreeNode<nodeHierarchicalSegmentation>* root, supervoxel& rootSv,
    TreeNode<nodeHierarchicalSegmentation>* rootSplit[2],
    supervoxel rootSplitSv[2]);
template float hierarchicalSegmentation::suggestSplit<unsigned short int>(
    TreeNode<nodeHierarchicalSegmentation>* root, supervoxel& rootSv,
    TreeNode<nodeHierarchicalSegmentation>* rootSplit[2],
    supervoxel rootSplitSv[2]);
template float hierarchicalSegmentation::suggestSplit<unsigned char>(
    TreeNode<nodeHierarchicalSegmentation>* root, supervoxel& rootSv,
    TreeNode<nodeHierarchicalSegmentation>* rootSplit[2],
    supervoxel rootSplitSv[2]);
