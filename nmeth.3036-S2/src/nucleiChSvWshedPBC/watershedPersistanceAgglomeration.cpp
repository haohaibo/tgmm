...
struct ForegroundVoxel {
  ...
  ForegroundVoxel(uint64 pos_, imgVoxelType val_, bool isBorder_)
      : pos(pos_),
        val(val_),
        isBorder(isBorder_){
...
struct pairPD {
  ...
  imgVoxelType getTau() { return (fMaxMergedRegion - fMergeVal); }
...
//) ^ hash<labelType>()(p.fNewMaxRegionPos) );
    ...
    return (seed ^ ((hash<labelType>()(p.fNewMaxRegionPos)) + 0x9e3779b9 +
                    (seed << 6) + (seed >> 2)));
    ...
    // http://stackoverflow.com/questions/7222143/unordered-map-hash-function-c
    size_t seed =
        (std::tr1::hash<labelType>()(p.fMaxMergedRegionPos)) + 0x9e3779b9;
    return (seed ^ ((std::tr1::hash<labelType>()(p.fNewMaxRegionPos)) +
...
struct threadWatershedData {
  ...
  threadWatershedData(const imgVoxelType* img_, const int64* imgDimsThread_,
                      uint64 offset_, imgVoxelType backgroundThr_, int conn3D_,
                      imgVoxelType tau_, set_unionC* L_, imgLabelType* imgL_)
      : img(img_),
        offset(offset_),
        backgroundThr(backgroundThr_),
        conn3D(conn3D_),
        tau(tau_),
        L(L_),
        imgL(imgL_) {
...
inline void insertNeigh(imgVoxelType* neighVal, labelType* neighLabel,
                        imgVoxelType* neighFmax, int neighSize,
                        imgVoxelType imgVal, labelType auxLabel,
                        imgVoxelType auxVal, imgVoxelType& fMaxValNeigh,
                        labelType& fMaxPosNeigh, imgVoxelType& fMaxVal,
                        labelType& fMaxPos) {
  ...
  } else if (neighVal[neighSize] == fMaxValNeigh && fMaxPosNeigh < 0)
...
int watershedPersistanceAgglomeration(const imgVoxelType* img,
                                      ...
                                      imgVoxelType tau, imgLabelType* imgL,
                                      imgLabelType* numLabels, set_unionC* L) {
  ...
  selectForegroundElements(img, imgDims, backgroundThr, conn3D, foregroundVec,
                           imgL);
...
// sort elements in P in descending order
...
#ifdef WATERSHED_TIMING_CPU
  time(&end);
  cout << "Sorting took " << difftime(end, start) << " secs for "
       << foregroundVec.size() << " voxels" << endl;
#endif
  ...
  int64* neighOffset =
      buildNeighboorhoodConnectivity(conn3D, imgDims, boundarySize);
  if (neighOffset == NULL) return 2;
  ...
  imgVoxelType* neighFmax =
      (imgVoxelType*)malloc(sizeof(imgVoxelType) * conn3D);
...
//#define MERGE_TO_REGION_FMAX
      ...
      for (int aa = 0; aa < neighSize; aa++) {
        ...
        // highest value
        if (neighLabel[aa] == fMaxPos) continue;
...
#ifdef WATERSHED_TIMING_CPU
  time(&end);
  cout << "in Watershed  main loop took " << difftime(end, start) << " secs"
       << endl;
#endif

  if (deallocateL == true)  // we are using a single-thread version, so this is
                            // the final result
  {
    // parse all the results to output
    *numLabels = 0;
    unordered_map<labelType, imgLabelType> mapLabel;
    mapLabel.rehash(ceil((foregroundVec.size() / 50) /
                         mapLabel.max_load_factor()));  // to avoid rehash.
                                                        // a.reserve(n) is the
                                                        // ame as
                                                        // a.rehash(ceil(n /
                                                        // a.max_load_factor()))
...
#endif
    for (vector<ForegroundVoxel>::const_iterator iter = foregroundVec.begin();
         iter != foregroundVec.end();
         ++iter)  // TODO: it can be faster if we access elements in order in
                  // the image for cache friendliness. Right now (iter->pos)
                  // jumps all over the image
    {
      ...
      if (mapLabelIter == mapLabel.end())  // element not found
      {
        ...
        if (L->fMax[auxL - 1] <= tau)  // merge components with background
        {
          ...
          if ((*numLabels) >= maxNumLabels) {
            cout << "ERROR: at watershedPersistanceAgglomeration: maximum "
                    "number of labels "
                 << maxNumLabels << "  reached" << endl;
            return 3;
          }
          (*numLabels)++;
...
int watershedPersistanceAgglomerationMultithread(
  ...
  cout << "WARNING!!!:at watershedPersistanceAgglomerationMultithread: "
          "multithread approach has not been completely tested. Number of "
          "final total regions changes +-3% depending on numThreads!!! I need "
          "to figure out what is causing that in the merging regions. A good "
          "clue are PLATEAUS of intensity"
       << endl;
  ...
  set_unionC* L = (set_unionC*)malloc(sizeof(set_unionC));  // we use union-find
                                                            // data structure to
                                                            // efficiently run
                                                            // algorithm
  set_union_init(L, imgSize);  // we initialize every object to null assignment

  // generate the foreground vector and sort it in order to check which regions
  // to merge
  vector<ForegroundVoxel> foregroundVec;
  const int lastDim = dimsImage - 1;
  int64* numForegroundElementsPerSlice =
      new int64[imgDims[lastDim]];  // vector in order to distirbute the load
                                    // between threads equally
  selectForegroundElements(img, imgDims, backgroundThr, conn3D, foregroundVec,
                           imgL, numForegroundElementsPerSlice);
...
// sort elements in P in descending order
...
#ifdef WATERSHED_TIMING_CPU
  time(&end);
  cout << "Sorting before merging watershed threads took "
       << difftime(end, start) << " secs for " << foregroundVec.size()
       << " voxels" << endl;
#endif

  // calculate watershed in each of the image blocks (we divide along Z for
  // cache friendliness)
  uint64 sliceOffset = imgDims[0];
  for (int ii = 1; ii < dimsImage - 1; ii++) sliceOffset *= imgDims[ii];
  int64 iniSlice = 0, endSlice = 0,
        cumSumSlice;  // used to calculate beggining and end for each thread
  uint64 offset = 0;
  int64 imgDimsThread[dimsImage];
  ...
  cout << "ERROR:watershedPersistanceAgglomerationMultithread: not fully "
          "tested and made cross-platform compatible"
       << endl;
/*
pthread_t *tid = new pthread_t[numThreads];      // array of thread IDs
vector< threadWatershedData* > tidData(numThreads);
for(int ii =0; ii<numThreads;ii++)//the last thread is special since it picks up
all the rest
{
        //calculate load distribution
        iniSlice = endSlice;
        cumSumSlice = 0;
        while(endSlice < imgDims[lastDim] && cumSumSlice <=
(numForegorundElements / numThreads  ) )
        {
                cumSumSlice += numForegroundElementsPerSlice[endSlice];
                endSlice++;
        }
        if( ii == numThreads -1)
                endSlice = imgDims[lastDim];//to guarantee all slices are
processed
        //launch thread
        imgDimsThread[lastDim] = endSlice - iniSlice;
        tidData[ii] = new threadWatershedData(img, imgDimsThread,
offset,backgroundThr, conn3D, tau, L, imgL);
        pthread_create(&(tid[ii]), NULL, &threadWatershedCallback, (void*)
(tidData[ii]) );
...
*/
...
#ifdef WATERSHED_TIMING_CPU
  time(&end);
  cout << "Fork/join threadWatershedCallback took " << difftime(end, start)
       << " secs" << endl;
#endif
  ...
  while (mergePairs.empty() == false) {
    ...
    time(&start);
#endif
    // check region merge already does the remapping. Since the last iteration
    // is the one without merges, the remapping is done appropriately
    int err = checkRegionMerge(img, foregroundVec, L, imgDims, conn3D, tau,
                               imgL, numLabels, mergePairs);
    if (err > 0) return err;

#ifdef WATERSHED_TIMING_CPU
    time(&end);
    cout << "Merging " << mergePairs.size() / 2
         << " regions for the watershed in iteration " << (numIter++)
         << " took " << difftime(end, start) << " secs" << endl;
#endif
  }
...
// imgVoxelType tau, set_unionC* L, imgLabelType *imgL)
void* threadWatershedCallback(void* p)  // adapted for pthreads now
{
  ...
  int64 imgDimsThread[dimsImage];
  for (int ii = 0; ii < dimsImage; ii++)
    imgDimsThread[ii] = ptr->imgDimsThread[ii];
  ...
  if (imgDimsThread[dimsImage - 1] == 0)
    return NULL;  // sometimes we have extra threads
  ...
  // alocate memory
  set_unionC* Lthread =
      (set_unionC*)malloc(sizeof(set_unionC));  // we use union-find data
                                                // structure to efficiently run
                                                // algorithm
  ...
  // call watershed
  int err = watershedPersistanceAgglomeration(
      &(img[offset]), imgDimsThread, backgroundThr, conn3D, tau,
      &(imgL[offset]), &numLabelsThread,
      Lthread);  // TODO: desgined a set_union structure with an offset so can
                 // use just a single L for all the threads
  if (err > 0) exit(err);

#ifdef WATERSHED_TIMING_CPU
  time(&end);
  cout << "Watershed for block with offset=" << offset << " took "
       << difftime(end, start) << " secs" << endl;
#endif
...
#ifdef WATERSHED_TIMING_CPU
  time(&end);
  cout << "Copying data for block with offset=" << offset
       << " from Lthread to global set_unionC* L took " << difftime(end, start)
       << " secs" << endl;
#endif
...
int checkRegionMerge(const imgVoxelType* img,
                     const vector<ForegroundVoxel>& foregroundVec,
                     set_unionC* L, const int64* imgDims, int conn3D,
                     imgVoxelType tau, imgLabelType* imgL,
                     imgLabelType* numLabels, vector<labelType>& mergePairs) {
  mergePairs.clear();
  ...
  // I do not need a hash table (a set would eb find) but it tends to be faster
  // for thousands of elements to identify if we have already inserted the
  // element
  unordered_map<labelType, imgLabelType> checkLabels;
  checkLabels.rehash(
      ceil((foregroundVec.size() / 50) /
           checkLabels.max_load_factor()));  // to avoid rehash. a.reserve(n) is
                                             // the ame as a.rehash(ceil(n /
                                             // a.max_load_factor()))
  checkLabels.insert(make_pair(-1, 0));
  unordered_map<labelType, imgLabelType>::const_iterator checkLabelsIter;

  (*numLabels) = 0;
  // for(uint64 posB = abs(neighOffset[0]);posB < imgSize -
  // neighOffset[conn3D-1]; posB++) // we skip the initial elementsto avoid out
  // of bounds. TODO: this is not completely clean
  int64 imgCoord[dimsImage];
  for (vector<ForegroundVoxel>::const_iterator iter = foregroundVec.begin();
       iter != foregroundVec.end(); ++iter) {
    posB = iter->pos;
    ...
    if ((*numLabels) >= maxNumLabels) {
      cout << "ERROR: at checkRegionMerge: maximum number of labels reached"
           << endl;
      return 3;
    }
    (*numLabels)++;
    ...
    if (iter->isBorder == false) {
      for (int jj = 0; jj < conn3D; jj++) {
        ...
        if (auxNeighLabel == auxL || auxL < 0)
          continue;  // they belong to the same region (== relationship) ||
                     // neighbor is part of background (-1)

        if (img[posBneigh] < iter->val)
          continue;  // neighbor has a lower image value->we should nor have
                     // "seen" it yet. PLATEAUS INTRODUCE SMALL DIFFERENCES IN
                     // MERGED REGIONS
        // neighFmax=get_fMax(L,auxNeighLabel);
        neighFmax = L->fMax[auxNeighLabel];

        if ((fmin(auxFmax, neighFmax) - iter->val) <=
            tau)  // they need to be merged
        {
          mergePairs.push_back(auxL);  // some pairs would be added multiple
                                       // times but it is not a big deal. It is
                                       // also not the bottleneck
  ...
  // merge all the pairs
  for (vector<labelType>::iterator iter = mergePairs.begin();
       iter != mergePairs.end(); ++iter) {
    auxL = *iter;
    iter++;
    // union_sets(L, fMaxPos, neighLabel[aa]);
...

{
  ...
  switch (conn3D) {
    ...
    default:
      cout << "ERROR: at watershedPersistanceAgglomeration: Code not ready for "
...

{
  if (dimsImage != 3) {
    ...
    cout << "ERROR: selectForegroundElements at: code is not ready for this "
            "kind or conn3D or this dimensionality. Conn3D = "
         << conn3D << "; dimsImage = " << dimsImage << endl;
    exit(3);
  }
...
int averageNumberOfNeighborsPerRegion(const imgLabelType* imgL,
  ...
  memset(&(avgNumberOfNeighbors[0]), 0,
         sizeof(float) * avgNumberOfNeighbors.size());
  ...
  for (int64 zz = boundarySize[2]; zz < imgDims[2] - boundarySize[2]; zz++) {
    ...
    for (int64 yy = boundarySize[1]; yy < imgDims[1] - boundarySize[1]; yy++) {
      ...
      for (int64 xx = boundarySize[0]; xx < imgDims[0] - boundarySize[0];
           xx++) {
        aux = imgL[pos2];
  ...
  for (imgLabelType ii = 0; ii < numLabels; ii++) {
    if (area[ii] > 0) avgNumberOfNeighbors[ii] /= ((float)area[ii]);
  }
...
// sort elements in P in descending order
...
#ifdef WATERSHED_TIMING_CPU
  time(&end);
  cout << "in buildH Sorting took " << difftime(end, start) << " secs for "
       << foregroundVec.size() << " voxels" << endl;
#endif
  ...
  // look up in average is constant time isntead of
  // logarithmic)
  pair<unordered_set<pairPD, pairPDKeyHash>::iterator, bool> PDhashIter;

  // to avoid rehash. a.reserve(n) is the same as
  // a.rehash(ceil(n / a.max_load_factor()))
  PDhash.rehash(ceil((foregroundVec.size() / 50) / PDhash.max_load_factor()));
  ...
  int64* neighOffset =
      buildNeighboorhoodConnectivity(conn3D, imgDims, boundarySize);
  if (neighOffset == NULL) return NULL;
...
//#define USE_MULTITHREAD: much slower because of the mutex
  ...
  imgVoxelType* neighValMT =
      (imgVoxelType*)malloc(sizeof(imgVoxelType) * conn3D);
  imgVoxelType* neighFmaxMT =
      (imgVoxelType*)malloc(sizeof(imgVoxelType) * conn3D);
  ...
  threadNeighborhoodInfoData::hEvent =
  ...
  for (int jj = 0; jj < numThreads; jj++) {
    threadHandle[jj] = CreateThread(
        NULL, 0, (LPTHREAD_START_ROUTINE) & (threadNeighborhoodInfoProc),
        (LPVOID)(&(threadData[jj])), 0, NULL);
  ...
  for (vector<ForegroundVoxel>::const_iterator iter = foregroundVec.begin();
    ...
    if (iter->isBorder == false)  // we are not in a border
    {
      ...
      for (int jj = 0; jj < conn3D; jj++) {
        ...
        if (img[posBneigh] > Pval) {
          auxLabel = find(
              L,
              posBneigh);  // this is teh bottleneck: it takes 50% of the time
          auxVal = L->fMax[auxLabel];
          insertNeigh(neighVal, neighLabel, neighFmax, neighSize,
                      img[posBneigh], auxLabel, auxVal, fMaxValNeigh,
                      fMaxPosNeigh, fMaxVal, fMaxPos);
      ...
      //	std::cout<<"Main function just updated
      // QueueSize="<<threadNeighborhoodInfoData::queueSize<<". Voxel
      //"<<countDebug<<" out of "<<foregroundVec.size()<<endl;
      LeaveCriticalSection(&(threadNeighborhoodInfoData::BufferLock));  // mutex

      // cout<<"Main function waking up all"<<endl;
      WakeConditionVariable(
          &(threadNeighborhoodInfoData::BufferNotEmpty));  // signal all the
                                                           // threads they can
                                                           // work
      ...
      */
      ...
      for (int jj = 0; jj < conn3D; jj++) {
        if (neighFmaxMT[jj] > imgVoxelMinValue)  // valid region
        {
          ...
          } else if (neighValMT[jj] == fMaxValNeigh &&
                     fMaxPosNeigh < 0)  // plateau and we still have not found a
                                        // labeled neighborhood in the plateau
          {
            fMaxValNeigh = neighValMT[jj];
            fMaxPosNeigh = neighLabelMT[jj];
          }
      ...
      for (int jj = 0; jj < conn3D; jj++) {
        ...
        posBneigh = posB + neighOffset[jj];
        if (posB < -neighOffset[jj])  // out of bounds. TODO: this is not
                                      // completely correct since we need to
                                      // check bounds from x,y,z separately.
                                      // However, most of our images have
                                      // background in the borders and it will
        ...
        /*
        auxLabel = find(L,posBneigh);
        if(auxLabel >= 0)//we only care about labels that have been assigned
        since we have sorted elements
        {
                auxVal = L->fMax[auxLabel];
                insertNeigh( neighVal,  neighLabel, neighFmax,neighSize,
        img[posBneigh],  auxLabel,  auxVal,  fMaxValNeigh, fMaxPosNeigh,
        fMaxVal, fMaxPos);
                neighSize++;
        }
        */
    ...
    // decide what to do with the value after checking neighbors: 3 cases (local
    // maximum, only one label, multiple labels -maybe merge-)
    if (Pval > fMaxValNeigh ||
        (Pval == fMaxValNeigh && fMaxPosNeigh < 0))  // we have a local maxima
                                                     // or a flat region with no
                                                     // assignments yet
    {
...
// COMMENT:VIP:THIS IS A CRITICIAL CHOICE. fMaxPos GENERERATES BETTER OUTLINES
// WHILE 0 is REALLY A PARTITION OF THE SPACE
// TODO: CHECK WHICH ONE WORKS BETTER FOR NUCLEI SEGMENTATION. A PRIORI, fMaxPos
// SEEM TO HAVE MORE LEAKAGE
// merge current pixel to the region with the AVERALL highest value
//#define MERGE_TO_REGION_FMAX
#ifdef MERGE_TO_REGION_FMAX
      add_element_to_set(L, fMaxPos, posB, Pval);
#else
      // merge current pixel to the region with the highest value in the
      // neighborhood
      add_element_to_set(L, fMaxPosNeigh, posB, Pval);
#endif
      ...
      for (int aa = 0; aa < neighSize; aa++) {
        if (neighLabel[aa] == fMaxPos)
          continue;  // we do not need to check merge since they belong already
                     // to maximum value

        if (find(PDmergedElem.begin(), PDmergedElem.end(), neighLabel[aa]) !=
            PDmergedElem.end())  // element neighLabel[aa] was already checked
          continue;

        PDmergedElem.push_back(neighLabel[aa]);

        if ((neighFmax[aa] - Pval) <=
            minTau)  // merge components using absolute value
        // TODO: add a generic "merging decision" function with common inputs so
        // I can play with different strategies
        // if((neighFmax[aa]-Pval)/neighFmax[aa]<=tau)//merge components using
        // relative value
        {
          ...
          // to build PD we would run the code with tau = Inf ( so basically we
          // would merge everything, and everytime we merge something we output
          // the pair (neighFmax[aa], Pval)
...
#ifdef WATERSHED_TIMING_CPU
  time(&end);
  cout << "in buildH main loop took " << difftime(end, start) << " secs"
       << endl;
#endif
  ...
  for (int ii = 0; ii < dimsImage; ii++) imgDimsUINT64[ii] = imgDims[ii];
...
// imgVoxelType auxFmax;
  ...
  for (vector<ForegroundVoxel>::const_iterator iter = foregroundVec.begin();
       iter != foregroundVec.end();
       ++iter)  // TODO: it can be faster if we access elements in order in the
                // image for cache friendliness. Right now (iter->pos) jumps all
                // over the image
  {
    auxL = find(
        L, iter->pos);  // here we do not need to reserve label 0 for background
    mapLabelIter = mapLabel.find(auxL);

    if (mapLabelIter == mapLabel.end())  // element not found
    {
      // auxFmax = get_fMax(L, iter->pos);
      if (L->fMax[auxL] > minTau)  // if L->fMax[auxL] < minTau, we consider
                                   // region as part of the background
      {
        // new label
        if (numBasicRegions >= numLabels) {
          cout << "ERROR: at buildHierarchicalSegmentation: maximum number of "
                  "labels "
        ...
        hs->basicRegionsVec[numBasicRegions].tempWildcard =
            L->fMax[auxL];  // we store the maximum value of this basic region
        hs->basicRegionsVec[numBasicRegions].dataPtr = (void*)(img);
        numBasicRegions++;
      }
    } else {  // element found
      // imgL [ iter->pos] = mapLabelIter->second;
      hs->basicRegionsVec[mapLabelIter->second].PixelIdxList.push_back(
          iter->pos);
    }
  }
...
#ifdef WATERSHED_TIMING_CPU
  time(&end);
  cout << "Parsing basic regions took " << difftime(end, start) << " secs"
       << endl;
#endif

// cout<<"DEBUGGING: number of basic regions "<<numBasicRegions<<"; number of
// PDvec points "<<PDhash.size()<<endl;
...
// second, build dendrogram from basic regions
  ...
  // build dendrogram (it is basically an agglomeration algorithm). Check
  // notebook February 5th 2013
  graphFA<nodeAgglomeration> agglomerateGraph(false);  // unidrected graph. We
                                                       // have to merge all the
                                                       // edges until only one
                                                       // node stands
  agglomerateGraph.reserveNodeSpace(hs->getNumberOfBasicRegions());

  // initialize node agglomeration: bottom of the binary tree containing basic
  // regions
  nodeAgglomeration auxNode;
  for (unsigned int countVec = 0; countVec < hs->getNumberOfBasicRegions();
       countVec++) {
  ...
  // cout<<"DEBUGGING:parse PD into a set of unique edges between basic regions
  // (nodes in the graph) to initialize all the edges in
  // agglomerationGraph"<<endl;
  labelType id1, id2;
  GraphEdge* auxEdge;
  ...
  for (unordered_set<pairPD, pairPDKeyHash>::iterator iterPD = PDhash.begin();
    ...
    if (id1 == id2) {
      numSelfEdges++;
      continue;  // TODO: this should not happen, but in some very large images
                 // it was crashing the code (since we get self edges). For
                 // example, in mouse 12-08-2010_TM0, 34 / 675393 nodes have
                 // self edges.
    }
    ...
    {
      agglomerateGraph.insert_edge(
          id1, id2, iterPD->fMergeVal);  // the weight is the value at which
                                         // both regions touch
    } else {                             // found->update edge
      auxEdge->weight =
          std::max(auxEdge->weight,
                   (double)(iterPD->fMergeVal));  // the weight is the value at
                                                  // which both regions touch
                                                  // (we are looking for maxima)
    }
  }

  if (numSelfEdges > 0) {
    cout << "WARNING: buildHierarchicalSegmentation: num nodes with self edges "
            "= "
         << numSelfEdges << " out of " << agglomerateGraph.getNumNodes()
         << ". Right now I have a quick fix but this should not happen" << endl;
  }
  ...
  vector<pair<GraphEdge*, imgVoxelType> > minTauVec(
      agglomerateGraph.nodes.size());
  vector<unsigned int> updateEdgeVec;
  updateEdgeVec.reserve(50);

  // initialize
  unsigned int countE = 0;
  for (vector<pair<GraphEdge *, imgVoxelType> >::iterator
           iter = minTauVec.begin();
  ...
  for(size_t nn = 0; nn < agglomerateGraph.getNumNodes(); nn++)
  {
          ...
          if(  auxD != NULL )
          {
                  cout<<"Node "<<nn<<" has a self edge with weight
  "<<auxD->weight<<endl;
          }
  }
  exit(3);
  */
  ...
  while (numNodes > 1) {
    ...
    // imgVoxelType tauE;
    // GraphEdge* edgeMin = findEdgeWithMinTau(agglomerateGraph, tauE); //this
    // function is too slow since it checks all teh edges everytime
...
#endif
    for (vector<pair<GraphEdge*, imgVoxelType> >::iterator iter =
             minTauVec.begin();
         iter != minTauVec.end(); ++iter) {
      if (iter->second < tauE) {
        tauE = iter->second;
        edgeMin = iter->first;
      }
    ...
    sum_time_while1_for1 += (double)(while1_for1_end - while1_for1_start);
#endif
    ...
    if (e1 == e2) {
      cout
          << "ERROR: buildHierarchicalSegmentation: e1 and e2 are the same node"
          << endl;
      cout << "Number of edges left in e1 = "
           << agglomerateGraph.getNumEdges(e1) << endl;
      cout << "Check point 2: iter=" << iterD << ";numNodes = " << numNodes
           << ".Selected nodes:e1=" << e1 << ";e2=" << e2 << ";tauE=" << tauE
           << endl;
      exit(3);
    }

    // cout<<"Check point 2: iter="<<(iterD)<<";numNodes =
    // "<<numNodes<<".Selected nodes:e1="<<e1<<";e2="<<e2<<";tauE="<<tauE<<endl;
    ...
    // save a list of the nodes where I have to update min tau (I use teh fact
    // that graph is bidirectional)
    updateEdgeVec.clear();
    ...
    sum_time_while1_while1 += (double)(while1_while1_end - while1_while1_start);
#endif
    ...
    sum_time_while1_while2 += (double)(while1_while2_end - while1_while2_start);
#endif

#ifdef WATERSHED_TIMING_CPU
    while1_phase1_end = omp_get_wtime();
    sum_time_while1_phase1 += (double)(while1_phase1_end - while1_phase1_start);
#endif
    ...
    sum_time_while1_sort += (double)(while1_sort_end - while1_sort_start);
#endif
    ...
    vector<unsigned int>::iterator it =
        unique(updateEdgeVec.begin(), updateEdgeVec.end());
    updateEdgeVec.resize(
        distance(updateEdgeVec.begin(), it));  // unique does not resize vector

#ifdef WATERSHED_TIMING_CPU
    while1_phase2_p1_end = omp_get_wtime();
    sum_time_while1_phase2_p1 +=
        (double)(while1_phase2_p1_end - while1_phase2_p1_start);
#endif
// cout<<"Check point 7"<<endl; //TODO: this is teh functions that seg faults
// for large specific datasets
// insert the new node (new region) into node e1 and delete e2. We have to
// recalculate all the edges from auxNode to all the edges between e1 and e2
    ...
    sum_time_while1_phase2_p2 +=
        (double)(while1_phase2_p2_end - while1_phase2_p2_start);
#endif
    ...
    for (vector<unsigned int>::iterator iter = updateEdgeVec.begin();
         iter != updateEdgeVec.end(); ++iter) {
      minTauVec[*iter].first =
          findEdgeWithMinTau(*iter, agglomerateGraph, minTauVec[*iter].second);
    }

#ifdef WATERSHED_TIMING_CPU
    while1_for2_end = omp_get_wtime();
    sum_time_while1_for2 += (double)(while1_for2_end - while1_for2_start);
#endif
    ...
    if (numNodes != agglomerateGraph.getNumNodes_Omp() + 1) {
      ...
      cout << "ERROR: buildHierarchicalSegmentation: numNodes = " << numNodes
           << "; numNodes (after agglomeration ) = "
           << agglomerateGraph.getNumNodes() << endl;
    ...
    sum_time_while1_phase2_p3 +=
        (double)(while1_phase2_p3_end - while1_phase2_p3_start);
#endif
    numNodes--;
    ...
    sum_time_while1_phase2 += (double)(while1_phase2_end - while1_phase2_start);
#endif
  }

  cout << "in buildH while1 loop " << count_while1 << " times" << endl;
#ifdef WATERSHED_TIMING_CPU
  cout << "in buildH while1 for1 took " << sum_time_while1_for1 << " secs"
       << endl;

  cout << "in buildH while1 for2 took " << sum_time_while1_for2 << " secs"
       << endl;
#endif

#ifdef WATERSHED_TIMING_CPU
  cout << "int buildH while1 while1 took " << sum_time_while1_while1 << " secs"
       << endl;
#endif

#ifdef WATERSHED_TIMING_CPU
  cout << "int buildH while1 while2 took " << sum_time_while1_while2 << " secs"
       << endl;
#endif

#ifdef WATERSHED_TIMING_CPU
  cout << "in buildH while1 sort took " << sum_time_while1_sort << " secs"
       << endl;
#endif

#ifdef WATERSHED_TIMING_CPU
  cout << "in buildH while1 phase1 took " << sum_time_while1_phase1 << " secs"
       << endl;

  cout << "in buildH while1 phase2 took " << sum_time_while1_phase2 << " secs"
       << endl;
  cout << "in buildH while1 phase2 p1 took " << sum_time_while1_phase2_p1
       << " secs" << endl;
  cout << "in buildH while1 phase2 p2 took " << sum_time_while1_phase2_p2
       << " secs" << endl;
  cout << "in buildH while1 phase2 p3 took " << sum_time_while1_phase2_p3
       << " secs" << endl;
  ...
  cout << "(new!) in buildH while1 took " << sum_time_while1 << " secs" << endl;
#ifdef WATERSHED_TIMING_CPU
  while1_end = omp_get_wtime();
  sum_time_while1 += (double)(while1_end - while1_start);

  cout << "in buildH while1 took " << sum_time_while1 << " secs" << endl;
#endif
...
#ifdef WATERSHED_TIMING_CPU
  time(&while2_end);
  cout << "in buildH while2 took " << difftime(while2_end, while2_start)
       << " secs" << endl;
#endif
...
#ifdef WATERSHED_TIMING_CPU
  time(&end);
  cout << "Building dendrogram took " << difftime(end, start) << " secs"
       << endl;
#endif
...
#ifdef WATERSHED_TIMING_CPU
  time(&buildH_end);
  cout << "in buildH total took " << difftime(buildH_end, buildH_start)
       << " secs" << endl;
#endif
  return hs;
}
...
GraphEdge* findEdgeWithMinTau(graphFA<nodeAgglomeration>& agglomerateGraph,
  ...
  for (vector<GraphEdge *>::iterator iterE = agglomerateGraph.edges.begin();
    ...
    auxEdge = (*iterE);
    while (auxEdge != NULL) {
      tauAux = min(agglomerateGraph.nodes[e1].fMax,
                   agglomerateGraph.nodes[auxEdge->e2].fMax) -
               auxEdge->weight;
      if (tauAux < tauE) {
        tauE = tauAux;
        edgeMin = auxEdge;
...
GraphEdge* findEdgeWithMinTau(unsigned int e1,
  ...
  if (e1 >= agglomerateGraph.edges.size()) return edgeMin;
