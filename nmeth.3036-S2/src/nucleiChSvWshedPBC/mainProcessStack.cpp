 ...
 * mainTest.cpp
 *
 * \brief Generates a hierachical segmentation of a 3D stack and saves teh
...
void generateSegmentationMaskFromHS(string hsFilename, int tau,
                                    size_t minSvSize);
void parseImageFilePattern(string& imgRawPath, int frame);

int main(int argc, const char** argv) {
  if (argc == 4)  // we have a .bin file from hierarchical segmentation and we
                  // want to output a segmentation for a specific tau
  {
    // Call function with ProcessStack <hsFilename.bin> <tau>
    // <minSupervoxelSize>
  ...
  string basename;  // filename without extension so we can save our binary
  ...
  if (argc == 3)  // we call program wiht <configFile> <timePoint>
  {
    ...
    basename = string(argv[1]);  // filename without extension so we can save
                                 // our binary hierarchical segmentation
    ...
    cout << "Wrong number of parameters. Call function with <imgBasename> "
            "<radiusMedianFilter> <minTau> <backgroundThr>  <conn3D>"
         << endl;
    cout
        << "Wrong number of parameters. Call function with <configFile> <frame>"
        << endl;
    return 2;
  }
  ...
  if (img == NULL) {
    ...
    if (data == NULL) {
      cout << "ERROR: could not open file " << imgFilename
           << " to read input image" << endl;
      return 5;
    }
  ...
  if (img->type != mylib::UINT16_TYPE) {
    cout << "ERROR: code is not ready for this types of images (change "
            "imgVoxelType and recompile)"
         << endl;
    return 2;
  }

  // calculate median filter
  medianFilterCUDASliceBySlice((imgVoxelType*)(img->data), img->dims,
                               radiusMedianFilter, devCUDA);

  // build hierarchical tree
  // cout<<"DEBUGGING: building hierarchical tree"<<endl;
  int64 imgDims[dimsImage];
  for (int ii = 0; ii < dimsImage; ii++) imgDims[ii] = img->dims[ii];
  hierarchicalSegmentation* hs = buildHierarchicalSegmentation(
      (imgVoxelType*)(img->data), imgDims, backgroundThr, conn3D, minTau, 1);
  ...
  if (!osTxt.is_open()) {
    cout << "ERROR: could not open file " << fileOutHS
         << " to save hierarchical segmentation paremeters" << endl;
    return 3;
  }
...
void generateSegmentationMaskFromHS(string hsFilename, int tau,
  ...
  if (fin.is_open() == false) {
    cout << "ERROR: at generateSegmentationMaskFromHS: opening file "
         << hsFilename << endl;
    exit(4);
  }
  ...
  for (int ii = 0; ii < dimsImage; ii++) dimsVec[ii] = supervoxel::dataDims[ii];

  mylib::Array* imgL = mylib::Make_Array(mylib::PLAIN_KIND, mylib::UINT16_TYPE,
                                         dimsImage, dimsVec);
  ...
  for (vector<supervoxel>::iterator iterS =
    ...
    if (iterS->PixelIdxList.size() < minSvSize) continue;
    if (numLabels == 65535) {
      cout << "ERROR: at generateSegmentationMaskFromHS: more labels than "
              "permitted in uint16"
           << endl;
      exit(4);
    }
