...
int mainTestBasic(int argc,
                  const char** argv);  // test watershed+merging algorithm
int mainTestNumberOfAvgNeighbors(int argc,
                                 const char** argv);  // counts average number
                                                      // of neighbors per voxel
                                                      // in a segmented region.
                                                      // It could be useful to
                                                      // differentiate
                                                      // foreground / background
                                                      // regions
int mainTestHierarchicalSegmentation(int argc,
                                     const char** argv);  // test hierarchical
                                                          // segmentation (the
                                                          // segmentation for
                                                          // all tau at once)
//---------------------------------------------------
...
int mainTestBasic(int argc, const char** argv) {
  int radiusMedianFilter =
      3;  // radiu sof teh emdian filter. A 3x3 median filter has radius 1

  //-------------small size---------------

  string filenameIn(
      "C:"
      "\\Users\\Fernando\\cppProjects\\nucleiChannelSupervoxelsWatershedWithPer"
      "sistanceAgglomeration\\testData\\testVol_medFilt7.mrc");
  string filenameOut(
      "C:"
      "\\Users\\Fernando\\cppProjects\\nucleiChannelSupervoxelsWatershedWithPer"
      "sistanceAgglomeration\\testData\\testVol_medFilt7_PS.bin");
  ...
  /*
  string
  filenameIn("G:\\12-07-17\\TimeFused\\Dme_E1_SpiderGFP-His2ARFP.TM0000_timeFused_blending\\SPC0_TM0000_CM0_CM1_CHN00_CHN01.fusedStack_medFilt5.mrc");
  string
  filenameOut("C:\\Users\\Fernando\\cppProjects\\nucleiChannelSupervoxelsWatershedWithPersistanceAgglomeration\\testData\\testVol_medFilt7_PS.bin");
  ...
  */
  ...
  /*
  string
  filenameIn("E:\\12-06-07\\Dre_E1_H2BeGFP_01_20120607_200041.corrected\\TimeFused\\Dre_E1_H2BeGFP.TM00000_timeFused_blending\\CM0_CM1_CHN00_CHN01.fusedStack_00000_medFilt5.mrc");
  string
  filenameOut("C:\\Users\\Fernando\\cppProjects\\nucleiChannelSupervoxelsWatershedWithPersistanceAgglomeration\\testData\\testVol_medFilt7_PS.bin");
  ...
  */
  ...
  time_t start, end;
  time(&start);
  // int err = watershedPersistanceAgglomeration(img, imgDims, backgroundThr,
  // conn3D, tau, imgL, &numLabels);
  int err = watershedPersistanceAgglomerationMultithread(
      img, imgDims, backgroundThr, conn3D, tau, imgL, &numLabels, 10);
  time(&end);
  if (err > 0) return err;
  cout << "Partitioned the image in " << numLabels << " regions took "
...
int mainTestNumberOfAvgNeighbors(int argc, const char** argv) {
  ...
  string imgLfilename(
      "G:\\12-07-17\\TimeFused_BackgrSubtraction_thrPctile40_maxSize3000_"
      "otzu\\TM00039\\CM0_CM1_CHN00_CHN01.fusedStack_bckgSub_PersistanceSeg_"
      "tau14_00039.bin");
  ...
  time_t start, end;
  time(&start);
  int err = averageNumberOfNeighborsPerRegion(imgL, imgDims, conn3D,
                                              avgNumberOfNeighbors);
  if (err > 0) return err;
  time(&end);
  ...
  fclose(fid);
  cout << "Writen solution in file " << outFilename
       << " in float32 precision with size " << imgDims[0] << "x" << imgDims[1]
       << "x" << imgDims[2] << endl;
...
int mainTestHierarchicalSegmentation(int argc, const char** argv) {
  ...
  int radiusMedianFilter =
      3;  // radius of the mdian filter. A 3x3 median filter has radius 1
  string fileOutHS(
      "C:"
      "\\Users\\Fernando\\cppProjects\\nucleiChannelSupervoxelsWatershedWithPer"
      "sistanceAgglomeration\\testData\\testHS.hs");  // file to save
                                                      // hierarchical histogram

  //-------------synthetic line---------------
  /*
  string
  filenameIn("C:\\Users\\Fernando\\cppProjects\\nucleiChannelSupervoxelsWatershedWithPersistanceAgglomeration\\testData\\syntheticLine.mrc");
  string
  ...
  imgVoxelType tau = 10;//10->3 regions; 30->2 regions; 40->2 regions; 70->1
  regions
  ...
  */
  ...
  /*
  string
  filenameIn("G:\\12-07-17\\TimeFused\\Dme_E1_SpiderGFP-His2ARFP.TM00000_timeFused_blending\\SPC0_TM0000_CM0_CM1_CHN00_CHN01.fusedStack_medFilt5.mrc");
  string
  filenameOut("C:\\Users\\Fernando\\cppProjects\\nucleiChannelSupervoxelsWatershedWithPersistanceAgglomeration\\testData\\testVol_medFilt7_PS.bin");
  int64 imgDims[dimsImage] = {1292,630,123};
  int conn3D = 74;
  ...
  */
  ...
  time(&start);
  hierarchicalSegmentation* hs = buildHierarchicalSegmentation(
      img, imgDims, backgroundThr, conn3D, minTau, 8);
  time(&end);
  cout << "Entire function buildHierarchicalSegmentation took "
       << difftime(end, start) << " secs" << endl;

  if (hs->debugCheckDendrogramCoherence() > 0) exit(3);

  // generate a segmentation for the given tau
  time(&start);
  hs->segmentationAtTau(tau);
  time(&end);
  cout << "Entire function segmentationAtTau took " << difftime(end, start)
       << " secs to generate " << hs->currentSegmentatioSupervoxel.size()
       << " labels" << endl;

  // compare with regular code for a single tau
  cout << "Calculating watershed persistance agglomeration (a single tau)..."
       << endl;
  imgLabelType numLabels;
  time(&start);
  int err = watershedPersistanceAgglomeration(img, imgDims, backgroundThr,
  ...
  for (vector<supervoxel>::iterator iterS =
           hs->currentSegmentatioSupervoxel.begin();
       iterS != hs->currentSegmentatioSupervoxel.end(); ++iterS) {
    memset(hist, 0, sizeof(int) * (numLabels + 1));
    for (vector<uint64>::iterator iter = iterS->PixelIdxList.begin();
    ...
    for (imgLabelType ii = 0; ii < numLabels + 1; ii++) {
      if (hist[ii] > maxVal) maxVal = hist[ii];
    }

    if (((float)maxVal) / ((float)iterS->PixelIdxList.size()) > 0.9)
      numUniqueAssignment++;
  }
  cout << numUniqueAssignment << " labels out of "
       << hs->currentSegmentatioSupervoxel.size() << " have unique assignments"
       << endl;
  ...
  for (vector<supervoxel>::iterator iterS =
           hsRead->currentSegmentatioSupervoxel.begin();
       iterS != hsRead->currentSegmentatioSupervoxel.end(); ++iterS) {
    memset(hist, 0, sizeof(int) * (numLabels + 1));
    for (vector<uint64>::iterator iter = iterS->PixelIdxList.begin();
         iter != iterS->PixelIdxList.end(); ++iter) {
      hist[imgL[*iter]]++;
  ...
  cout << numUniqueAssignment << " labels out of "
       << hsRead->currentSegmentatioSupervoxel.size()
       << " have unique assignments" << endl;
