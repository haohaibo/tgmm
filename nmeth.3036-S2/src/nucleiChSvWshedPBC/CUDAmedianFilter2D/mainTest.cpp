...
int main(int argc, const char** argv) {
  ...
  if (argc == 1) {
    ...
    cout << "ERROR: at mainTest. Number of input arguments is incorrect"
         << endl;
    return 2;
  }

  cout << "Testing CUDA 2D median filter with radius " << radiusMedianFilter
       << " and image size " << imSize[0] << "x" << imSize[1] << endl;
  ...
  for (int ii = 0; ii < numIter; ii++) {
    if (medianFilterCUDA(im, imSize, radiusMedianFilter, devCUDA) > 0) exit(3);
  }
  cout << "\nMedian filter calculated successfully in "
       << toc(&timerF) / (float)numIter << " secs" << endl;

  // write out results
  writeImage(im, imN * sizeof(imageType),
             "E:/temp/testMedianFilter_result.bin");
  ...
  cout << "Testing CUDA 2D median filter with radius " << radiusMedianFilter
       << " and stack slice by slice with  size " << imSize[0] << "x"
       << imSize[1] << "x" << imSize[2] << endl;
  ...
  // write out input image
  writeImage(im, imN * sizeof(imageType),
             "E:/temp/testMedianFilterSliceBySlice_input.bin");
  ...
  numIter = 1;
  for (int ii = 0; ii < numIter; ii++) {
    if (medianFilterCUDASliceBySlice(im, imSize, radiusMedianFilter, devCUDA) >
        0)
      exit(3);
  }
  cout << "\nMedian filter calculated successfully in "
  ...
  // write out results
  writeImage(im, imN * sizeof(imageType),
             "E:/temp/testMedianFilterSliceBySlice_result.bin");
