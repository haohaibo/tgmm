/*
 * \brief Graph data structure based on "The algorithm design book" by Steven S.
 * Skiena
 */
...
class graphFA {
 public:
  // main variables
  vector<ItemType> nodes;    // in case you want to hold some information in the
                             // node, we made it a template class
  vector<bool> activeNodes;  // keeps track of which nodes are active and which
                             // ones have been "deleted" (we need to do this
                             // because we use indexes to nodes instead of
                             // pointers to objects floating in different parts
                             // of memory)
  vector<GraphEdge*> edges;  // adjecincy info for each node (it is a single
                             // list node, so for each element in the graph it
                             // just has teh first edge). It should be teh same
                             // size as nodes.
  // vector<int> degrees;//degrees (number of edges) for each node. It should
  // have the same size as nodes.
  bool isDirected;
  ...
  size_t getNumNodes();  // because deleteNodes just "isolate" nodes (it does
                         // not physically delete them from memory because we
                         // would have to change all the indexes
  ...
  GraphEdge* findEdge(unsigned int e1,
                      unsigned int e2);  // finds if edge e1->e2 exists (linear
  ...
  GraphEdge* insert_edge(
      unsigned int e1, unsigned int e2, double weight,
      bool checkDirection = true);      // returns NULL if there was an error
  void insert_node(const ItemType& p);  // TODO: use activeNodes to avoid
                                        // creating new spaces if there are
                                        // inactive nodes

  // other functions
  void mergeNodes(unsigned int e1, unsigned int e2, ItemType& newNode,
                  double (*weightOp)(double, double));
  ...
  int debugCheckUnidrectionalConditions();  // if it is undirectional graph, all
                                            // edges should be duplicated
...
graphFA<ItemType>::~graphFA() {
  ...
  for (vector<GraphEdge*>::iterator iter = edges.begin(); iter != edges.end();
       ++iter) {
    nextAux = (*iter);
    while (nextAux != NULL) {
      nextAux2 = nextAux;
...
template <class ItemType>
GraphEdge* graphFA<ItemType>::insert_edge(unsigned int e1, unsigned int e2,
                                          double weight, bool checkDirection) {
  if (e1 >= nodes.size() || e2 >= nodes.size()) {
    cout << "ERROR: at graphFA<ItemType>::insert_edge: requested id for edges "
            "is larger than the current number of nodes"
         << endl;
    return NULL;
  }
...
//================================================================
template <class ItemType>
inline GraphEdge* graphFA<ItemType>::findEdge(unsigned int e1,
                                              unsigned int e2) {
  if (e1 >= nodes.size() || e2 >= nodes.size()) return NULL;
...
//==================================================================
template <class ItemType>
void graphFA<ItemType>::mergeNodes(unsigned int e1, unsigned int e2,
                                   ItemType& newNode,
                                   double (*weightOp)(double, double)) {
  if (e1 == e2) return;
  ...
  // the edges of e1 are going to be there for sure (they might be modified by
  // e2 but thats all.
  ...
  while (auxEdge != NULL) {
    ...
    {
      if (auxEdge->e2 !=
          e1)  // to make sure we do not introduce an edge to itself
      {
        ...
        if (isDirected == false)  // update the other direction
        {
          ...
          if( auxEdge2 == NULL )
          {
                  cout<<"ERROR: at mergeNodes: graph is undirected but we could
          not find counterpart (new edge)"<<endl;
                  exit(3);
          }else
          */
          {
      ...
      if (isDirected == false)  // update the other direction
      {
        auxEdge2 = findEdge(auxEdge2->e2,
                            e1);  // this edge already existed pointing at e1
        /*
        if( auxEdge2 == NULL )
        {
                cout<<"ERROR: at mergeNodes: graph is undirected but we could
        not find counterpart (edge already exists)"<<endl;
                exit(3);
        }else
        */
        { auxEdge2->weight = auxW; }
  ...
  // delete elements
  if (isDirected == true) {
    // we need to find all the edges that were pointing towards e2 and make them
    // point towards e1
...
void graphFA<ItemType>::deleteNode(unsigned int e1) {
  if (e1 >= nodes.size()) return;

  GraphEdge* auxEdge = edges[e1];
  GraphEdge* auxEdge2;
  if (isDirected == false)  // easier since we don't have to look all over the
                            // graph to find all the edges
  {
    while (auxEdge != NULL) {
...
//=================================================================
template <class ItemType>
void graphFA<ItemType>::deleteEdge(unsigned int e1, unsigned int e2,
                                   bool checkDirection) {
  if (e1 >= nodes.size() || e2 >= nodes.size()) return;
  ...
  if (isDirected == false &&
      checkDirection == true)  // delete the otehr direction
...
int graphFA<ItemType>::debugCheckUnidrectionalConditions() {
  cout << "DEBUGGING: debugCheckUnidrectionalConditions: MAKE SURE ALL EDGES "
          "ARE DUPLICATED!!!!"
       << endl;
  if (isDirected == true) return 0;
  ...
  for (size_t ii = 0; ii < edges.size(); ii++) {
    ...
    while (auxEdge != NULL) {
      ...
      if (findEdge == NULL) {
        cout << "ERROR: edge " << ii << "->" << auxEdge->e2
             << " is not duplicated at the other end" << endl;
        return 1;
      } else {
        if (findEdge->weight != auxEdge->weight) {
          cout << "ERROR: edge " << ii << "->" << auxEdge->e2
               << " have different weights " << findEdge->weight << "; "
               << auxEdge->weight << endl;
          ;
          return 2;
        }
      ...
      if (auxEdge->e1 != ii) {
        cout << "ERROR: node " << ii << " has edge e1 equal to" << auxEdge->e1
             << " (it should be the same!!!!)" << endl;
        return 3;
      }
...
int graphFA<ItemType>::debugCheckIfThereAreEdgesToItself() {
  cout << "DEBUGGING: debugCheckIfThereAreEdgesToItself: MAKE SURE THERE AREA "
          "NO EDGES TO ITSELF!!!!"
       << endl;
  if (isDirected == true) return 0;
  ...
  for (size_t ii = 0; ii < edges.size(); ii++) {
    ...
    while (auxEdge != NULL) {
      if (ii == auxEdge->e2) {
        cout << "ERROR: edge " << ii << "->" << auxEdge->e2 << " are the same!!"
             << endl;
        return 2;
      }
      auxEdge = auxEdge->next;
