...
static const int MAX_REF_POINTS =
    3000;  // we need to predefined this in order to store reference points as
...
void knnKernelCppEmule(int *indCUDA, float *queryCUDA, int ref_nb, int query_nb,
                       dimIdStruct threadIdx, dimIdStruct blockIdx,
                       dimIdStruct blockDim, dimIdStruct gridDim,
                       float *refCUDA) {
  ...
  while (tid < query_nb) {
    /*texture mmemory
    queryAux[0]=tex2D(queryTexture2D,tid,0);//stores query point to compare
    against all the references
    ...
    */
    ...
    for (int ii = 0; ii < ref_nb; ii++) {
      ...
      */
      ...
      dist += (queryAux[1] - refCUDA[refIdx + 1]) *
              (queryAux[1] - refCUDA[refIdx + 1]);
      dist += (queryAux[2] - refCUDA[refIdx + 2]) *
              (queryAux[2] - refCUDA[refIdx + 2]);
      refIdx += 3;
...
int mainTestKnnCudaCppEmule(void) {
  ...
  // consider
  int iterations = 1;  // at each iteration we will upload the query points (to
  ...
  int numThreads = 16;
  int numGrids =
      std::min(256 / numThreads,
               (query_nb + numThreads - 1) /
  ...
  for (int ii = 0; ii < iterations; ii++) {
    ...
    for (int jj = 0; jj < numGrids; jj++) {
      for (int kk = 0; kk < numThreads; kk++) {
        threadIdx.x = kk;
        blockIdx.x = jj;
        knnKernelCppEmule(ind, query, ref_nb, query_nb, threadIdx, blockIdx,
                          blockDim, gridDim, ref);
      }
    }
  ...
  for (int kk = 0; kk < query_nb; kk++) {
    printf(
        "==================Checking results for query point %d==============\n",
        kk);
