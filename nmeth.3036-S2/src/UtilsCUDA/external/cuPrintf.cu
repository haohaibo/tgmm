...
__constant__ static char *globalPrintfBuffer =
    NULL;  // Start of circular buffer (set up by host)
__constant__ static int printfBufferLength =
    0;  // Size of circular buffer (set up by host)
...
#define CUPRINTF_SM11_MAGIC \
  (unsigned short)0xC811  // Not a valid ascii character
...
__device__ static char *getNextPrintfBufPtr() {
  ...
  if ((restrictRules.threadid != CUPRINTF_UNRESTRICTED) &&
      (restrictRules.threadid != (threadIdx.x + blockDim.x * threadIdx.y +
                                  blockDim.x * blockDim.y * threadIdx.z)))
    return NULL;

// Conditional section, dependent on architecture
#if __CUDA_ARCH__ == 100
  // For sm_10 architectures, we have no atomic add - this means we must split
  // the
  // entire available buffer into per-thread blocks. Inefficient, but what can
  // you do.
  int thread_count =
  ...
  // We *must* have a thread buffer length able to fit at least two printfs (one
  // header, one real)
  ...
  if (hdr.magic != CUPRINTF_SM10_MAGIC) {
    ...
    *(cuPrintfHeaderSM10 *)(void *)myPrintfBuffer =
        hdr;  // Write back the header
    ...
    cuPrintfHeaderSM10 *tophdr =
        (cuPrintfHeaderSM10 *)(void *)globalPrintfBuffer;
  ...
  size_t offset =
      atomicAdd((unsigned int *)&printfBufferPtr, CUPRINTF_MAX_LEN) -
      (size_t)globalPrintfBuffer;
...
__device__ static void writePrintfHeader(char *ptr, char *fmtptr) {
  if (ptr) {
    ...
    header.threadid = threadIdx.x + blockDim.x * threadIdx.y +
                      blockDim.x * blockDim.y * threadIdx.z;
...
//
__device__ static char *cuPrintfStrncpy(char *dest, const char *src, int n,
                                        char *end) {
...
// partitioning.
//
static int doPrintfDisplay(int headings, int clear, char *bufstart,
                           char *bufend, char *bufptr, char *endptr) {
  ...
  while (bufptr != endptr) {
    ...
    cudaMemcpy(printfbuf_local, bufptr, CUPRINTF_MAX_LEN,
               cudaMemcpyDeviceToHost);

    // If the magic number isn't valid, then this write hasn't gone through
    // yet and we'll wait until it does (or we're past the end for non-async
    // printfs).
    cuPrintfHeader *hdr = (cuPrintfHeader *)printfbuf_local;
    if ((hdr->magic != CUPRINTF_SM11_MAGIC) ||
        (hdr->fmtoffset >= CUPRINTF_MAX_LEN)) {
...
extern "C" cudaError_t cudaPrintfDisplay(void *outputFP, bool showThreadID) {
  ...
  cudaMemcpy(&magic, printfbuf_device, sizeof(unsigned short),
             cudaMemcpyDeviceToHost);
  ...
  if (magic == CUPRINTF_SM10_MAGIC) {
    ...
    while (blockptr < (printfbuf_device + printfbuf_len)) {
      ...
      if (hdr.offset > 0) {
        // For synchronous printfs, we must print from endptr->bufend, then from
        // start->end
        if (sync_printfs)
          doPrintfDisplay(showThreadID, clearOnPrint,
                          blockptr + CUPRINTF_MAX_LEN,
  ...
  // For SM_11 and up, everything is a single buffer and it's simple
    ...
    char *bufptr = ((printfbuf_start - printfbuf_device) % printfbuf_len) +
                   printfbuf_device;
    char *endptr =
        ((printfbuf_end - printfbuf_device) % printfbuf_len) + printfbuf_device;

    // For synchronous (i.e. after-kernel-exit) printf display, we have to
    // handle circular
    // buffer wrap carefully because we could miss those past "end".
    if (sync_printfs)
      doPrintfDisplay(showThreadID, clearOnPrint, printfbuf_device,
                      printfbuf_device + printfbuf_len, endptr,
                      printfbuf_device + printfbuf_len);
    doPrintfDisplay(showThreadID, clearOnPrint, printfbuf_device,
                    printfbuf_device + printfbuf_len, bufptr, endptr);

    printfbuf_start = printfbuf_end;
  } else
    ;  // printf("Bad magic number in cuPrintf buffer header\n");

  // If we were synchronous, then we must ensure that the memory is cleared on
  // exit
