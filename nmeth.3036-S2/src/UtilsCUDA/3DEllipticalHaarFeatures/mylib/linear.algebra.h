...
*  Author:  Gene Myers *
*  Date  :  April 2007 *
*  Mod   :  June 2008 -- Added TDG's and Cubic Spline to enable snakes and
curves         *
* *
\*****************************************************************************************/
...
typedef struct {
  Double_Matrix *lu_mat;  //  LU decomposion: L is below the diagonal and U is
                          //  on and above it
...
LU_Factor *G(Copy_LU_Factor)(
    LU_Factor *f);  //  As per convention for an packable object
...
Band_Factor *G(Copy_Band_Factor)(
    Band_Factor *f);  //  As per convention for an packable object
...
Double_Matrix *Set_Matrix2Product(Double_Matrix *R(M(c)), Double_Matrix *a,
                                  boolean ta, Double_Matrix *b, boolean tb);
Double_Matrix *G(Matrix_Multiply)(Double_Matrix *a, boolean ta,
                                  Double_Matrix *b, boolean tb);
