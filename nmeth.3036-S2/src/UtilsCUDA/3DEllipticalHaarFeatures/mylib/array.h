...
{ PLAIN_KIND =
      0,  //    The outermost dimension encodes each channel (R, G, B, or A)
  RGB_KIND = 1,  //  Others may encode COMPLEX data (see below):
  RGBA_KIND =
      2,  //    The innermost dimension encodes the real and imaginary parts.
  COMPLEX_KIND = 3,
} Array_Kind;
...
typedef struct {
  Array_Kind kind;  //  Interpreation of the array: one of the four enum
                    //  constants above
...
#define AINDX(a) ((Indx_Type *)(a)->data)
#define ASIZE(a) ((Size_Type *)(a)->data)
#define AOFFS(a) ((Offs_Type *)(a)->data)
...
Array *G(Make_Array)(Array_Kind kind, Value_Type type, int ndims,
                     Dimn_Type *dims);

Array *G(Make_Array_With_Shape)(Array_Kind kind, Value_Type type,
                                Coordinate *F(shape));

Array *G(Make_Array_Of_Data)(Array_Kind kind, Value_Type type, int ndims,
                             Dimn_Type *dims, void *data);
...
Frame *G(Make_Frame)(Array *I(target), Coordinate *S(shape),
                     Coordinate *S(anchor));
...
boolean Same_Type(AForm *a,
                  AForm *b);  // Do a and b have the same shape and type?
...
AForm *G(Copy_AForm)(
    AForm *form);  //  Object-specific versions of the class primitives
...
APart *Complex_Op_Scalar(APart *R(M(a)), Operator op, Value_Type type,
                         Value rpart, Value ipart);
...
Array *Convert_Array_Inplace(Array *R(M(array)), Array_Kind kind,
                             Value_Type type, int scale,
                             ... X(int | double factor));
Array *G(Convert_Array)(Array *array, Array_Kind kind, Value_Type type,
                        int scale, ... X(int | double factor));

boolean Image_Check(Array *array);

Array *Convert_Image_Inplace(Array *R(M(array)), Array_Kind kind,
                             Value_Type type, int scale);
Array *G(Convert_Image)(Array *array, Array_Kind kind, Value_Type type,
                        int scale);
...
Array *Clip_Array_Inplace(Array *R(M(source)), Coordinate *F(beg),
                          Coordinate *F(end));
Array *G(Clip_Array)(AForm *source, Coordinate *F(beg), Coordinate *F(end));

Array *Pad_Array_Inplace(Array *R(M(source)), Coordinate *F(anchor),
                         Coordinate *F(shape));
...
Double_Vector *Simple_Regression(Array *R(O(vector)),
                                 Regression_Bundle *O(stats), AForm *obs,
                                 AForm *inp);
