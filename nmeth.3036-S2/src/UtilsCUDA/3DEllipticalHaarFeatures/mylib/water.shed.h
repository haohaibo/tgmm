...
typedef Array
    Label_Array;  //  A UINT8_, UINT16_, or UINT32_TYPE, PLAIN_KIND array
...
Partition *G(Make_Partition)(APart *I(image), Label_Array *C(M(labels)),
                             int nregions, boolean iscon2n, boolean color);
...
Partition *G(Build_Watershed)(Pixel_APart *I(image), boolean iscon2n,
                              boolean color);
...
Map_Bundle *Static_Collapse(Map_Bundle *R(O(map)), Partition *shed, void *info,
                            tristate (*handler)(int a, int h, int b,
                                                void *info));

Map_Bundle *General_Collapse(
    Map_Bundle *R(O(map)), Partition *p, boolean dynamic, void *info,
    tristate (*decide)(int a, int ab, int b, void *info),
    void (*fuse)(int a, int ac, int c, int cb, int b, void *info),
    boolean (*compare)(int a, int ab, int b, int c, int cd, int d, void *info));
