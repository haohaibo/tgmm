...
Convolver *G(Make_Convolver)(Array *I(image), Double_Array *C(filter),
                             Coordinate *C(anchor));
...
Double_Array *G(Filter_Product)(Double_Array *F(filter1),
                                Double_Array *F(filter2));
Double_Vector *G(Filter_Convolution)(Double_Vector *F(filter1),
                                     Double_Vector *F(filter2));

Double_Array *G(LOG_Filter)(int ndims, double sigma, int radius);
Double_Array *G(DOG_Filter)(int ndims, double sigma1, double sigma2,
                            int radius);
Double_Array *G(DOB_Filter)(int ndims, int radius1, int radius2);

Double_Array *G(ALOG_Filter)(int ndims, double *sigma, int *radius);
Double_Array *G(ADOG_Filter)(int ndims, double *sigma1, double *sigma2,
                             int *radius);
Double_Array *G(ADOB_Filter)(int ndims, int *radius1, int *radius2);

Double_Matrix *G(Gabor_Filter)(double aspect, double sigma, double wavelen,
                               double phase, double orient, int radius1,
                               int radius2);
...
Array *Filter_Dimension(Array *R(M(image)), Double_Vector *F(filter),
                        int dim);  //  O(RA)
...
Array *ADOG_Array(Array *R(M(image)), double *sigma1, double *sigma2,
                  int *radius);
Array *ADOB_Array(Array *R(M(image)), int *radius1, int *radius2);

Array *Median_Array(Array *R(M(image)), int radius);  //  TO BE WRITTEN!
Array *Order_Array(Array *R(M(image)), int radius,
                   int rank);  //  TO BE WRITTEN!
...
/* We offer a collection of seperable filters for computing the derivative and
   higher order
   partials, namely the simple matlab difference (Matlab), the Sobel filter
   (Sobel), and the
   beautiful and more accurate filter scheme described in Farid & Simoncelli,
   "Differentiation
   of Discrete Multidimensional Signals", IEEE Trans. Image Proc. 13, 4 (2004),
   496-507, with
   designes of span 3, 5, and 7 (Iso3, Iso5, Iso7).  Obviously the larger the
   span the more
   time the derivative or partial takes to compute, but also the more "accurate"
   the derivative.

   If you which to specify your own scheme you can do so by giving
   Make_Derivative_Scheme
   a basis vector and a derivative vector.  For example, for the classic Sobel
   filter the
   basis vector is [.25, .50, .25] and the derivative vector is [-1, 0, +1].  A
   scheme
   (other than the system provided ones) can be removed by calling
   Kill_Derivative_Scheme.
*/
...
D_Scheme *Make_Derivative_Scheme(Double_Vector *C(base),
                                 Double_Vector *C(deriv));
void Kill_Derivative_Scheme(D_Scheme *scheme);

/* The "Matlab" gradient is simple slope about a pixel in a given direction, say
   'dim'.
     Matlab_Gradient_Dim takes an 'image' and a floating point array 'grad' of
   the same shape
     and places the Matlab gradient in direction 'dim' within 'grad'.  A pointer
   to grad is
     returned as the functions result.
   Matlab_Gradient takes an image and generates an ndims x shape(image) array
   containing the
...
Float_Array *G(Partial_Array)(D_Scheme *scheme, Array *image, int n, int x1,
                              ... X(int xn));
...
/* The routines above are quite efficient as they operate over the entire array,
   but they
     are also space intensive as they require copies of the original image.  For
   example,
     for a 400Mb uint8 3D stack, Matlab_Gradient uses 400Mb * 3 * 4 = 4.8Gb!
   Often it
     suffices to have the gradient or other quantity at a pixel in one or more
   directions
     in a sweep of the image as a temporary quantity.  To acheive this we
   introduce 'iterators'
     that you setup on an image, and then you call them for the gradient(s) or
   other quantities
     as an index 'p' progresses through any slice of the underlying array (see
   array.h for the
     definition of a slice).  At the end of the sweep you need to kill the
   iterator.  The idea
     is that the iterator is setup to make the sweep computation as efficient as
   possible,
     albeit not quite as efficient as the code for the array-based routines
   above.

   For a given quantity there is a routine that sets up an iterator, which can
   be either a
     Value_Iterator of a Vector_Iterator, and then there are macro-implemented
   routines to
     get the next value or vector, and to kill the value or vector iterator as
   follows:
   ...
   For example, below, Matlab_Gradient(slice,dim), returns a value iterator that
   will advance
     over each pixel of 'slice' returning the Matlab gradient in direction dim.
   If one wants
     the gradient in every direction simultaneously, then
   Matlab_Gradient_Vector(slice), builds
     a vector iterator that returns a vector iterator that returns a vector of
   the derivatives
     in each direction for each successive pixel of slice.  Moreover, iterators
   can be built
     over iterators.  For example, Magnitute_Iterator is a value iterator that
   given a vector
...
Vector_Iterator *General_Vector(int n, Value_Iterator *C(v1),
                                ... X(Value_Iterator *C(vn)));

Value_Iterator *Derivative(D_Scheme *scheme, AForm *I(image), int dim);

Value_Iterator *Partial(D_Scheme *scheme, AForm *I(image), int n, int x1,
                        ... X(int xn));
...
Value_Iterator  *Gradient_Flow_Field(AForm *image, Value_Iterator *gradient,
double mu);
*/
...
Label_Array *G(Voronoi_Labeling)(int nseed, Indx_Type *seeds,
                                 Coordinate *F(shape));

Partition *G(Delauney_Partition)(int nseed, Indx_Type *seeds,
                                 Coordinate *F(shape));
