...
#define NUM_THREADS \
  8  //  number of simultaneously executing threads you can have on
     //    your machine (not the number of threads you can spawn).
     //    Typically this equals the number of cores.

#define M(x) \
  x  //  parameter x is (M)odified by the routine, i.e. input & output
#define O(x) x  //  parameter x is set by the routine, i.e. (O)utput only

#define G(x) \
  x  //  function is a (G)enerator.  If function returns a bundle then all
     //  objects of
...
#define S(x) \
  x  //  a reference to parameter x is (S) subsumed, i.e. made a (S)ub-object
...
typedef enum    //  Comparator ops for defining regions: see Draw_Floodfill in
                //  draw.h &
{ LE_COMP = 0,  //     Record_Region in region.h
  ...
  NE_COMP = 3,
  GT_COMP = 4,
  GE_COMP = 5 } Comparator;

typedef union {
  uint64 uval;   //    Use this variant for the 4 unsigned integers types
  int64 ival;    //    Use this variant for the 4 signed integer types
  float64 fval;  //    Use this variant for the 2 floating point number types
} Value;
...
_inline Value VALU(uint64 u) {
  Value v;
  v.uval = u;
  return v;
}
_inline Value VALI(int64 i) {
  Value v;
  v.ival = i;
  return v;
}
_inline Value VALF(float64 f) {
  Value v;
  v.fval = f;
  return v;
}
...
#define VALU(u) ((Value)(uint64)(u))
#define VALI(i) ((Value)(int64)(i))
#define VALF(f) ((Value)(float64)(f))
