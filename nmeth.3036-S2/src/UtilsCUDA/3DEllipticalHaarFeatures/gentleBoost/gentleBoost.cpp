...
int compare(const void *a, const void *b) {
  if (*(feature *)a < *(feature *)b)
    ...
    return 1;  // we assume equal sign is less than.
}
...
bool operator==(const treeStump &p1, const treeStump &p2) {
  if (p1.featureNdx != p2.featureNdx) return false;
  if (p1.gtChild != p2.gtChild) return false;
  if (p1.leChild != p2.leChild) return false;
  if (p1.parentIdx != p2.parentIdx) return false;

  if (fabs(p1.th - p2.th) > 1e-3) return false;
  if (fabs(p1.a - p2.a) > 1e-3) return false;
  if (fabs(p1.b - p2.b) > 1e-3) return false;

  return true;
}
bool operator!=(const treeStump &p1, const treeStump &p2) {
  if (p1.featureNdx != p2.featureNdx) return true;
  if (p1.gtChild != p2.gtChild) return true;
  if (p1.leChild != p2.leChild) return true;
  if (p1.parentIdx != p2.parentIdx) return true;

  if (fabs(p1.th - p2.th) / fabs(p1.th) > 1e-2) return true;
  if (fabs(p1.a - p2.a) / fabs(p1.a) > 1e-2) return true;
  if (fabs(p1.b - p2.b) / fabs(p1.b) > 1e-2) return true;

  return false;
}

//==================================================================================================================================
void gentleTreeBoost(feature *xTrain, feature *yTrain, feature *w,
                     long long int numWeakClassifiers,
  ...
  for (unsigned long long int ii = 0; ii < numSamples; ii++) {
    if (fabs(fabs(yTrain[ii]) - 1.0) > 1e-4) {
      cout << "ERROR: training set labels should be +1 or -1. Code is not "
              "ready for other alternatives"
           << endl;
      exit(2);
    }
  }

  // sort samples: we do it only once. It takes more memory but it saves a lot
  // of time. Basically sorting algorithm within the loop takes 99% time
  unsigned long long int *sortIdx =
      new unsigned long long int[numSamples * numFeatures];
  ...
  unsigned long long int numSplits =
      (unsigned long long int)pow(2.0f, (int)J) -
      1;  // number of stumps we need to define the tree
  vector<vector<unsigned long long int> > listIdTree(
      numSplits);  // stores the elemnts at each stage in the tree
  ...
  for (long long int kk = 0; kk < numWeakClassifiers; kk++) {
    ...
    for (unsigned long long int ii = 0; ii < J; ii++) {
      ...
      for (long long int jj = 0; jj < numLeaves; jj++) {
        fitTreeStumpParallelThreads(xTrain, yTrain, w, numSamples, numFeatures,
                                    sortIdx, listIdTree[aa], bestWc);
        ...
        if (bestWc.featureNdx < 0)  // sometimes leave is empty
        {
          ...
          for (vector<unsigned long long int>::const_iterator iter =
                   listIdTree[aa].begin();
               iter != listIdTree[aa].end(); ++iter) {
...
//==================================================================================================================================
void gradientTreeBoost(feature *xTrain, feature *yTrain,
                       long long int numWeakClassifiers,
                       vector<vector<treeStump> > &classifier,
                       unsigned long long int numSamples,
                       unsigned long long int numFeatures,
                       unsigned long long int J, lossFunction L) {
  treeStump bestWc;
  vector<weightAndSample> auxRegression(numSamples);
  feature *w =
      new feature[numSamples];  // needed just to be able to call fitStump
  feature *residTrain =
      new feature[numSamples];  // residuals to fit regressions tree
  feature *fmTrain =
      new feature[numSamples];  // cumulative value of boosting (additive) model
  memset(fmTrain, 0, sizeof(feature) * numSamples);
  feature auxW = 1.0 / ((feature)numSamples);
  feature delta = 1.0;  // needed for Huber function: at the beginning
                        // residTrain=yTrain->abs(residTrain=1.0 for all
  // check that yTrain\in{+1,-1}
  for (unsigned long long int ii = 0; ii < numSamples; ii++) {
    w[ii] = auxW;
    if (fabs(fabs(yTrain[ii]) - 1.0) > 1e-4) {
  ...
  vector<vector<unsigned long long int> > listIdTree(
      numSplits);  // stores the elemnts at each stage in the tree
  memcpy(residTrain, yTrain,
         numSamples * sizeof(feature));  // initialize residTrain
  for (long long int kk = 0; kk < numWeakClassifiers; kk++) {
    classifier[kk].clear();
    classifier[kk].resize(numSplits);
    long long int aa = 0;
    ...
    for (unsigned long long int ii = 0; ii < J; ii++) {
      long long int numLeaves = (long long int)pow(2.0f, (int)ii);
      for (long long int jj = 0; jj < numLeaves; jj++) {
        fitTreeStumpParallelThreads(xTrain, residTrain, w, numSamples,
                                    numFeatures, sortIdx, listIdTree[aa],
                                    bestWc);
        long long int gtChildIdx = aa + numLeaves + jj;
        long long int leChildIdx = gtChildIdx + 1;  // to save time
        // save stump
        ...
        // find the optimal values a,b according to the loss function (fitting
        // the tree to the residual only gives us the regions)
        ...
        if (leChildIdx >= 0 && listIdTree[leChildIdx].empty() == false) {
          switch (L) {
            case L_2:  // mean
            {
              feature auxMean = 0;
              for (vector<unsigned long long int>::const_iterator iter =
                       listIdTree[leChildIdx].begin();
                   iter != listIdTree[leChildIdx].end(); ++iter)
            ...
            case L_1:  // median TODO: using optimization is probably faster
            {
              ...
              for (vector<unsigned long long int>::const_iterator
                       iter = listIdTree[leChildIdx].begin();
                   iter != listIdTree[leChildIdx].end(); ++iter, count++)
            ...
            case Huber:  // requires some optimization TODO
            {
              ...
              // qsort(auxMedian,listIdTree[leChildIdx].size(),sizeof(feature),compare);
              // bestWc.b=auxMedian[listIdTree[leChildIdx].size()/2];
              bestWc.b = findHubercenter(auxMedian,
                                         listIdTree[leChildIdx].size(), delta);
              delete[] auxMedian;
              break;
            }
            default: {
        ...
        if (gtChildIdx >= 0 && listIdTree[gtChildIdx].empty() == false) {
          switch (L) {
            case L_2:  // mean
            {
              feature auxMean = 0;
              for (vector<unsigned long long int>::const_iterator iter =
                       listIdTree[gtChildIdx].begin();
                   iter != listIdTree[gtChildIdx].end(); ++iter)
                auxMean += (yTrain[*iter] - fmTrain[*iter]);
              bestWc.a =
                  auxMean / ((feature)listIdTree[gtChildIdx].size()) - bestWc.b;
              break;
            }
            case L_1:  // median TODO: using optimization is probably faster
            {
              ...
              long long int count = 0;
              for (vector<unsigned long long int>::const_iterator
                       iter = listIdTree[gtChildIdx].begin();
                   iter != listIdTree[gtChildIdx].end(); ++iter, count++)
                auxMedian[count] = (yTrain[*iter] - fmTrain[*iter]);
              // parabolic approximation tends to be faster and can be reused
              ...
              bestWc.a =
                  findL1center(auxMedian, listIdTree[gtChildIdx].size()) -
                  bestWc.b;
              delete[] auxMedian;
              break;
            }
            case Huber:  // requires some optimization
            {
              ...
              // qsort(auxMedian,listIdTree[gtChildIdx].size(),sizeof(feature),compare);
              // bestWc.a=auxMedian[listIdTree[gtChildIdx].size()/2]-bestWc.b;
              bestWc.a = findHubercenter(auxMedian,
                                         listIdTree[gtChildIdx].size(), delta) -
                         bestWc.b;
              delete[] auxMedian;
              break;
            }
    ...
    if (L == Huber) {
      ...
      for (unsigned long long int ii = 0; ii < numSamples; ii++) {
        ...
        if (fabs(fm) <= delta)
          ...
          residTrain[ii] = delta * signAmatf(fm);
      }
    }
  }
  // release memory
...
//----------------------------------------------------------------------------------------------
void boostingTreeClassifier(feature *xTest, feature *Fx,
                            vector<vector<treeStump> > &classifier,
                            unsigned long long int numSamples,
                            unsigned long long int numFeatures) {
...
//----------------------------------------------------------------------------------------------
void boostingTreeClassifierTranspose(feature *xTest, feature *Fx,
                                     vector<vector<treeStump> > &classifier,
                                     unsigned long long int numSamples,
                                     unsigned long long int numFeatures) {
  memset(Fx, 0, sizeof(feature) * numSamples);  // reset value
...
// In order to be able to reuse the code recursively, we use listId to indicate
// the points belonging to this split
// numSamples is the total number of samples
void fitTreeStump(feature *xTrain, feature *yTrain, feature *w,
                  unsigned long long int numSamples,
                  unsigned long long int numFeatures,
                  unsigned long long int *sortIdx,
                  const vector<unsigned long long int> &listId,
                  treeStump &bestWc) {
  ...
  // check for special case when all the values of yTrain are the same: the
  // solution to the minimization is undefinded
  feature aux = yTrain[listId[0]];
  bool allEqual = true;
  for (vector<unsigned long long int>::const_iterator iter = listId.begin();
       iter != listId.end(); ++iter) {
  ...
  if (allEqual) {
    ...
    for (vector<unsigned long long int>::const_iterator iter = listId.begin();
         iter != listId.end(); ++iter) {
      if (xTrain[*iter] > bestWc.th) bestWc.th = xTrain[*iter];
    }
    bestWc.a = -aux - aux;
    bestWc.b = aux;
  ...
  unsigned long long int *sortIdxAux =
      new unsigned long long int[numSamplesStump];
  unsigned char *intersection = new unsigned char[numSamples];
  memset(intersection, 0, numSamples);
  for (vector<unsigned long long int>::const_iterator iter = listId.begin();
       iter != listId.end(); ++iter)
    intersection[*iter] = '1';  // this is the same for all the features
  ...
  for (unsigned long long int ii = 0; ii < numFeatures; ii++) {
    ...
    if (numSamplesStump !=
        numSamples)  // in the first pass we can avoid this part
    {
      unsigned long long int count = 0;
      for (unsigned long long int jj = 0;
           jj < numSamples && count <= numSamplesStump; jj++) {
        if (intersection[sortIdxPtr[jj]] == '1')
          sortIdxAux[count++] = sortIdxPtr[jj];
      }
      sortIdxPtr = sortIdxAux;
      if (count != numSamplesStump) {
        cout << "ERROR: mismatch between numSamplesStump " << numSamplesStump
             << " and counted samples " << count << endl;
        exit(2);
      }
    }
    /*
    ...
    {
            unsigned long long int count=0;
            for(unsigned long long int jj=0;jj<numSamples &&
    count<=numSamplesStump;jj++)
            {
            ...
            if(count!=numSamplesStump)
            {
                    cout<<"ERROR: mismatch between numSamplesStump and counted
    samples"<<endl;
                    exit(2);
            }
    }
    ...
    for (unsigned long long int jj = 0; jj < numSamplesStump - 1; jj++) {
      ...
      // VIP: we assume yTrain \in {-1,1}. Otherwise this error formula does not
      // hold
      // check notebook October 12th 2011 for exact derivations of this error
      errorAux = b * b * partialW - 2.0 * b * cumSum[jj] +
                 aPb * aPb * (totalW - partialW) -
                 2.0 * aPb * (totalCumSum - cumSum[jj]);
...
feature fitTreeStumpToSingleFeature(
    const feature *xTrainPtr, const feature *yTrain, const feature *w,
    const unsigned long long int *sortIdxPtr, unsigned long long int numSamples,
    unsigned long long int numSamplesStump, const unsigned char *intersection,
    treeStump &bestWc, long long int featureNdx) {
  feature errorAux, errorMin = 1e32;
  feature aPb, b;
...
void fitTreeStumpParallelThreads(feature *xTrain, feature *yTrain, feature *w,
                                 unsigned long long int numSamples,
                                 unsigned long long int numFeatures,
                                 unsigned long long int *sortIdx,
                                 const vector<unsigned long long int> &listId,
...
#pragma omp parallel for
  for (long long int ii = 0; ii < (long long int)numFeatures; ii++) {
    ...
    feature *xTrainPtr = &(xTrain[ii * numSamples]);
    treeStump auxWc;
    errorVec[ii] = fitTreeStumpToSingleFeature(xTrainPtr, yTrain, w, sortIdxPtr,
                                               numSamples, numSamplesStump,
                                               intersection, auxWc, ii);
    bestWcVec[ii] = auxWc;
  }
...
feature findL1center(feature *residTrain, unsigned long long int numSamples) {
  ...
  while (fabs(x - xOld) / fabs(xOld) > 0.1 && numIter < 10) {
    xOld = x;
    x = b -
        0.5 * ((b - a) * (b - a) * (bF - cF) - (b - c) * (b - c) * (bF - aF)) /
            ((b - a) * (bF - cF) - (b - c) * (bF - aF));
...
feature findHubercenter(feature *residTrain, unsigned long long int numSamples,
                        feature delta) {
  feature a = 0, b = 0, c = 0, x = 0;
  feature aF, bF, cF, xF;  // cost associated to each point
  // initial guess is the mean
...
long long int mainTest(long long int argc, const char *argv[]) {
  if (argc != 6) {
    cout << "ERROR: wrong number of arguments. Call has to be fileTrainingSet "
            "numSamples numFeatures numWeakClassifiers numLevelsPerTree"
         << endl;
    return 1;
  }
  ...
  cout << "Reading Num features=" << numFeatures
       << ";num samples=" << numSamples << endl;
  ...
  //------------------Numerical recipes incremental estimate is the slowest by
  // far. qsort and parabolic method are on par-----------------------
  /*
  ...
  cout<<"Estimated median is "<<medianAux.report(0.5)<<" in
  "<<difftime(end,start)/numRuns2<<" secs"<<endl;
  ...
  time(&end);
  cout<<"Exact median is "<<xTrain[numSamples*numFeatures/2]<<" in
  "<<difftime(end,start)/numRuns2<<" secs"<<endl;
  ...
  cout<<"Gradient estimated median is
  "<<findL1center(xTrain,numSamples*numFeatures)<<" in
  "<<difftime(end,start)/numRuns2<<" secs"<<endl;
  ...
  cout << "Training classifier with " << numWeakClassifiers
       << " weak classifiers; Each weak classifiers is a tree with " << J
       << " levels" << endl;
  vector<vector<treeStump> > classifier;
  time(&start);
  /*
  ...
  // gentleTreeBoost(xTrain,yTrain,NULL, numWeakClassifiers,
  // classifier,numSamples,numFeatures,J);

  cout << "Using gradient boost with Huber loss function" << endl;
  gradientTreeBoost(xTrain, yTrain, numWeakClassifiers, classifier, numSamples,
                    numFeatures, J, Huber);
  ...
  for (unsigned long long int ii = 0; ii < classifier.size(); ii++) {
    ...
    for (unsigned long long int jj = 0; jj < classifier[ii].size(); jj++)
      // fout<<classifier[ii][jj].b<<" "<<classifier[ii][jj].featureNdx<<"
      // "<<classifier[ii][jj].th<<"
      // "<<classifier[ii][jj].a<<";"<<classifier[ii][jj].gtChild<<";"<<classifier[ii][jj].leChild<<";"<<endl;
      fout << "Node " << jj << ":" << classifier[ii][jj].b << " "
           << classifier[ii][jj].featureNdx << " " << classifier[ii][jj].th
           << " " << classifier[ii][jj].a << ";" << classifier[ii][jj].gtChild
           << ";" << classifier[ii][jj].leChild << ";" << endl;
  }
...
void transposeXtrainInPlace(feature *xTrain, unsigned long long int numSamples,
                            unsigned long long int numFeatures) {
  cout << "ERROR: transposeXtrainInPlace: code not ready. For non-square "
          "matrices you need to study the cycles!!!!!!!"
       << endl;
  exit(3);
...
//=============================================================================================
void transposeXtrainOutOfPlace(feature *xTrain,
                               unsigned long long int numSamples,
                               unsigned long long int numFeatures) {
  // copy memory
  long long int sizeX =
      ((long long int)(numSamples)) * ((long long int)(numFeatures));
...
//----------------------------------------------------------------------------------------------------------------
// TODO: this function could be done faster by sorting Fx (since we would try
// every single threshold just making one change at a time, but it is not worth
// the effort)
void precisionRecallAccuracyCurve(feature *yTest, feature *Fx,
                                  unsigned long long int numSamples,
                                  ostream &out, feature thrStep) {
  // feature thrStep = 0.3;
  ...
  for (feature thr = minFx; thr < maxFx;
       thr += thrStep)  // arbitrary set of thresholds
  {
    ...
    if ((tp + fp) > 0 &&
        (tp + fn) > 0)  // threshold might be too large ->no more positives
      out << thr << "," << prec << "," << recall << "," << accuracy << ";"
          << endl;  // thr, precision, recall, accuracy
    else
...
//======================================================================================================================
// From: [1] A. Niculescu-mizil and R. Caruana, “Obtaining Calibrated
// Probabilities from Boosting,” in In: Proc. 21st Conference on Uncertainty in
// Artificial Intelligence (UAI ’05), AUAI Press, 2005.
// Check Notebook January 30th 2013. P(y = 1 | Fx ) = 1 / (1 + exp( A*Fx + B );
// We look for A,B with maximum likelihood
void calibrateBoostingScoreToProbabilityPlattMethod(
    feature *yTest, feature *Fx, unsigned long long int numSamples) {
  ...
  double Amin = -3.0, Amax = 3.0,
         Astep = 0.2;  // change this to modify the grid search
  double Bmin = -3.0, Bmax = 3.0,
         Bstep = 0.2;  // change this to modify the grid search
  for (double A = Amin; A < Amax; A += Astep) {
    for (double B = Bmin; B < Bmax; B += Bstep) {
      fValAux = 0.0;
      for (vector<unsigned long long int>::const_iterator iter = posIdx.begin();
           iter != posIdx.end(); ++iter) {
        fValAux += log(1.0 + exp(A * Fx[*iter] + B));
      }
      for (vector<unsigned long long int>::const_iterator iter = negIdx.begin();
           iter != negIdx.end(); ++iter) {
        aux = A * Fx[*iter] + B;
  ...
  cout << "Optimal calibration for boosting scores into probabilities using "
          "Platt's Model  is A="
       << Astar << "; B=" << Bstar << endl;
}
//===================================================================================================================================

long long int saveClassifier(const vector<vector<treeStump> > &classifier,
                             string filename) {
  if (classifier.empty()) return 0;

  ofstream out(filename.c_str());
  if (!out.is_open()) {
    cout << "ERROR: at saveClassifier: file " << filename
         << " could not be opened to save classifier" << endl;
    return 1;
  }
  ...
  for (unsigned long long int ii = 0; ii < numWeakClassifiers; ii++) {
    out << classifier[ii].size()
        << endl;  // number of stumps per weak classifier

    for (unsigned long long int jj = 0; jj < classifier[ii].size(); jj++) {
      out << classifier[ii][jj].a << " " << classifier[ii][jj].b << " "
          << classifier[ii][jj].featureNdx << " " << classifier[ii][jj].gtChild
          << " " << classifier[ii][jj].leChild << " "
          << classifier[ii][jj].parentIdx << " " << classifier[ii][jj].th
          << endl;
    }
  }
...
long long int loadClassifier(vector<vector<treeStump> > &classifier,
                             string filename) {
  ifstream out(filename.c_str());
  if (!out.is_open()) {
    cout << "ERROR: at loadClassifier: file " << filename
         << " could not be opened to load classifier" << endl;
    return 1;
  }
  ...
  for (unsigned long long int ii = 0; ii < numWeakClassifiers; ii++) {
    ...
    for (unsigned long long int jj = 0; jj < classifier[ii].size(); jj++) {
      out >> classifier[ii][jj].a >> classifier[ii][jj].b >>
          classifier[ii][jj].featureNdx >> classifier[ii][jj].gtChild >>
          classifier[ii][jj].leChild >> classifier[ii][jj].parentIdx >>
          classifier[ii][jj].th;
    }
  }
...
long long int cleanTrainingSetTranspose(feature *xTrain, feature *yTrain,
                                        long long int *numSamples,
                                        long long int *nPos,
                                        long long int *nNeg,
                                        long long int numFeatures) {
  vector<long long int> outlierIdx;  // keeps location of outlier to move all
                                     // the memory at the end in-place
  ...
  std = sqrt((std - mean * mean) * ((feature)(*numSamples)) /
             ((feature)((*numSamples) - 1)));
  ...
  for (long long int ii = 0; ii < *numSamples; ii++) {
    if (avgVal[ii] > maxThr || avgVal[ii] < minThr) outlierIdx.push_back(ii);
  }
  outlierIdx.push_back(-1);  // to indicate end and avoid extra comparisons
                             // later
  ...
  if (outlierIdx.size() > 1) {
    ...
    long long int outlierIdxPos = 0;  // we assume outlierIdx are sorted and we
                                      // have added a -1 at the end to avoid an
                                      // extra comparison
    for (long long int ii = 0; ii < *numSamples; ii++) {
      if (outlierIdx[outlierIdxPos] == ii)  // this position is an outlier
      {
        ...
        {
          memcpy(&(xTrain[offsetNew]), &(xTrain[offsetOrig]),
  ...
  if (*numSamples != ((*nNeg) + (*nPos))) {
    cout << "ERROR: at cleanTrainingSetTranspose: numSamples does not agree"
         << endl;
    exit(3);
  }
