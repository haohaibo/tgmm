...
class IQagent {
 private:
  ...
  // data values.
  static const int
      nbuf;  // Batch size. You may x10 if you expect > 10^6 data values.
  int nq, nt, nd;
  vector<double> pval, dbuf, qile;
  double q0, qm;
  // Batch update. This function is called by add or report and should not be
 ...
 public:
  IQagent()
      : nq(251),
        ...
        qile(nq, 0.),
        q0(1.e99),
        qm(-1.e99) {  // Constructor. No arguments. 251 quantiles, the rest are
                      // linearly interpolated
    for (int j = 85; j <= 165; j++) pval[j] = (j - 75.) / 100.;
    // Set general purpose array of p-values ranging from 10^-6 to 1-10^-6. You
  ...
  double report(double p) {
    // Return estimated p-quantile for the data seen so far. (E.g., p=0.5 for
    // median.)
    double q;
    if (nd > 0) update();
    int jl = 0, jh = nq - 1, j;
    ...
    j = jl;
    q = qile[j] +
        (qile[j + 1] - qile[j]) * (p - pval[j]) / (pval[j + 1] - pval[j]);
