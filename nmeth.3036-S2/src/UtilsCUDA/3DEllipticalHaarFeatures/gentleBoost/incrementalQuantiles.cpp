...
const int IQagent::nbuf =
    1000;  // Batch size. You may x10 if you expect > 10^6 data values.

void IQagent::update(void) {
  // Batch updateThis function is called by add or report and should not be
  ...
  for (iq = 1; iq < nq - 1;
       iq++) {  // Main loop over target p-values for interpolation
    target = (nt + nd) * pval[iq];
    if (tnew < target)
      for (;
           ;) {  // Here’s the guts: We locate a succession of abscissa-ordinate
                 // pairs (qnew,tnew) that are the discontinuities of value or
                 // slope in Figure 8.5.1(c), breaking to perform an
                 // interpolation as we cross each target.
        if (jq < nq && (jd >= nd || qile[jq] < dbuf[jd])) {
          ...
          if (qile[jq] > qile[jq - 1])
            tnew += nt * (pval[jq] - pval[jq - 1]) * (qnew - qold) /
                    (qile[jq] - qile[jq - 1]);
          jd++;
          if (tnew >= target) break;
          told = tnew++;
