...
AnnotationEllipsoid::AnnotationEllipsoid(const double* mu_, const double* W_,
                                         string& imgFilename_) {
...
AnnotationEllipsoid& AnnotationEllipsoid::operator=(
    const AnnotationEllipsoid& p) {
  if (this != &p) {
    imgFilename = p.imgFilename;
    memcpy(mu, p.mu, sizeof(double) * dimsImage);
    memcpy(W, p.W, sizeof(double) * dimsImage * (dimsImage + 1) / 2);
...
ostream& AnnotationEllipsoid::writeXML(ostream& os) {
  ...
  os << "name=\""
     << "Ellipsoid"
     << "\" id=\"" << 1 << "\" numCoeffs=\"" << dimsImage * dimsImage << "\" "
     << endl;

  os << "coeffs=\"";
  for (unsigned int ii = 0; ii < dimsImage * (1 + dimsImage) / 2; ii++)
    os << W[ii] << " ";
  ...
  if (svIdx.empty() == false) {
    ...
    for (vector<int>::const_iterator iter = svIdx.begin(); iter != svIdx.end();
         ++iter) {
      os << (*iter) << " ";
    }
    os << "\"";
...
AnnotationEllipsoid::AnnotationEllipsoid(XMLNode& xml, int position) {
  ...
  int ii = 0;
  for (; ii < dimsImage * (1 + dimsImage) / 2; ii++) W[ii] = coeffs_[ii];
  int jj = 0;
  for (; ii < (int)numCoeffs_; ii++, jj++) mu[jj] = coeffs_[ii];
  ...
  if ((int)vv[0] != dimsImage) {
    cout << "ERROR: at AnnotationEllipsoid::readXML : trying to read ellipsoid "
            "of dimensions "
         << vv[0] << " using " << dimsImage
         << " dimensions. Change covarianceMatrixSize and recompile" << endl;
    exit(10);
  }
  vv.clear();
}
