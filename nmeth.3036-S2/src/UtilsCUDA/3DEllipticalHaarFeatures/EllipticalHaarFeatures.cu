...
__constant__ double r0CUDA[1];      // initial radius (based on sigma) to define
                                    // central cell ellipsoid
__constant__ double kSigmaCUDA[1];  // factor to define outer rings dimensions
                                    // in the C-HoG block
                                    // (r_m=(r0+m*kSigma)*sigma)
__constant__ int
    numCellsHEALPixCUDA[1];  // number of sectors per fixed radius using HEALpix
...
__constant__ float kernelSeparableConv[1 + 2 * maxRadiusBox];  // kernel for
                                                               // separable
                                                               // convolution.
                                                               // It does not
                                                               // make sense if
...
static const float constDoG_0 =
    2.0f * sqrt(2.0f);  // some ocnstants we need for DoG kernel
...
__device__ void vec2pix_nest(const long long int nside, double* vec,
                             long long int* ipix) {
  /* =======================================================================
  ...
  int ix = 0, iy = 0, ix_low = 0, ix_hi = 0, iy_low = 0, iy_hi = 0, ipf = 0,
      ntt = 0;
  // double piover2 = 0.5*PI_, twopi = 2.0*PI_;
  // int    ns_max = 8192;
  // int x2pix[128], y2pix[128]; //set as __constant__ in the GPU since they are
  ...
  if (za <= z0) { /* equatorial region */
    ...
    jp = (int)floor(ns_max *
                    (0.5 + tt - z * 0.75)); /* ascending edge line index */
    jm = (int)floor(ns_max *
                    (0.5 + tt + z * 0.75)); /* descending edge line index */
    ...
    if (ifp == ifm)
      face_num = (int)fmod((float)ifp, 4.0f) + 4; /* faces 4 to 7 */
    else if (ifp < ifm)
      face_num = (int)fmod((float)ifp, 4.0f); /* (half-)faces 0 to 3 */
    else
      face_num = (int)fmod((float)ifm, 4.0f) + 8; /* (half-)faces 8 to 11 */
  ...
  ipf = (long long int)(ipf / pow((float)ns_max / nside,
                                  2)); /* in {0, nside**2 - 1} */
  *ipix = (long long int)(ipf +
                          face_num * pow((float)nside,
                                         2)); /* in {0, 12*nside**2 - 1} */
}
...
//===============================beginning of eigenvalues
// kernel============================================================
// order indicates if we want to order the elements according to eigenvectors.
// order=0 ->no need to order them; order<0 -> order in descend
// order;order>0->order in ascendant order
__global__ void __launch_bounds__(MAX_THREADS_CUDA)
    computeEigenDecompositionKernel(double* W, int numEllipsoids, double* dCUDA,
                                    double* vCUDA, int order) {
...
__global__ void __launch_bounds__(MAX_THREADS_CUDA)
    applySymmetryToEigenvectorsKernel(double* vCUDA, int numEllipsoids,
                                      int symmetry) {
  ...
  if (tid < vCUDAsize) {
    ...
    switch (vPos % dimsImage) {
      case 0:  // this is a v element that will multiply X value so we need to
               // check if X needs to be flipped
        if ((symmetry & 0x01)) vCUDA[tid] *= -1.0;
        break;
      case 1:  // this is a v element that will multiply Y value so we need to
               // check if Y needs to be flipped
        if ((symmetry & 0x02)) vCUDA[tid] *= -1.0;
        break;
      case 2:  // this is a v element that will multiply Z value so we need to
               // check if Z needs to be flipped
        if ((symmetry & 0x04)) vCUDA[tid] *= -1.0;
        break;
    }
  }
}
...
// kernel===============================================
// when you defined the properties of the texture textureImage you decide what
// to do with out of bounds values and if you prefer linear or nearest neighbor
// interpolation
__global__ void __launch_bounds__(MAX_THREADS_CUDA)
    interpolate3DBoxKernel(double* mCUDA, double* vCUDA, int ellipseIdx,
                           int numEllipsoids, float* boxCUDA, int radiusBox_0,
                           int radiusBox_1, int radiusBox_2, float* meanBoxCUDA,
                           float* stdBoxCUDA) {
  int boxSize =
      (1 + 2 * radiusBox_0) * (1 + 2 * radiusBox_1) * (1 + 2 * radiusBox_2);
  int tid = threadIdx.x + blockIdx.x * blockDim.x;

  __shared__ float m[dimsImage];
  __shared__ float
      v[dimsImage * dimsImage];  // eigen vectors of the covariance matrix

  __shared__ float meanBox[MAX_THREADS_CUDA];  // stores partial sums of the
                                               // intensity to normalize the box
  __shared__ float stdBox[MAX_THREADS_CUDA];   // stores partial sums of the
                                               // intensity^2 to normalize the
                                               // box

  // read from global memory to shared memory: unfortunately we do not have
  // coalescent access
  if (threadIdx.x < dimsImage) {
    ...
    v[threadIdx.x - dimsImage] =
        vCUDA[(threadIdx.x - dimsImage) * numEllipsoids + ellipseIdx];
  }
  ...
  if (tid < boxSize) {
    ...
    val = tex3D(textureImage, xi + 0.5f, yi + 0.5f,
                zi + 0.5f);  // For CUDA, NN means just ceiling the coordinates,
                             // so we add 0.5 to really make it NN
    ...
    else if(dimsImage==2)
    {
            xi=v[0]*x+v[1]*y+m[0];
            yi=v[3]*x+v[4]*y+m[1];
            val=tex2D(textureImage,xi,yi); //I can not use tex2D with a 3D
    binded texture
    }
    */
...
__global__ void __launch_bounds__(MAX_THREADS_CUDA)
    addBoxIntensityStatisticsKernel(float* meanBoxCUDA, float* stdBoxCUDA,
                                    float* meanFinalCUDA, float* stdFinalCUDA,
                                    int boxSize) {
  ...
  if (aux2 == 0) {
    ...
    auxS = stdFinal[0] / ((float)aux) -
           auxS * auxS * ((float)boxSize) / ((float)aux);
    if (auxS < 1e-3)
      stdFinalCUDA[0] =
          1.0;  // if the Gausian is too small we might have nothing
    else
      stdFinalCUDA[0] = sqrt(auxS);
  }
}

//=========================beginning of box intensity statistics
// kernel=====================================
__global__ void __launch_bounds__(MAX_THREADS_CUDA)
    normalizeBoxKernel(float* boxCUDA, int boxSizeAux, float* meanFinalCUDA,
                       float* stdFinalCUDA) {
  __shared__ float mean;
  __shared__ float std;

  if (threadIdx.x == 0) mean = meanFinalCUDA[0];
  if (threadIdx.x == 1) std = stdFinalCUDA[0];
...
//=========================beginning of kernel to calculate cell idx for each
// voxel in a box================
__global__ void __launch_bounds__(MAX_THREADS_CUDA)
    calculateCellIdx3DKernel(double* dCUDA, int radiusBox_0, int radiusBox_1,
                             int radiusBox_2, int ellipseIdx, int numEllipsoids,
                             unsigned short int* boxCellIdxCUDA) {
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
  ...
  if (tid < boxSize) {
    ...
    aux = 1;
    aux << numAngCells;  // equivalent to (long long int)pow(2.0f,numAngCells) .
                         // Cuda 5.0 release compiled version does not retirn
                         // correct answer with pow function
...
//=======================================================================================================
// each grop of threads within a block calculates the values of convolution in a
// whole column along the direction of the separable kernel. It does not provide
// coalescent memory access along Y and Z axis but we can use the shared memory
// to store all the column at once
__global__ void __launch_bounds__(MAX_THREADS_CUDA)
    separableConvolutionKernel(float* volCUDA, int dims_0, int dims_1,
                               int dims_2, int kernelRadius, int dim) {
  __shared__ int kernelSize;

  __shared__ float colConv[maxDiameterBox];
  __shared__ float colOrig[maxDiameterBox];  // so we do not overwrite values
                                             // while computing convolution
  ...
  iniPosXYZ[offset] =
      blockIdx.x %
      dimsShared[offset];  // generate a grid withthe other dimensions
  iniPosXYZ[(dim + 2) % dimsImage] =
      (blockIdx.x - iniPosXYZ[offset]) / dimsShared[offset];
  ...
  offset = 1;  // how much do we have to skip to find next value along the
               // separable kernel axis
  for (int ii = 0; ii < dim;
       ii++)  // for ii=0->offset=1->coalescent memory access
  ...
  for (int ii = 0; ii < kernelSize; ii++) {
    if (offset < 0)  // make sure we are within bounds. TODO: allow different
                     // boundary conditions. Right no we are extending the last
                     // value.
      auxVal = colOrig[0];
...
//========================kernel to calculate the average intensity in each
// sector======================
__global__ void __launch_bounds__(MAX_THREADS_CUDA)
    countIntensityPerCellKernel(float* fCellVecCUDA, int* nCellVecCUDA,
                                float* boxCUDA,
                                unsigned short int* boxCellIdxCUDA,
                                int numCellsIdx, int boxSize) {
  ...
  if (tid < boxSize) {
    ...
    if (nAux > 0)  // we do not care about outside voxels label as zero cell
                   // index (we save a lot of atomic additions)
    {
...
//====================end of kernel to calculate the average intensity in each
// sector======================

//========================================================================================================
void calculateSeparableConvolutionBoxInCUDA(float* volCUDA,
                                            const int* volRadiusDims,
                                            const double* d,
                                            cudaStream_t& stream) {
  const int KsigmaDoG =
      5;  // kSigma*sigma defines the length of the Gaussian kernel
  int volDims[dimsImage];
  for (int ii = 0; ii < dimsImage; ii++)
    volDims[ii] = 1 + 2 * volRadiusDims[ii];

  if (maxDiameterBox > MAX_THREADS_CUDA) {
    cout << "ERROR: at calculateSeparableConvolutionBoxInCUDA. Box cannot be "
            "larger than MAX_THREADS_CUDA for this particular implementation "
            "of convolution"
         << endl;
    exit(3);
  }
  ...
  if (maxDiameterBox * maxDiameterBox > MAX_BLOCKS_CUDA) {
    cout << "ERROR: at calculateSeparableConvolutionBoxInCUDA. Box cannot be "
            "larger than MAX_BLOCKS_CUDA for this particular implementation of "
            "convolution"
         << endl;
    exit(3);
  }
  ...
  for (int ii = 1; ii < dimsImage; ii++) volSize *= volDims[ii];

  int kernelRadius = 0;
  float sigmaDoG = 0.0f, diameter = 0.0f;
  float* kernelHOST =
      new float[1 +
                2 * maxRadiusBox];  // this is the maximum size for the kernel
  float w = 0.0f;
  // calculate separable convolution for the first sigma
  for (int ii = 0; ii < dimsImage; ii++) {
    // calculate kernel
    diameter = constDoG_0 * sqrt(1.0 / d[ii]);  // usually when we plot
                                                // Gaussians they are between
                                                // 2-3 sigmas
    sigmaDoG = std::max(diameter / constDoG_1, 1.0f);
    sigmaDoG = std::min(
        sigmaDoG, (floor(float(maxRadiusBox) / ((float)KsigmaDoG * 1.6f))));
    ...
    HANDLE_ERROR(cudaMemcpyToSymbol(kernelSeparableConv, kernelHOST,
                                    sizeof(float) * (1 + 2 * kernelRadius)));
    // calculate convolution
    separableConvolutionKernel<<<volSize / volDims[ii], volDims[ii], 0,
                                 stream>>>(volCUDA, volDims[0], volDims[1],
                                           volDims[2], kernelRadius, ii);
    HANDLE_ERROR_KERNEL;
  }
...
basicEllipticalHaarFeatureVector** calculateEllipticalHaarFeatures(
    const double* m, const double* W, int numEllipsoids, const imageType* im,
    const long long int* dims, int devCUDA, int symmetry) {
  basicEllipticalHaarFeatureVector** f =
      NULL;  // vector containing the final value fo features
  HANDLE_ERROR(cudaSetDevice(devCUDA));

  if (sizeMeanStdBoxVector > MAX_THREADS_CUDA) {
    cout << "ERROR: sizeMeanStdBoxVecotr cannot be bigger than MAX_THREADS_CUDA"
         << endl;
    return f;
  }
  if (MAX_NUM_CELLS_IDX < numCellsIdx) {
    cout << "ERROR: MAX_NUM_CELLS_IDX < numCellsIdx. Chang ethe maximum size "
            "so we can allocate share memory for reduction purposes"
         << endl;
    return f;
  }

  //---------------allocate memory in GPU and transfer the
  // data----------------------------------------------
  ...
  HANDLE_ERROR(
      cudaMemcpyToSymbol(numCellsHEALPixCUDA, &numCellsHEALPix, sizeof(int)));
  ...
  // boxes are not declared as cudaArray (or binded to texture) because we
  // cannot write on cudaArrays
  float* boxCUDA = NULL;  // holds the interpolated box for each Gaussian
  float* stdBoxCUDA =
      NULL;  // partial sums of intensity^2 values to normalize box
  float* meanBoxCUDA =
      NULL;  // partial sums of intensity values to normalize box
  float* meanFinalCUDA = NULL;                // stores final value for mean
  float* stdFinalCUDA = NULL;                 // stores final value for mean
  unsigned short int* boxCellIdxCUDA = NULL;  // contains the cell id for each
                                              // voxel, so we can calculate
                                              // features
  ...
  if (dimsImage == 3) {
    ...
    /*create a cudaMemcpy3DParms structures--this is a structure that tells cuda
     * how to copy data using the cudaMemcpy3D function. Basically, this
     * prevents having to pass a bunch of parameters in favor of a single
     * complex structure.*/
    cudaMemcpy3DParms copyParmsImage = {0};
    copyParmsImage.srcPtr = make_cudaPitchedPtr(
        (void*)im, dims[0] * sizeof(imageType), dims[0], dims[1]);
  ...
  textureImage.normalized = false;  // coordinates are not between
                                    // [0,1]^dimsImage but between image size
                                    // boundaries
  textureImage.filterMode =
      cudaFilterModePoint;  // nearest neighbor interpolation. Use
                            // cudaFilterModeLinear for linear interpolation
  textureImage.addressMode[0] =
      cudaAddressModeClamp;  // How out of bounds requests are handled. For
                             // non-normalized mode only clamp is supported. In
                             // clamp addressing mode x is replaced by 0 if x<0
                             // and N-1 if x>=N;
  ...
  // allocate memory to hold the image and boxes (we just allocate the max
  // allowed since it is small enough)
  // HANDLE_ERROR( cudaMalloc( (void**)&(imCUDA), imSize*sizeof(imageType) ) );
  HANDLE_ERROR(cudaMalloc((void**)&(boxCUDA), maxBoxSize * sizeof(float)));
  HANDLE_ERROR(cudaMalloc((void**)&(boxCellIdxCUDA),
                          maxBoxSize * sizeof(unsigned short int)));

  // int sizeMeanStdBoxVector =
  // (int)ceil(((float)(maxDiameterBox*maxDiameterBox*maxDiameterBox)/((float)MAX_THREADS_CUDA));
  // we need it ot be a power of 2
  // sizeMeanStdBoxVector =
  // (int)pow(2.0f,(int)ceil(log2((float)sizeMeanStdBoxVector)));

  HANDLE_ERROR(
      cudaMalloc((void**)&(meanBoxCUDA), sizeMeanStdBoxVector * sizeof(float)));
  HANDLE_ERROR(
      cudaMalloc((void**)&(stdBoxCUDA), sizeMeanStdBoxVector * sizeof(float)));
  ...
  HANDLE_ERROR(
      cudaMalloc((void**)&(mCUDA), numEllipsoids * dimsImage * sizeof(double)));
  HANDLE_ERROR(cudaMalloc(
      (void**)&(wCUDA),
      numEllipsoids * dimsImage * ((dimsImage + 1) / 2) * sizeof(double)));

  // copy to GPU
  // HANDLE_ERROR( cudaMemcpy( imCUDA, im, imSize*sizeof(imageType) ,
  // cudaMemcpyHostToDevice ) );
  HANDLE_ERROR(cudaMemcpy(mCUDA, m, numEllipsoids * dimsImage * sizeof(double),
                          cudaMemcpyHostToDevice));
  HANDLE_ERROR(cudaMemcpy(wCUDA, W, numEllipsoids * dimsImage *
                                        ((dimsImage + 1) / 2) * sizeof(double),
                          cudaMemcpyHostToDevice));

  // allocate memory for eigenvalues and eigenvectors
  HANDLE_ERROR(
      cudaMalloc((void**)&(dCUDA), numEllipsoids * dimsImage * sizeof(double)));
  HANDLE_ERROR(cudaMalloc((void**)&(vCUDA), numEllipsoids * dimsImage *
                                                dimsImage * sizeof(double)));
  ...
  int* nCellVecHOSTDoG = NULL;    // stores the weights so we can calculate
                                  // average
  ...
  int* nCellVecCUDADoG = NULL;    // stores the weights so we can calculate
                                  // average
  // allocate as pinned memory so transfers are faster and we can use streams to
  // launch kernels in parallel with data transfer (Chapter 10 of CUDA By
  // example book)
  HANDLE_ERROR(cudaHostAlloc((void**)&fCellVecHOST, numCellsIdx * sizeof(float),
                             cudaHostAllocDefault));
  HANDLE_ERROR(cudaHostAlloc((void**)&nCellVecHOST, numCellsIdx * sizeof(int),
                             cudaHostAllocDefault));
  HANDLE_ERROR(
      cudaMalloc((void**)&(fCellVecCUDA), numCellsIdx * sizeof(float)));
  HANDLE_ERROR(cudaMalloc((void**)&(nCellVecCUDA), numCellsIdx * sizeof(int)));
  HANDLE_ERROR(cudaHostAlloc((void**)&fCellVecHOSTDoG,
                             numCellsIdx * sizeof(float),
                             cudaHostAllocDefault));
  HANDLE_ERROR(cudaHostAlloc((void**)&nCellVecHOSTDoG,
                             numCellsIdx * sizeof(int), cudaHostAllocDefault));
  HANDLE_ERROR(
      cudaMalloc((void**)&(fCellVecCUDADoG), numCellsIdx * sizeof(float)));
  HANDLE_ERROR(
      cudaMalloc((void**)&(nCellVecCUDADoG), numCellsIdx * sizeof(int)));

  //----------------calculate eigenvalues and eigenvectors for each
  // Gaussian---------------
  int numThreads = std::min(MAX_THREADS_CUDA, numEllipsoids);
  int numBlocks =
      std::min(MAX_BLOCKS_CUDA, (numEllipsoids + numThreads - 1) / numThreads);
  computeEigenDecompositionKernel<<<numBlocks, numThreads>>>(
      wCUDA, numEllipsoids, dCUDA, vCUDA, 1);
  HANDLE_ERROR_KERNEL;  // egigenvalues organized in ascend order
  ...
  if (symmetry > 7) {
    ...
    return f;
             ...
             0) {  // apply symmetry before copying eigenvalues to host
    numThreads =
        std::min(MAX_THREADS_CUDA, numEllipsoids * dimsImage * dimsImage);
    numBlocks = std::min(
        MAX_BLOCKS_CUDA,
        (numEllipsoids * dimsImage * dimsImage + numThreads - 1) / numThreads);
    applySymmetryToEigenvectorsKernel<<<numBlocks, numThreads>>>(
        vCUDA, numEllipsoids, symmetry);
    HANDLE_ERROR_KERNEL;  // egigenvalues organized in ascend order
  }

  HANDLE_ERROR(cudaMemcpy(dHOST, dCUDA,
                          sizeof(double) * numEllipsoids * dimsImage,
                          cudaMemcpyDeviceToHost));
  HANDLE_ERROR(cudaMemcpy(
      vHOST, vCUDA, sizeof(double) * numEllipsoids * dimsImage * dimsImage,
      cudaMemcpyDeviceToHost));

  //----------main for loop calculating features for each
  // ellipsoid---------------------
  // cout<<"Starting main loop to calculate features for "<<numEllipsoids<<"
  // boxes"<<endl;
  ...
  for (int ii = 0; ii < numEllipsoids; ii++) {
    ...
    HANDLE_ERROR(
        cudaMemset(meanBoxCUDA, 0, sizeof(float) * sizeMeanStdBoxVector));
    HANDLE_ERROR(
        cudaMemset(stdBoxCUDA, 0, sizeof(float) * sizeMeanStdBoxVector));
    ...
    numThreads = MAX_THREADS_CUDA;  // we do reduction (i.e. we need a power of
                                    // 2) and most of the time
                                    // boxSizeAux>MAX_THREADS_CUDA, so we just
                                    // set it to MAX_THREADS_CUDA
    numBlocks =
        std::min(MAX_BLOCKS_CUDA, (boxSizeAux + numThreads - 1) / numThreads);

    if (dimsImage == 3) {
      interpolate3DBoxKernel<<<numBlocks, numThreads>>>(
          mCUDA, vCUDA, ii, numEllipsoids, boxCUDA, radiusBox[0], radiusBox[1],
          radiusBox[2], meanBoxCUDA, stdBoxCUDA);
      HANDLE_ERROR_KERNEL;
    } else if (dimsImage == 2) {
      // TODO: I cannot define a 2D and 3D texture atthe same time->I need to
      // write separate codes or thing how to do it
    ...
    addBoxIntensityStatisticsKernel<<<1, sizeMeanStdBoxVector>>>(
        meanBoxCUDA, stdBoxCUDA, meanFinalCUDA, stdFinalCUDA, boxSizeAux);
    HANDLE_ERROR_KERNEL;
    normalizeBoxKernel<<<numBlocks, numThreads>>>(boxCUDA, boxSizeAux,
                                                  meanFinalCUDA, stdFinalCUDA);
    HANDLE_ERROR_KERNEL;

    //----------------------------------------------------
    // calculate basic features values (sectors) for the box
    // figure out the cell for each voxel in the box
    calculateCellIdx3DKernel<<<numBlocks, numThreads>>>(
        dCUDA, radiusBox[0], radiusBox[1], radiusBox[2], ii, numEllipsoids,
        boxCellIdxCUDA);
    HANDLE_ERROR_KERNEL;
    ...
    //-------------------debug-------------------------------
    /*
    cout<<"DEBUGGING: saving box to E:\\temp\\box.bin with size
    "<<(1+2*radiusBox[0])<<","<<(1+2*radiusBox[1])<<","<<(1+2*radiusBox[2])<<endl;
    float* boxHOST = new float[boxSizeAux];
    HANDLE_ERROR( cudaMemcpy( boxHOST, boxCUDA, boxSizeAux*sizeof(float) ,
    cudaMemcpyDeviceToHost) );
    ...
    HANDLE_ERROR( cudaMemcpy( boxCellIdxHOST, boxCellIdxCUDA,
    boxSizeAux*sizeof(unsigned short int) , cudaMemcpyDeviceToHost) );
    ...
    */
    ...
    countIntensityPerCellKernel<<<numBlocks, numThreads, 0, stream0>>>(
        fCellVecCUDA, nCellVecCUDA, boxCUDA, boxCellIdxCUDA, numCellsIdx,
        boxSizeAux);
    HANDLE_ERROR(cudaMemcpyAsync(fCellVecHOST, fCellVecCUDA,
                                 numCellsIdx * sizeof(float),
                                 cudaMemcpyDeviceToHost, stream0));
    HANDLE_ERROR(cudaMemcpyAsync(nCellVecHOST, nCellVecCUDA,
                                 numCellsIdx * sizeof(int),
                                 cudaMemcpyDeviceToHost, stream0));
    ...
    countIntensityPerCellKernel<<<numBlocks, numThreads, 0, stream1>>>(
        fCellVecCUDADoG, nCellVecCUDADoG, boxCUDA, boxCellIdxCUDA, numCellsIdx,
        boxSizeAux);

    // calculate values of f for each cell
    HANDLE_ERROR(cudaMemcpyAsync(fCellVecHOSTDoG, fCellVecCUDADoG,
                                 numCellsIdx * sizeof(float),
                                 cudaMemcpyDeviceToHost, stream1));
    HANDLE_ERROR(cudaMemcpyAsync(nCellVecHOSTDoG, nCellVecCUDADoG,
                                 numCellsIdx * sizeof(int),
                                 cudaMemcpyDeviceToHost, stream1));
    ...
    HANDLE_ERROR( cudaMemcpy( boxHOST, boxCUDA, boxSizeAux*sizeof(float) ,
    cudaMemcpyDeviceToHost) );
    cout<<"DEBUGGING: saving box to E:\\temp\\boxDoG.bin with size
    "<<(1+2*radiusBox[0])<<","<<(1+2*radiusBox[1])<<","<<(1+2*radiusBox[2])<<endl;
    ...
    {
            cout<<fCellVecHOST[hh]<<" "<<nCellVecHOST[hh]<<"
    "<<fCellVecHOSTDoG[hh]<<" "<<nCellVecHOSTDoG[hh]<<endl;
    }
    exit (2);
    */
    //-----------------------------------------------------
    ...
    f[ii]->ringAvgIntensity[0] =
        fCellVecHOST[1] / (float)(nCellVecHOST[1]);  // central ring
    f[ii]->ringAvgIntensityDoG[0] =
        fCellVecHOSTDoG[1] / (float)(nCellVecHOSTDoG[1]);  // central ring
    count = 0;
    int count2 = count + 2;
    for (int jj = 0; jj < numRadialCells - 1; jj++) {
      ...
      for (int kk = 0; kk < numCellsHEALPix; kk++) {
        ...
        if (nCellVecHOST[count2] == 0)  // sometimes ellipsois are so flat in Z
                                        // that some cells do not get any pixel
          f[ii]->cellAvgIntensity[count] = 0;
        else
          f[ii]->cellAvgIntensity[count] =
              fCellVecHOST[count2] / (float)(nCellVecHOST[count2]);

        NringDoG += nCellVecHOSTDoG[count2];
        WringDoG += fCellVecHOSTDoG[count2];
        if (nCellVecHOSTDoG[count2] == 0)  // sometimes ellipsois are so flat in
                                           // Z that some cells do not get any
                                           // pixel
          f[ii]->cellAvgIntensityDoG[count] = 0;
        else
          f[ii]->cellAvgIntensityDoG[count] =
              fCellVecHOSTDoG[count2] / (float)(nCellVecHOSTDoG[count2]);
...
/*
        //--------------------------------------debug:
   cuda3Dtexture------------------------
                cout<<"DEBUGGING: 3D texture access. Writing copied file
   out"<<endl;
                imageType* imAuxCUDA, *imAuxHOST;
                imAuxHOST = new imageType[imSize];
                HANDLE_ERROR( cudaMalloc(
   (void**)&(imAuxCUDA),imSize*sizeof(imageType) ) );

                int numThreadsAux=std::min(MAX_THREADS_CUDA,(int)imSize);
                int
   numBlocksAux=std::min(MAX_BLOCKS_CUDA,((int)imSize+numThreadsAux-1)/numThreadsAux);

                debuggingCopy3DTextureKernel<<<numBlocksAux,numThreadsAux>>>(imAuxCUDA,imSize,dims[0],dims[1],dims[2]);HANDLE_ERROR_KERNEL;
                HANDLE_ERROR( cudaMemcpy( imAuxHOST, imAuxCUDA,
   imSize*sizeof(imageType) , cudaMemcpyDeviceToHost ) );
...
//=======================================debug
kernel========================================================
__global__ void __launch_bounds__(MAX_THREADS_CUDA) debuggingCopy3DTextureKernel
(imageType* imAuxCUDA,long long int imSize,int dims_0,int dims_1,int dims_2)
{
        ...
        while(tid<imSize)
        {
                ...
                if(tid==0)
                {
                        printf("Value at (%f,%f,%f) from debuggin kernel is
