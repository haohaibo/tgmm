...
static unsigned int iterEMdebug =
    0;  // used to print out each of the EM iterations
...
__device__ static const double expectedLogC1 =
    dimsImage * 0.69314718055994528623;  // dimsImage*log(2.0);
__device__ static const double pow_2Pi_dimsImage2 =
    15.74960994572241901324;  //(2*pi)^(dimsImage/2.0) For Gaussian evaluation
__device__ static const double pow_2Pi_dimsImage =
    248.05021344239852965075;  //(2*pi)^(dimsImage) For Gaussian evaluation

__device__ static const double PI_ = 3.14159265358979311600;
__device__ static const double SQRT3_ = 1.73205080756887719318;

//=========================================================================================
__device__ GaussianMixtureModelCUDA &GaussianMixtureModelCUDA::operator=(
    const GaussianMixtureModelCUDA &p) {
  if (this != &p) {
    int count = 0;
    ...
    // CUDA  does not allowed to call std::vector functions inside __device__
    // function because it requires memory creation destruction. But I do not
    // need this in here
...
__device__ inline double expectedMahalanobisDistanceCUDA(
    GaussianMixtureModelCUDA *p, float *x_n) {
  double dx[dimsImage];
  ...
  return ((double)dimsImage) / p->beta_k +
         p->nu_k *
             (p->W_k[0] * dx[0] * dx[0] + p->W_k[3] * dx[1] * dx[1] +
              p->W_k[5] * dx[2] * dx[2] + 2.0 * p->W_k[1] * dx[0] * dx[1] +
              2.0 * p->W_k[2] * dx[0] * dx[2] +
              2.0 * p->W_k[4] * dx[1] * dx[2]);
  // return (dimsImage/beta_k+nu_k*(x_n-m_k).transpose()*W_k*(x_n-m_k));
}
// we assume dimsImage=3 to speed up the code by loop unrolling
__device__ inline double mahalanobisDistanceCUDA(GaussianMixtureModelCUDA *p,
                                                 float *x_n) {
  double dx[dimsImage];
  ...
  return p->nu_k *
         (p->W_k[0] * dx[0] * dx[0] + p->W_k[3] * dx[1] * dx[1] +
          p->W_k[5] * dx[2] * dx[2] + 2.0 * p->W_k[1] * dx[0] * dx[1] +
          2.0 * p->W_k[2] * dx[0] * dx[2] + 2.0 * p->W_k[4] * dx[1] * dx[2]);
  // return (dimsImage/beta_k+nu_k*(x_n-m_k).transpose()*W_k*(x_n-m_k));
}
...
__device__ inline void eig3(const double *w, double *d, double *v,
                            int vIsZero2) {
  double m, p, q;
  int vIsZero = 0;
...
//==========================================================================
/// Computes the Digamma function which is mathematically defined as the
...
/// <returns>The value of the DiGamma function at <paramref
/// name="x"/>.</returns>
...
__device__ __host__ inline double DiGamma(double x) {
  ...
  if (x < 0) {
    ...
    return DiGammaPositiveX(x);
}
//============================================================================================================================
__global__ void __launch_bounds__(MAX_THREADS)
    GMEMcopyGaussianCenter2ConstantMemoryKernel(
        GaussianMixtureModelCUDA *vecGMCUDA, float *refTempCUDA, int ref_nb) {
...
//============================================================================================================================
__global__ void __launch_bounds__(MAX_THREADS)
    memCpyDeviceToDeviceKernel(int *src, int *dest, long long int numElem) {
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
...
//=====================================================================
__global__ void __launch_bounds__(MAX_THREADS)
    GMEMexpectedLogDetCovarianceKernel(GaussianMixtureModelCUDA *vecGMCUDA,
                                       int ref_nb) {
...
__global__ void __launch_bounds__(MAX_THREADS)
    GMEMtotalAlphaKernel(GaussianMixtureModelCUDA *vecGMCUDA,
                         double *totalAlphaTempCUDA, int ref_nb) {
  ...
  if (tid < ref_nb)
    cache[cacheIndex] = vecGMCUDA[tid].alpha_k;
  else
  ...
  if (cacheIndex == 0) totalAlphaTempCUDA[blockIdx.x] = cache[0];
}

//=====================================================================
__global__ void __launch_bounds__(MAX_THREADS)
    GMEMexpectedLogResponsivityKernel(GaussianMixtureModelCUDA *vecGMCUDA,
  ...
  if (tid < ref_nb)  // we might launch more kernels than needed
  {
    ...
    if (GM->m_o[0] < -1e31)  // dead cell
    {
      ...
      GM->expectedLogResponsivityCUDA =
          DiGamma(GM->alpha_k) - totalAlphaDiGamma;
...
//=======================================================================
__global__ void __launch_bounds__(MAX_THREADS)
    GMEMcomputeXkNkKernel(int *indCUDA, float *queryCUDA, pxi *rnkCUDA,
                          float *X_k, float *N_k, long long int query_nb,
                          int ref_nb) {
  // map from threadIdx/BlockIdx to pixel position
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
  int offset = blockDim.x * gridDim.x;
  ...
  while (tid < query_nb) {
    ...
    for (int ii = 0; ii < maxGaussiansPerVoxel; ii++) {
      ...
      if (aux > 1e-3)  // to avoid unnecessary atomicAdd accesses: rnk has been
                       // normalized, so it should be high
      {
        // aux*=imgData;
...
//=======================================================================
// this kernel has to be launch with MAX_THREADS (and with as large of a number
// as possible)
__global__ void __launch_bounds__(MAX_THREADS_CUDA)
    GMEMcomputeXkNkKernelNoAtomic(int *indCUDA, float *queryCUDA, pxi *rnkCUDA,
                                  float *X_k, float *N_k,
  ...
  for (long long int ii = threadIdx.x; ii < query_nb * maxGaussiansPerVoxel;
       ii += MAX_THREADS_CUDA)  // to have colescent access indCUDA
  {
    if (indCUDA[ii] ==
        blockIdx.x)  // each block of threads processes a specific Gaussian
    {
      aux = (float)rnkCUDA[ii];
      if (aux > 1e-3)  // to avoid unnecessary memory reads: rnk has been
                       // normalized so it should be high
      {
  ...
  if (aux > 0.0f) {
    ...
    if (cacheIndex == 0)
      X_k[pos] = auxX_k0[0] / aux;  // we don't need normalization later on!!
    __syncthreads();
...
// as possible)
__global__ void __launch_bounds__(MAX_THREADS_CUDA)
    GMEMcalculateLocalKullbackDiversityKernel(
        int *indCUDA, float *queryCUDA, pxi *rnkCUDA, float *N_k,
        GaussianMixtureModelCUDA *vecGMCUDA, long long int query_nb,
        int ref_nb) {
  ...
  double logZp =
      log(sqrt(determinantSymmetricW_3D(GM->W_k) * pow(GM->nu_k, dimsImage)) /
          pow_2Pi_dimsImage2);
       ...
       ii += MAX_THREADS_CUDA)  // to have colescent access indCUDA
  {
        ...
        blockIdx.x)  // each block of threads processes a specific Gaussian
    {
                       ...
                       // normalized so it should be high
      {
        ...
        auxJ_k[threadIdx.x] +=
            aux * (log(aux) + 0.5 * mahalanobisDistanceCUDA(GM, x_n) - logZp);
      }
    }
  }
  ...
  if (cacheIndex == 0 && aux > 0.0)
    GM->splitScore = (auxJ_k[0] / aux) - log(aux);
}
//=======================================================================
// this thread needs to be launched with gridDim.x=ref_nb and MAX_THREADS_CUDA
__global__ void __launch_bounds__(MAX_THREADS_CUDA)
    GMEMcalculateLocalKullbackDiversityKernelTr(
        float *queryCUDA, float2 *rnkIndCUDA, float *N_k, int *csrPtrRowACUDA,
        GaussianMixtureModelCUDA *vecGMCUDA, long long int query_nb,
        int ref_nb) {
  __shared__ float auxJ_k[MAX_THREADS_CUDA];
  __shared__ int pIni;
  ...
  // double
  // logZp=log(sqrt(determinantSymmetricW_3D(GM->W_k)*pow(GM->nu_k,dimsImage))/pow_2Pi_dimsImage2);

  for (long long int ii = pIni + threadIdx.x; ii < pEnd;
       ii += MAX_THREADS_CUDA)  // to have colescent access indCUDA
  {
                     ...
                     // normalized so it should be high
    {
      ...
      auxJ_k[threadIdx.x] +=
          aux * (log(aux) + 0.5 * mahalanobisDistanceCUDA(GM, x_n));
    }
  }
  __syncthreads();
  ...
  aux = N_k[blockIdx.x];
  if (cacheIndex == 0 && aux > 0.0) {
    double logZp = 0.5 * log(determinantSymmetricW_3D(GM->W_k) *
                             pow(GM->nu_k, dimsImage) / pow_2Pi_dimsImage);
...
//=======================================================================
__global__ void __launch_bounds__(MAX_THREADS)
    GMEMnormalizeXkKernel(float *X_k, float *N_k, int ref_nb) {
  // map from threadIdx/BlockIdx to pixel position
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
...
//=======================================================================
__global__ void __launch_bounds__(MAX_THREADS)
    GMEMcomputeSkKernel(int *indCUDA, float *queryCUDA, pxi *rnkCUDA,
                        float *X_k, float *S_k, long long int query_nb,
                        int ref_nb) {
  // map from threadIdx/BlockIdx to pixel position
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
  int offset = blockDim.x * gridDim.x;
...
// as possible)
__global__ void __launch_bounds__(MAX_THREADS_CUDA)
    GMEMcomputeSkKernelNoAtomic(int *indCUDA, float *queryCUDA, pxi *rnkCUDA,
                                float *X_k, float *S_k, long long int query_nb,
                                int ref_nb) {
  ...
  if (cacheIndex == 0)
    S_k[pos] = auxS_k0[0];  // we don't need normalization later on!!
  __syncthreads();
  //----------------------------------
  pos += ref_nb;
  i = blockDim.x / 2;
  ...
  if (cacheIndex == 0)
    S_k[pos] = auxS_k1[0];  // we don't need normalization later on!!
  __syncthreads();
  //----------------------------------
  pos += ref_nb;
  i = blockDim.x / 2;
  ...
  if (cacheIndex == 0)
    S_k[pos] = auxS_k2[0];  // we don't need normalization later on!!
  __syncthreads();
  //----------------------------------
  pos += ref_nb;
  i = blockDim.x / 2;
  ...
  if (cacheIndex == 0)
    S_k[pos] = auxS_k3[0];  // we don't need normalization later on!!
  __syncthreads();
  //----------------------------------
  pos += ref_nb;
  i = blockDim.x / 2;
  ...
  if (cacheIndex == 0)
    S_k[pos] = auxS_k4[0];  // we don't need normalization later on!!
  __syncthreads();
  //----------------------------------
  pos += ref_nb;
  i = blockDim.x / 2;
  ...
  if (cacheIndex == 0)
    S_k[pos] = auxS_k5[0];  // we don't need normalization later on!!
  //----------------------------------
}
...
__global__ void __launch_bounds__(MAX_THREADS)
    GMEMnormalizeSkKernel(float *S_k, float *N_k, int ref_nb) {
  // map from threadIdx/BlockIdx to pixel position
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
...
__global__ void __launch_bounds__(MAX_THREADS)
    GMEMcomputeXkNkSkKernelTr(int *indCUDAtr, float *queryCUDA,
                              float2 *rnkIndCUDA, int *csrPtrRowACUDA,
                              float *X_k, float *N_k, float *S_k,
                              long long int query_nb, int ref_nb) {
  ...
  for (long long int ii = pIni + threadIdx.x; ii < pEnd;
       ii += MAX_THREADS)  // to have colescent access indCUDA
  {
    rnkAux = rnkIndCUDA[ii];
    aux = rnkAux.x;
    if (aux > 1e-3)  // to avoid unnecessary memory reads: rnk has been
                     // normalized so it should be high
    {
      auxN_k[threadIdx.x] += aux;
      pos = (long long int)(rnkAux.y);
      x_n[0] = queryCUDA[pos];  // non-coalescent access (maybe I should use
                                // constant memory)
      pos += query_nb;
      x_n[1] = queryCUDA[pos];
      pos += query_nb;
...
__global__ void __launch_bounds__(MAX_THREADS)
    GMEMcomputeXkNkSkKernelTrWithSupervoxels(
        float *imgDataCUDA, long long int *labelListPtrCUDA, int *indCUDAtr,
        float *queryCUDA, float2 *rnkIndCUDA, int *csrPtrRowACUDA, float *X_k,
        float *N_k, float *S_k, long long int query_nb, int ref_nb) {
  __shared__ float auxX_k0[MAX_THREADS];
  __shared__ float auxX_k1[MAX_THREADS];
  __shared__ float auxX_k2[MAX_THREADS];
  __shared__ float auxN_k[MAX_THREADS];
  ...
  __shared__ float2
      rnkAux;  // now rnk is common to all voxels within a supervoxels
  ...
  for (long long int ii = pIni; ii < pEnd;
       ii++)  // csrPTRRowACUDA indicates which super-voxels are associated to
              // each Gaussian. It is expected that very few supervoxels are
              // associated to each Gaussian so we prefer to parallelize the
              // inner for loop where we cheack each voxel within a super-voxel
  {
    if (threadIdx.x == 0) {
      rnkAux = rnkIndCUDA[ii];
    }
    ...
    if (auxOrig > 1e-3)  // to avoid unnecessary memory reads: rnk has been
                         // normalized so it should be high
    {
      ...
      for (long long int jj = pIniVoxels + threadIdx.x; jj < pEndVoxels;
           jj += MAX_THREADS)  // to have colescent access to imgDataCUDA
      {
        // read imgValue and multiply it by rnk since they always appear
        // together
        aux = auxOrig * imgDataCUDA[jj];  // even if rnkAux.x is the same all
                                          // the time, I can not
                                          // reuse aux because it will be
        // rnkAux.x*ImgDataCUDA[jj]*ImgDataCUDA[jj+MAX_THREADS]*....
        // The issue is opnly present for regions with
        // area>MAX_THREADS
...
//calculate mean and covariance in a single pass over teh data
__global__ void __launch_bounds__(MAX_THREADS_CUDA)
GMEMcomputeXkNkSkKernelNoAtomic(int *indCUDA,float *queryCUDA,pxi *rnkCUDA,float
*X_k,float *N_k,float *S_k,long long int query_nb,int ref_nb)
                {
        ...
        for(long long int
ii=threadIdx.x;ii<query_nb*maxGaussiansPerVoxel;ii+=MAX_THREADS_CUDA)//to have
colescent access indCUDA
        {
                if(indCUDA[ii]==blockIdx.x)//each block of threads processes a
specific Gaussian
                {
                        aux=(float)rnkCUDA[ii];
                        if(aux>1e-3) //to avoid unnecessary memory reads: rnk
has been normalized so it should be high
                        {
                                auxN_k[threadIdx.x]+=aux;
                                pos=ii%query_nb;
                                x_n[0]=queryCUDA[pos];
...
//=======================================================================
__global__ void __launch_bounds__(MAX_THREADS)
    GMEMupdateGaussianParametersKernel(GaussianMixtureModelCUDA *vecGMCUDA,
                                       float *X_k, float *S_k, float *N_k,
                                       int ref_nb) {
  // map from threadIdx/BlockIdx to pixel position
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
  ...
  if (tid < ref_nb) {
    ...
    if (GM->m_o[0] > -1e31 &&
        GM->fixed == false)  // cell is alive and it is not fixed
    {
      float auxNk = N_k[tid];
      ...
      GM->alpha_k =
          fmax(GM->alpha_o + auxNk,
               0.0);  // alpha_o can be negative (improper Dirichlet prior)
      ...
      // loop unroll for
      // W_k=(W_o.inverse()+N_k*S_k+(auxBeta_o*N_k/(auxBeta_o+N_k))*(x_k-m_o)*(x_k-m_o).transpose()).inverse();
      x_k[0] -= m_o[0];
      x_k[1] -= m_o[1];
      x_k[2] -= m_o[2];
      auxBeta_o /= (auxBeta_o + auxNk);  // precompute

      // VIP:we assume S_k has not been normalized previously so we save
      // operations and improve precision
...
//__device__ static const double
// lambdaMin=0.02;//aux=scaleSigma/(maxRadius*maxRadius) with scaleSigma=2.0 and
// maxRadius=10 (adjust with scale)
//__device__ static const double
// lambdaMax=0.2222;//aux=scaleSigma/(maxRadius*maxRadius) with scaleSigma=2.0
// and minRadius=3.0 (adjust with scale)  (when nuclei divide they can be very
// narrow)
//__device__ static const double maxExcentricity=3.0*3.0;//maximum excentricity
// allowed: sigma[i]=1/sqrt(d[i]). Therefore maxExcentricity needs to be squared
// to used in terms of radius.

__global__ void __launch_bounds__(MAX_THREADS)
    GMEMregularizeWkKernel(GaussianMixtureModelCUDA *vecGMCUDA, int ref_nb,
                           bool W4DOF, double lambdaMin, double lambdaMax,
                           double maxExcentricity) {
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
  if (tid < ref_nb) {
    GaussianMixtureModelCUDA *GM = &(vecGMCUDA[tid]);

    if (GM->m_o[0] < -1e31 || GM->fixed == true)
      return;  // dead cell or fixed cell in the mixture

    double auxNu = GM->nu_k;
    double auxMax =
        lambdaMax /
        auxNu;  // aux=scaleSigma/(minRadius*minRadius*nu_k) with scaleSigma=2.0
    double auxMin =
        lambdaMin /
        auxNu;  // aux=scaleSigma/(maxRadius*maxRadius*nu_k) with scaleSigma=2.0

    double W_k[dimsImage * (dimsImage + 1) / 2];
    // copy to local memory in order to perform operations
    // NOT ALLOWED INSIDE KERNEL
    // cudaMemcpy(W_k,&(GM->W_k[0]),sizeof(double)*dimsImage*(dimsImage+1)/2,cudaMemcpyDeviceToDevice);
    int count;
    for (int count = 0; count < dimsImage * (dimsImage + 1) / 2; count++)
      W_k[count] = GM->W_k[count];
    ...
    */
    ...
    auxNu = d[0] / d[1];
    if (auxNu > maxExcentricity)
      d[0] = maxExcentricity * d[1];
    else if (1. / auxNu > maxExcentricity)
      d[1] = maxExcentricity * d[0];

    auxNu = d[0] / d[2];
    if (auxNu > maxExcentricity)
      d[0] = maxExcentricity * d[2];
    else if (1. / auxNu > maxExcentricity)
      d[2] = maxExcentricity * d[0];

    auxNu = d[1] / d[2];
    if (auxNu > maxExcentricity)
      d[1] = maxExcentricity * d[2];
    else if (1. / auxNu > maxExcentricity)
      d[2] = maxExcentricity * d[1];
...
__global__ void __launch_bounds__(MAX_THREADS)
    GMEMcheckDeadCellsKernel(GaussianMixtureModelCUDA *vecGMCUDA, float *N_k,
                             int ref_nb, double totalAlpha) {
  // map from threadIdx/BlockIdx to pixel position
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
  ...
  if (tid < ref_nb) {
    ...
    for (int ii = 0; ii < dimsImage * (1 + dimsImage + 1) / 2; ii++)
      if (isnan(GM->W_k[ii])) isnanW = true;

    if (isnanW == true || (GM->alpha_k / totalAlpha < minPi_kForDead_CUDA &&
                           GM->m_o[0] > -1e31 && GM->fixed == false)) {
      // kill mixture
      N_k[tid] = 0.0f;
      GM->alpha_k = 0.0;
      for (int ii = 0; ii < dimsImage; ii++) {
        ...
        GM->W_k[3] = 1;
        GM->W_k[4] = 0;
        GM->W_k[5] = 1;  // to avoid NaN from regularization
      }
    }
  }
}
//=======================================================================
__global__ void __launch_bounds__(MAX_THREADS)
    GMEMcomputeLikelihoodKernel(float *likelihoodVecCUDA,
                                GaussianMixtureModelCUDA *vecGMCUDA,
                                int *indCUDA, float *queryCUDA,
                                float *imgDataCUDA, long long int query_nb,
                                int ref_nb, double totalAlpha) {
  ...
  for (int ii = 0; ii < maxGaussiansPerVoxel; ii++) {
    ...
    aux = sqrt(determinantSymmetricW_3D(GM->W_k) * pow(GM->nu_k, dimsImage)) /
          pow_2Pi_dimsImage2;
...
//=======================================================================
// since all the voxels in the same region are associated with the same
// Gaussian, we are going to save a lot of memory accesses by computing the
// likelihood of each super-voxel within each block and then generating a second
// kernel to add the likelihood from all the super-voxels
__global__ void __launch_bounds__(MAX_THREADS)
    GMEMcomputeLikelihoodKernelWithSupervoxels(
        float *likelihoodVecCUDA, GaussianMixtureModelCUDA *vecGMCUDA,
        int *indCUDA, float *queryCUDA, float *imgDataCUDA,
        long long int *labelListPtrCUDA, long long int query_nb,
        unsigned short int numLabels, double totalAlpha) {
  __shared__ float cache[MAX_THREADS];  // stores partial sums of log-likelihood
  // If all threads read from the same shared memory address then a broadcast
  // mechanism is automatically invoked and serialization is avoided. Shared
  // memory broadcasts are an excellent and high-performance way to get data to
  // many threads simultaneously.
  __shared__ GaussianMixtureModelCUDA
      vecGMlabel[maxGaussiansPerVoxel];  // stores the data for each Gaussian.
                                         // We use one block per super-voxel so
                                         // all threads in the same block access
                                         // the same Gaussians
  ...
  for (long long int tid = threadIdx.x + labelListPtrCUDA[blockIdx.x];
       tid < labelListPtrCUDA[blockIdx.x + 1]; tid += MAX_THREADS) {
    pos = tid;
    // loop unrolling for 3D
    x_n[0] = queryCUDA[pos];
    pos += query_nb;
    ...
    auxLikelihood = 0;
    for (int ii = 0; ii < maxGaussiansPerVoxel; ii++) {
      aux = sqrt(determinantSymmetricW_3D(vecGMlabel[ii].W_k) *
                 pow(vecGMlabel[ii].nu_k, dimsImage)) /
            pow_2Pi_dimsImage2;
...
// it is teh same code as GMEMcomputeLikelihoodKernelWithSupervoxels but
// substituting blockIdx.x by supervoxelIdx
__global__ void __launch_bounds__(MAX_THREADS)
    GMEMcomputeLocalLikelihoodKernelWithSupervoxels(
        float *likelihoodVecCUDA, GaussianMixtureModelCUDA *vecGMCUDA,
        int *indCUDA, float *queryCUDA, float *imgDataCUDA,
        long long int *labelListPtrCUDA, long long int query_nb, int ref_nb,
        unsigned short int numLabels, double totalAlpha,
        int *listSupervoxelsIdx, int listSupervoxelsIdxLength) {
  __shared__ float cache[MAX_THREADS];  // stores partial sums of log-likelihood
  // If all threads read from the same shared memory address then a broadcast
  // mechanism is automatically invoked and serialization is avoided. Shared
  // memory broadcasts are an excellent and high-performance way to get data to
  ...
  if (blockIdx.x >= listSupervoxelsIdxLength) return;
  ...
  if (pos < maxGaussiansPerVoxel) {
    ...
    if( supervoxelIdx == 7)
    {
            printf("Gaussian:%3d; %12f %12f %12f; %12f %12f %12f %12f %12f
    %12f;%12f\n",indCUDA[supervoxelIdx+pos*numLabels],
    vecGMlabel[pos].m_k[0],vecGMlabel[pos].m_k[1], vecGMlabel[pos].m_k[2],
    vecGMlabel[pos].W_k[0] , vecGMlabel[pos].W_k[1] , vecGMlabel[pos].W_k[2] ,
    vecGMlabel[pos].W_k[3] , vecGMlabel[pos].W_k[4] , vecGMlabel[pos].W_k[5],
    vecGMlabel[pos].nu_k );
            printf("Parameter for:%3d %1.16f %1.16f %1.16f\n",
    indCUDA[supervoxelIdx+pos*numLabels],determinantSymmetricW_3D(vecGMlabel[pos].W_k),
    vecGMlabel[pos].alpha_k/totalAlpha,
    determinantSymmetricW_3D(vecGMlabel[pos].W_k)*pow(vecGMlabel[pos].nu_k,dimsImage)
    );
    }
    */
    //------------------------------------------------
  ...
  for (long long int tid = threadIdx.x + labelListPtrCUDA[supervoxelIdx];
       tid < labelListPtrCUDA[supervoxelIdx + 1]; tid += MAX_THREADS) {
    pos = tid;
    // loop unrolling for 3D
    x_n[0] = queryCUDA[pos];
    pos += query_nb;
    ...
    /*looking at all the Gaussians instead of just the nearest neighbors (it
    should be a little bit more accurate in some cases)
    for(int ii = 0; ii < ref_nb; ii++) //hola2
    {
            aux=sqrt(
    determinantSymmetricW_3D(vecGMCUDA[ii].W_k)*pow(vecGMCUDA[ii].nu_k,dimsImage)
    )/pow_2Pi_dimsImage2;
...
// if(likelihoodPerPixel[ii]>0)
// logLikelihood+=imgData[ii]*log(likelihoodPerPixel[ii]);
__global__ void __launch_bounds__(MAX_THREADS)
    GMEMsumLikelihoodKernel(float *imgDataCUDA, float *likelihoodTempCUDA,
                            float *likelihoodVecCUDA, int query_nb) {
  __shared__ float cache[MAX_THREADS];
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
  int cacheIndex = threadIdx.x;
  float aux;
...
//=====================================================================
// we sum the elements of a vector (it follows the classical example of dot
// product on how to perform reduction operations)
// the vector must have length=2^n
// the partial sums are saved in vector[0...numBlocks], so you have to finish
// some computations on the GPU side
// VIP: the values of the vector are modified with partial sums!!!
__global__ void __launch_bounds__(MAX_THREADS)
    sumVector(float *vector, long long int length) {
  __shared__ float cache[MAX_THREADS];
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
  int cacheIndex = threadIdx.x;
  ...
  if (cacheIndex == 0) vector[blockIdx.x] = cache[0];
}

//=======================================================================
__global__ void __launch_bounds__(MAX_THREADS)
    GMEMcomputeRnkKernel(GaussianMixtureModelCUDA *vecGMCUDA, int *indCUDA,
                         float *queryCUDA, pxi *rnkCUDA, float *imgDataCUDA,
  ...
  while (tid < query_nb) {
    ...
    if (rnkNorm > 0.0f) {
      ...
      for (int ii = 0; ii < maxGaussiansPerVoxel; ii++) {
        rnkCUDA[pos] = imgData * rnkAux[ii] / rnkNorm;  // rnk is always
                                                        // multiplied by imgData
...
//============================================================================================================================
// the approach for this kernel is very different than the approach for the same
// kernel without supervoxels since we have to perform reduction techniques in
// the GPU
// actually the approach is very similar to the kernel
// GMEMcomputeXkNkKernelNoAtomic where each block deals with a single Gaussian
// mixture (in this case each blocks deals with a single region)

// VIP: we assume we points are ordered by region assignment, so each thread can
// access it in a cache friendly manner
__global__ void __launch_bounds__(MAX_THREADS_CUDA)
    GMEMcomputeRnkKernelWithSupervoxels(GaussianMixtureModelCUDA *vecGMCUDA,
                                        int *indCUDA, float *queryCUDA,
                                        pxi *rnkCUDA,
                                        long long int *labelListPtrCUDA,
                                        long long int query_nb,
                                        unsigned short int numLabels) {
  __shared__ float
      rnkVec[MAX_THREADS_CUDA * maxGaussiansPerVoxel];  // it stores partial
                                                        // accumulative values
                                                        // of rnk for a regions
  // If all threads read from the same shared memory address then a broadcast
  // mechanism is automatically invoked and serialization is avoided. Shared
  // memory broadcasts are an excellent and high-performance way to get data to
  // many threads simultaneously.
  ...
  if (threadIdx.x < maxGaussiansPerVoxel) {
    vecGMlabel[threadIdx.x] =
        vecGMCUDA[indCUDA[blockIdx.x + threadIdx.x * numLabels]];
  }
  __syncthreads();
  ...
  for (long long int tid = threadIdx.x + labelListPtrCUDA[blockIdx.x];
       tid < labelListPtrCUDA[blockIdx.x + 1]; tid += MAX_THREADS_CUDA) {
    pos = tid;
    // loop unrolling for 3D
    x_n[0] = queryCUDA[pos];
    pos += query_nb;
  ...
  for (int jj = 0; jj < maxGaussiansPerVoxel; jj++) {
    ...
    while (ii != 0) {
      if (pos < ii + offset) rnkVec[pos] += rnkVec[pos + ii];
      __syncthreads();
      ii /= 2;
    }
    offset += MAX_THREADS_CUDA;
  ...
  // check notebook July 10th 2012 + wikipedia for logarithm addition
  // http://en.wikipedia.org/wiki/List_of_logarithmic_identities without losing
  // precision
  if (threadIdx.x < maxGaussiansPerVoxel) {
    aux = 0.0;
...
//============================================================================================================================
__global__ void GMEMprintGaussianMixtureModelKernel(
    GaussianMixtureModelCUDA *vecGMCUDA, float *refTempCUDA, int idx) {
#ifdef USE_CUDA_PRINTF
  ...
  cuPrintf("m_k(constant mem buffer)=%g %g %g\n", refTempCUDA[3 * idx],
           refTempCUDA[3 * idx + 1], refTempCUDA[3 * idx + 2]);
  cuPrintf("W_k=%g %g %g %g %g %g\n", GM->W_k[0], GM->W_k[1], GM->W_k[2],
           GM->W_k[3], GM->W_k[4], GM->W_k[5]);

  cuPrintf("beta_o=%g;alpha_o=%g;nu_o=%g\n", GM->beta_o, GM->alpha_o, GM->nu_o);
  cuPrintf("m_o=%g %g %g\n", GM->m_o[0], GM->m_o[1], GM->m_o[2]);
  cuPrintf("W_o=%g %g %g %g %g %g\n", GM->W_o[0], GM->W_o[1], GM->W_o[2],
           GM->W_o[3], GM->W_o[4], GM->W_o[5]);
...
//==========================================================================================================================
// slight modification from Nvidia SDK transpose matrix since our matrices are
// very skinny
// dim3 grid(min(65536,(int)(query_nb+TILE_DIM-1)/TILE_DIM), 1),
// threads(TILE_DIM,1);
__global__ void __launch_bounds__(TILE_DIM)
    GMEMtransposeArrayInt(int *idata, int *odata, long long int query_nb) {
  __shared__ int tile[TILE_DIM][maxGaussiansPerVoxel];

  // read idata from global memory to share memory
  int row =
      blockIdx.x * blockDim.x;  // we need to execute whole blocks at a time
  int offset = blockDim.x * gridDim.x;
  ...
  while (row < query_nb)  // we need to execute whole blocks at a time
  {
    ...
    if (pos < (query_nb - TILE_DIM) * maxGaussiansPerVoxel) {
      for (int ii = 0; ii < maxGaussiansPerVoxel; ii++) {
        odata[pos] =
            tile[p][n];  // TILE_DIM is a multiple of maxGaussians per voxel
        pos += TILE_DIM;
        p += aux;
      }
...
// threads(TILE_DIM,1);
__global__ void __launch_bounds__(TILE_DIM)
    GMEMtransposeArrayFloat(float *idata, float *odata,
                            long long int query_nb) {
...
//=========================================
__global__ void GMEMsetcscColPtrA(int *cscColPtrACUDA,
...
__global__ void GMEMsetcsrColIndA(int *csrColIndACUDA, float2 *rnkIndCUDA,
                                  long long int query_nb) {
...
__global__ void GMEMsetRnktrKernel(float *rnkCUDAtr, float2 *rnkIndCUDA,
                                   long long int query_nb) {
...
__global__ void GMEMsetcsrRowPtrAKernel(int *csrRowPtrACUDA, int *indCUDAtr,
                                        long long int query_nb, int ref_nb) {
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
  int offset = blockDim.x * gridDim.x;
...
void GMEMsetcsrRowPtrA(int *csrRowPtrAHost, int *csrRowPtrACUDA, int *indCUDAtr,
                       long long int query_nb, int ref_nb) {
  long long int numThreads_query =
      std::min((long long int)MAX_THREADS, query_nb);
  long long int numGrids_query =
      std::min((long long int)MAX_BLOCKS,
               (query_nb + numThreads_query - 1) / numThreads_query);
  ...
  GMEMsetcsrRowPtrAKernel<<<numGrids_query, numThreads_query>>>(
      csrRowPtrACUDA, indCUDAtr, query_nb, ref_nb);
  HANDLE_ERROR_KERNEL;
  // finish in host to fill in the gaps
  HANDLE_ERROR(cudaMemcpy(csrRowPtrAHost, csrRowPtrACUDA,
                          sizeof(int) * (ref_nb + 1), cudaMemcpyDeviceToHost));

  int lastVal = csrRowPtrAHost[ref_nb];  // needed to handle end of the
                                         // csrRowPtr
  // printf("====DEBUGGING: at void GMEMsetcsrRowPtrA. Incorrect lastVal = %d
  // and ref_nb = %d\n",lastVal, ref_nb);
  if (lastVal >= ref_nb) {
    printf(
        "ERROR: at void GMEMsetcsrRowPtrA. Incorrect lastVal = %d and ref_nb = "
        "%d\n",
        lastVal, ref_nb);
    exit(3);
  }
  ...
  FILE
  *pFile=fopen("/Users/amatf/TrackingNuclei/tmp/GMEMtracking3D_1311858269_CPU/rowPtrBefore.txt","w");
  ...
  for (int ii = 1; ii < ref_nb; ii++) {
    ...
    if (aux > 0) {
      ...
      csrRowPtrAHost[ii] = lastElem;
  }

  // copy back to device
  HANDLE_ERROR(cudaMemcpy(csrRowPtrACUDA, csrRowPtrAHost,
                          sizeof(int) * (ref_nb + 1), cudaMemcpyHostToDevice));

  //--------------------------debug----------------------------------------------------
  /*
  printf("DEBUGGING: debugging col compress pointer");
  int *indHost=new int[maxGaussiansPerVoxel*query_nb];
  HANDLE_ERROR( cudaMemcpy( indHost, indCUDAtr, sizeof(int) * query_nb
  *maxGaussiansPerVoxel, cudaMemcpyDeviceToHost ) );
...
//======================================================================================================
__global__ void GMEMcopyRnkBeforeSortKernel(float *rnkCUDA, float2 *rnkIndCUDA,
                                            long long int query_nb) {
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
  int offset = blockDim.x * gridDim.x;
...
//=========================================
__global__ void __launch_bounds__(MAX_THREADS)
    GMEMsetMatrixBkernel(float *B, float *queryCUDA, long long int query_nb) {
  int tid = threadIdx.x + blockIdx.x * blockDim.x;
  int offset = blockDim.x * gridDim.x;
  float x_n[dimsImage];
  while (tid < query_nb) {
...
void allocateMemoryForGaussianMixtureModelCUDA(
    ...
    int numGridsCheck) {
  HANDLE_ERROR(cudaMalloc((void **)&(*vecGMCUDA),
                          ref_nb * sizeof(GaussianMixtureModelCUDA)));
  HANDLE_ERROR(cudaMalloc((void **)&(*rnkCUDA),
                          query_nb * maxGaussiansPerVoxel * sizeof(pxi)));
  HANDLE_ERROR(cudaMalloc((void **)&(*totalAlphaTempCUDA),
                          numGridsCheck * sizeof(double)));
}
...
void updateMemoryForGaussianMixtureModelCUDA(
  ...
  HANDLE_ERROR(cudaMemcpy(vecGMCUDA, vecGMCUDAtemp,
                          sizeof(GaussianMixtureModelCUDA) * ref_nb,
...
double calculateTotalAlpha(GaussianMixtureModelCUDA *vecGMCUDA,
  ...
  GMEMtotalAlphaKernel<<<grids, threads>>>(vecGMCUDA, totalAlphaTempCUDA,
                                           ref_nb);
  HANDLE_ERROR_KERNEL;

  // finish adding sum
  HANDLE_ERROR(cudaMemcpy(totalAlphaTemp, totalAlphaTempCUDA,
                          numGrids * sizeof(double),
                          cudaMemcpyDeviceToHost));  // retrieve partial sum
...
double addTotalLikelihood(float *imgDataCUDA, float *likelihoodVecCUDA,
                          ...
                          float *queryCUDA, long long int query_nb, int ref_nb,
                          double totalAlpha) {
  // calculate number of threads and blocks
  long long int numThreads = std::min((long long)MAX_THREADS, query_nb);
  long long int numGrids =
      std::min((long long)MAX_BLOCKS, (query_nb + numThreads - 1) / numThreads);
  ...
  GMEMcomputeLikelihoodKernel<<<numGrids, numThreads>>>(
      likelihoodVecCUDA, vecGMCUDA, indCUDA, queryCUDA, imgDataCUDA, query_nb,
      ref_nb, totalAlpha);
  HANDLE_ERROR_KERNEL;

  // GMEMsumLikelihoodKernel<<<numGrids,numThreads>>>(imgDataCUDA,likelihoodTempCUDA,likelihoodVecCUDA,query_nb);
  HANDLE_ERROR_KERNEL;
  HANDLE_ERROR(cudaMemcpy(finalSumVectorInHostF, likelihoodVecCUDA,
                          sizeof(float) * numGrids, cudaMemcpyDeviceToHost));
...
double addTotalLikelihoodWithSupervoxels(
  ...
  HANDLE_ERROR(cudaMemset(
      likelihoodVecCUDA, 0,
      sizeof(float) * sizeLikelihoodVec));  // some elements might not be
                                            // updated since we chose the
                                            // smallest power of 2 above
                                            // numLabels

  GMEMcomputeLikelihoodKernelWithSupervoxels<<<numGrids, numThreads>>>(
      likelihoodVecCUDA, vecGMCUDA, indCUDA, queryCUDA, imgDataCUDA,
      labelListPtrCUDA, query_nb, numLabels, totalAlpha);
  HANDLE_ERROR_KERNEL;
  ...
  sumVector<<<numGrids, numThreads>>>(likelihoodVecCUDA, sizeLikelihoodVec);
  HANDLE_ERROR_KERNEL;
...
double addLocalLikelihoodWithSupervoxels(
  ...
  if (numLocalLabels == 0)  // to protect from error launching the kernel. It is
                            // just an empty Gaussian
    return -1e32;           // very unlikely event

  int sizeLikelihoodVec =
      (int)pow(2.0f, (int)ceil(log2((float)numLocalLabels)));
  ...
  GMEMcomputeLocalLikelihoodKernelWithSupervoxels<<<numGrids, numThreads>>>(
      likelihoodVecCUDA, vecGMCUDA, indCUDA, queryCUDA, imgDataCUDA,
      labelListPtrCUDA, query_nb, ref_nb, numLabels, totalAlpha,
      listSupervoxelsIdxCUDA, listSupervoxelsIdxLength);
  ...
  //-------------------debug----------------------------
  /*
          cout<<"DEBUGGING: printing out local likelihood for each surpevoxel.
     listSupervoxelsIdxLength="<<listSupervoxelsIdxLength<<endl;
          ...
          HANDLE_ERROR( cudaMemcpy( listSupervoxelsIdx, listSupervoxelsIdxCUDA,
     sizeof(int) * numGrids, cudaMemcpyDeviceToHost ) );
          HANDLE_ERROR( cudaMemcpy( likelihoodVec, likelihoodVecCUDA,
     sizeof(float) * sizeLikelihoodVec, cudaMemcpyDeviceToHost ) );
...
//==============================================================================================================
void calculateLocalLikelihood(
    vector<double> &localLikelihood, const vector<vector<int>> &listSupervoxels,
    float *queryCUDA, float *imgDataCUDA, pxi *rnkCUDA, int *indCUDA,
    GaussianMixtureModelCUDA *vecGMtemp, long long int *labelListPtrCUDA,
    long long int query_nb, int ref_nb, int numLabels) {
  ...
  for (size_t ii = 0; ii < listSupervoxels.size(); ii++)
    numLocalLabelsMax =
        max(numLocalLabelsMax, (int)(listSupervoxels[ii].size()));

  // calculate numThreads and numGrids for different kernels
  int numThreads_ref = std::min(MAX_THREADS, ref_nb);
  int numGrids_ref =
      std::min(MAX_BLOCKS, (ref_nb + numThreads_ref - 1) / numThreads_ref);
  long long int numThreads_query =
      std::min((long long int)MAX_THREADS, query_nb);
  long long int numGrids_query =
      std::min((long long int)MAX_BLOCKS,
               (query_nb + numThreads_query - 1) / numThreads_query);
  long long int numThreads_labels =
      std::min((long long int)MAX_THREADS, (long long int)numLocalLabelsMax);
  long long int numGrids_labels =
      std::min((long long int)MAX_BLOCKS,
               (long long int)((numLocalLabelsMax + numThreads_labels - 1) /
                               numThreads_labels));

  int sizeLikelihoodVec = (int)pow(
      2.0f, (int)ceil(log2((float)numLocalLabelsMax)));  // we need it to be
                                                         // apower of 2 in order
                                                         // to perform summation
                                                         // as a kernel (dot
                                                         // product example)
  long long int numThreads_likelihood =
      std::min((long long int)MAX_THREADS, (long long int)sizeLikelihoodVec);
  long long int numGrids_likelihood =
      std::min((long long int)MAX_BLOCKS,
               (long long int)((sizeLikelihoodVec + numThreads_likelihood - 1) /
                               numThreads_likelihood));

  // allocate memory in host
  double *finalSumVectorInHostD =
      (double *)malloc(sizeof(double) * numGrids_ref);
  float *finalSumVectorInHostF =
      (float *)malloc(sizeof(float) * numGrids_likelihood);
  ...
  HANDLE_ERROR(cudaMalloc(
      (void **)&likelihoodVecCUDA,
      sizeLikelihoodVec * sizeof(float)));  // we allocate a maximum amount of
                                            // memory need (usually it is
                                            // smaller since we use a small
                                            // subset of supervoxels to
                                            // calculate likelihood
  HANDLE_ERROR(cudaMalloc((void **)&listSupervoxelsIdxCUDA,
                          numLocalLabelsMax * sizeof(int)));  // we allocate a
                                                              // maximum amount
                                                              // of memory
                                                              // needed
                                                              // (usually, it is
  ...
  // upload vecGM info
  HANDLE_ERROR(cudaMemcpy(vecGMCUDA, vecGMtemp,
                          sizeof(GaussianMixtureModelCUDA) * ref_nb,
                          cudaMemcpyHostToDevice));

  // calculate total alpha
  double totalAlpha =
      calculateTotalAlpha(vecGMCUDA, totalAlphaTempCUDA, finalSumVectorInHostD,
                          ref_nb, numGrids_ref);

  // main for loop: calculate local likelihood for each element
  for (size_t ii = 0; ii < localLikelihood.size(); ii++) {
    if (listSupervoxels[ii].empty() == true) continue;
    HANDLE_ERROR(cudaMemcpy(listSupervoxelsIdxCUDA, &(listSupervoxels[ii][0]),
                            sizeof(int) * listSupervoxels[ii].size(),
                            cudaMemcpyHostToDevice));
    localLikelihood[ii] = addLocalLikelihoodWithSupervoxels(
        imgDataCUDA, likelihoodVecCUDA, finalSumVectorInHostF, vecGMCUDA,
        indCUDA, queryCUDA, labelListPtrCUDA, query_nb, ref_nb, numLabels,
        totalAlpha, listSupervoxelsIdxCUDA, listSupervoxels[ii].size());
  }
...
void GMEMcomputeRnkCUDA(pxi *rnk, pxi *rnkCUDA, int *indCUDA, float *queryCUDA,
  ...
  HANDLE_ERROR(
      cudaMemset(rnkCUDA, 0, sizeof(float) * maxGaussiansPerVoxel *
                                 query_nb));  // some rnk might not be set
  GMEMcomputeRnkKernel<<<grids, threads>>>(vecGMCUDA, indCUDA, queryCUDA,
                                           rnkCUDA, imgDataCUDA, query_nb);
  HANDLE_ERROR_KERNEL;
  ...
  HANDLE_ERROR(cudaMemcpy(rnk, rnkCUDA,
                          query_nb * maxGaussiansPerVoxel * sizeof(pxi),
                          cudaMemcpyDeviceToHost));  // retrieve r_nk
}
//===================================================================================================
void GMEMcomputeRnkCUDAInplace(pxi *rnkCUDA, int *indCUDA, float *queryCUDA,
  ...
  HANDLE_ERROR(
      cudaMemset(rnkCUDA, 0, sizeof(pxi) * maxGaussiansPerVoxel *
                                 query_nb));  // some rnk might not be set
  GMEMcomputeRnkKernel<<<grids, threads>>>(vecGMCUDA, indCUDA, queryCUDA,
                                           rnkCUDA, imgDataCUDA, query_nb);
  HANDLE_ERROR_KERNEL;
  ...
  // HANDLE_ERROR(cudaMemcpy(rnk,rnkCUDA,query_nb*maxGaussiansPerVoxel*sizeof(pxi),cudaMemcpyDeviceToHost));//retrieve
  // r_nk
}

//===================================================================================================
void GMEMcomputeRnkCUDAInplaceWithSupervoxels(
    pxi *rnkCUDA, int *indCUDA, float *queryCUDA, long long int query_nb,
    unsigned short int numLabels, GaussianMixtureModelCUDA *vecGMCUDA,
    long long int *labelListPtrCUDA) {
  // calculate number of threads and blocks
  ...
  HANDLE_ERROR(
      cudaMemset(rnkCUDA, 0, sizeof(pxi) * maxGaussiansPerVoxel *
                                 numLabels));  // some rnk might not be set
  GMEMcomputeRnkKernelWithSupervoxels<<<grids, threads>>>(
      vecGMCUDA, indCUDA, queryCUDA, rnkCUDA, labelListPtrCUDA, query_nb,
      numLabels);
  HANDLE_ERROR_KERNEL;
...
void GMEMupdatePriorConstantsCUDA(GaussianMixtureModelCUDA *vecGMCUDA,
  ...
  HANDLE_ERROR_KERNEL;
  GMEMexpectedLogResponsivityKernel<<<grids, threads>>>(vecGMCUDA, ref_nb,
                                                        totalAlphaDiGamma);
  HANDLE_ERROR_KERNEL;
#ifdef USE_CUDA_PRINTF
  cudaPrintfDisplay(stdout, true);
...
void initializeCuSparseLinrary(cusparseHandle_t &handle,
  ...
  cusparseSetMatType(
      descra,
      CUSPARSE_MATRIX_TYPE_GENERAL);  // not symmetric or anything like this
...
void mainTestGMEMcomputeRnkCUDA(long long int query_nb) {
  ...
  vecGM = (GaussianMixtureModelCUDA *)malloc(ref_nb *
                                             sizeof(GaussianMixtureModelCUDA));
  totalAlphaTemp = (double *)malloc(numGridsCheck * sizeof(double));

  // generate random points
  // Init
  srand((unsigned int)time(NULL));
  for (int i = 0; i < query_nb * dimsImage; i++)
    query[i] = (float)rand() / (float)RAND_MAX;
  for (int i = 0; i < query_nb * maxGaussiansPerVoxel; i++)
    ind[i] = (int)floor(0.99 * ref_nb * (float)rand() / (float)RAND_MAX);
  ...
  for (int i = 0; i < ref_nb; i++) {
    ...
    for (int j = 0; j < dimsImage; j++)
      vecGM[i].m_k[j] = (float)rand() / (float)RAND_MAX;
  ...
  size_t memoryNeededInBytes = query_nb * maxGaussiansPerVoxel * sizeof(int) +
                               query_nb * dimsImage * sizeof(float) +
                               maxGaussiansPerVoxel * query_nb * sizeof(pxi) +
                               ref_nb * sizeof(GaussianMixtureModelCUDA);
  size_t memTotal, memAvailable;
  cuMemGetInfo(&memAvailable, &memTotal);
  printf(
      "Memory required: %lu;CUDA choosing device:  %d;memory "
      "available=%lu;total mem=%lu\n",
  ...
  */
  // allocate memory in device
  allocateMemoryForGaussianMixtureModelCUDA(&vecGMCUDA, &rnkCUDA,
                                            &totalAlphaTempCUDA, query_nb,
                                            ref_nb, numGridsCheck);
  ...
  HANDLE_ERROR(cudaMalloc((void **)&(rnkCUDAtr),
                          query_nb * maxGaussiansPerVoxel * sizeof(pxi)));

  HANDLE_ERROR(cudaMemcpy(indCUDA, ind,
                          sizeof(int) * query_nb * maxGaussiansPerVoxel,
                          cudaMemcpyHostToDevice));
  HANDLE_ERROR(cudaMemcpy(queryCUDA, query,
                          sizeof(float) * query_nb * dimsImage,
                          cudaMemcpyHostToDevice));
  ...
  for (int ii = 0; ii < iterations; ii++) {
    ...
    totalAlpha = calculateTotalAlpha(vecGMCUDA, totalAlphaTempCUDA,
                                     totalAlphaTemp, ref_nb, numGridsCheck);
    GMEMupdatePriorConstantsCUDA(vecGMCUDA, ref_nb, DiGamma(totalAlpha));
    GMEMcomputeRnkCUDA(rnk, rnkCUDA, indCUDA, queryCUDA, query_nb, imgDataCUDA,
                       vecGMCUDA);
  }
  ...
  deallocateMemoryForGaussianMixtureModelCUDA(&vecGMCUDA, &rnkCUDA,
                                              &totalAlphaTempCUDA);
...
void GMEMvariationalInferenceCUDA(
  ...
  // memory
  size_t memoryNeededInBytes =
      ref_nb * sizeof(GaussianMixtureModelCUDA) +
      numGrids_ref * sizeof(double) + ref_nb * dimsImage * sizeof(float) +
      ref_nb * (1 + dimsImage + dimsImage * (1 + dimsImage) / 2) *
          sizeof(float) +
      numGrids_query * sizeof(float) + sizeof(int) * (ref_nb + 1) +
      sizeof(float2) * query_nb *
          maxGaussiansPerVoxel;  // this is the main load aside from
                                 // preallocated rnkCUDA,rnkCUDAtr and
                                 // indCUDA,indCUDAtr
...
#if defined(_WIN32) || defined(_WIN64)
  printf(
      "Memory required: %lu;CUDA choosing device:  %Iu;memory "
      "available=%Iu;total mem=%Iu\n",
      memoryNeededInBytes, devCUDA, memAvailable, memTotal);
#else
  printf(
      "Memory required: %lu;CUDA choosing device:  %zu;memory "
      "available=%zu;total mem=%zu\n",
      memoryNeededInBytes, devCUDA, memAvailable, memTotal);
#endif
  cuCtxDetach(cuContext);
  ...
  */
  ...
  // query_nb*maxGaussiansPerVoxel*sizeof(int) ) );
  HANDLE_ERROR(
      cudaMalloc((void **)&refTempCUDA, ref_nb * dimsImage * sizeof(float)));
  HANDLE_ERROR(cudaMalloc((void **)&N_k, ref_nb * sizeof(float)));
  HANDLE_ERROR(cudaMalloc((void **)&X_k, ref_nb * dimsImage * sizeof(float)));
  HANDLE_ERROR(cudaMalloc(
      (void **)&S_k, ref_nb * dimsImage * (dimsImage + 1) * sizeof(float) / 2));
  // HANDLE_ERROR( cudaMalloc( (void**)&likelihoodTempCUDA,
  // query_nb*sizeof(float) ) );
  HANDLE_ERROR(
      cudaMalloc((void **)&likelihoodVecCUDA, numGrids_query * sizeof(float)));
  HANDLE_ERROR(
      cudaMalloc((void **)&csrRowPtrACUDA, sizeof(int) * (ref_nb + 1)));
  HANDLE_ERROR(cudaMalloc((void **)&rnkIndCUDA,
                          sizeof(float2) * query_nb * maxGaussiansPerVoxel));
  ...
  int numIterEM = 1;
  double totalAlpha;
  double ll = -1.0, llOld = ll * 10, llOld2 = ll * 100,
         llOld3 = ll * 1000;  // sometimes it oscillates between multiple values
  while (fabs(ll - llOld) / fabs(llOld) > tolLikelihood &&
         fabs(ll - llOld2) / fabs(llOld2) > tolLikelihood &&
         fabs(ll - llOld3) / fabs(llOld3) > tolLikelihood &&
         numIterEM <= maxIterEM) {
    //------------------------debug each iteration and save it in a file (look
    // at the function to change the destination folder)
    // HANDLE_ERROR( cudaMemcpy( vecGMtemp, vecGMCUDA,
    // sizeof(GaussianMixtureModelCUDA) * ref_nb, cudaMemcpyDeviceToHost ) );
    // writeXMLdebugCUDA(vecGMtemp,ref_nb);
    //---------------------------------------------------------------------------------------------------------

    // calculate nearest neighbors
    GMEMcopyGaussianCenter2ConstantMemoryKernel<<<numGrids_ref,
                                                  numThreads_ref>>>(
        vecGMCUDA, refTempCUDA, ref_nb);
    HANDLE_ERROR_KERNEL;
    knnCUDAinPlace(indCUDA, queryCUDA, refTempCUDA, query_nb, ref_nb);

    // calculate responsibilities (expectation step)
    totalAlpha =
        calculateTotalAlpha(vecGMCUDA, totalAlphaTempCUDA,
                            finalSumVectorInHostD, ref_nb, numGrids_ref);
    GMEMupdatePriorConstantsCUDA(vecGMCUDA, ref_nb, DiGamma(totalAlpha));
    GMEMcomputeRnkCUDAInplace(rnkCUDA, indCUDA, queryCUDA, query_nb,
                              imgDataCUDA, vecGMCUDA);
    ...
    HANDLE_ERROR( cudaMemcpy( rnkHost, rnkCUDA, sizeof(float) * query_nb
    *maxGaussiansPerVoxel, cudaMemcpyDeviceToHost ) );
    HANDLE_ERROR( cudaMemcpy( indHost, indCUDA, sizeof(int) * query_nb
    *maxGaussiansPerVoxel, cudaMemcpyDeviceToHost ) );
    HANDLE_ERROR( cudaMemcpy( imgDataHost, imgDataCUDA, sizeof(float) *
    query_nb, cudaMemcpyDeviceToHost ) );


    FILE
    *pFile=fopen("/Users/amatf/TrackingNuclei/tmp/GMEMtracking3D_1311858269_CPU/rnk.txt","w");
    for(int ii=0;ii<query_nb;ii++)
    {
            for(int jj=0;jj<maxGaussiansPerVoxel;jj++)
            {
    ...
    //-----------------------using no transpose: memory access is
    // awful--------------------------------------
    ...
    // GMEMcomputeXkNkKernelNoAtomic<<<ref_nb,MAX_THREADS_CUDA>>>(indCUDA,queryCUDA,rnkCUDA,X_k,N_k,query_nb,ref_nb);HANDLE_ERROR_KERNEL;//it
    // does not require X_k normalization
    ...
    GMEMcopyRnkBeforeSortKernel<<<numGrids_query, numThreads_query>>>(
        rnkCUDA, rnkIndCUDA, query_nb);
    HANDLE_ERROR_KERNEL;

    // amatf
    // HANDLE_ERROR(cudaMemcpy(indCUDAtr,indCUDA,sizeof(int)*maxGaussiansPerVoxel*query_nb,cudaMemcpyDeviceToDevice));//because
    // we need indCUDA later
    memCpyDeviceToDeviceKernel<<<numGrids_query, numThreads_query>>>(
        indCUDA, indCUDAtr, maxGaussiansPerVoxel * query_nb);
    HANDLE_ERROR_KERNEL;

    thrust::sort_by_key(thrust_indCUDAtr,
                        thrust_indCUDAtr + maxGaussiansPerVoxel * query_nb,
                        thrust_rnkIndCUDA);  // super fast radix sorting (166e6
                                             // elements/sec in my GPU)

    // set row compress index format
    GMEMsetcsrRowPtrA(csrRowPtrAHost, csrRowPtrACUDA, indCUDAtr, query_nb,
                      ref_nb);

    //-------------------------------------using cuSparse: it is slower than our
    // own kernel and needs more
    // memory------------------------------------------------
    /*
    ...
    GMEMsetMatrixBkernel<<<numGrids_query,numThreads_query>>>(B,queryCUDA,query_nb);HANDLE_ERROR_KERNEL;//TODO
    this only needs to be done once per EM iterations, so we could move it
    outside teh foor loop
    ...
    ref_nb, 10, query_nb, 1.0f, descrA, rnkCUDAtr,
                                    csrRowPtrACUDA, csrColIndACUDA, B, query_nb,
    0.0f, C, ref_nb );//C = alpha*op(A)*B+beta*C
    ...
    GMEMcomputeXkNkSkKernelTr<<<ref_nb, MAX_THREADS>>>(
        indCUDAtr, queryCUDA, rnkIndCUDA, csrRowPtrACUDA, X_k, N_k, S_k,
        query_nb, ref_nb);
    HANDLE_ERROR_KERNEL;

    //-------------------------------------------------------------------------------

    // too long of a kernel->it is kind of unestable and we don't seem to gain
    // much
    // GMEMcomputeXkNkSkKernelNoAtomic<<<ref_nb,MAX_THREADS_CUDA>>>(indCUDA,queryCUDA,rnkCUDA,X_k,N_k,S_k,query_nb,ref_nb);HANDLE_ERROR_KERNEL;//it
    // does not require X_k normalization
    //-------------------------debug----------------------------
    /*
    if(frame==20)
    {
    printf("========================DEBUGGING: S_k on CUDA.
    IterEM=%d\n=====================",numIterEM);
    float *N_khost=new float[ref_nb];
    float *X_khost=new float[ref_nb*3];
    float *S_khost=new float[ref_nb*6];
    HANDLE_ERROR( cudaMemcpy( X_khost, X_k, sizeof(float) * ref_nb *3,
    cudaMemcpyDeviceToHost ) );
    HANDLE_ERROR( cudaMemcpy( N_khost, N_k, sizeof(float) * ref_nb *1,
    cudaMemcpyDeviceToHost ) );
    HANDLE_ERROR( cudaMemcpy( S_khost, S_k, sizeof(float) * ref_nb *6,
    cudaMemcpyDeviceToHost ) );
    printf("N_k=[\n");
    ...
    GMEMupdateGaussianParametersKernel<<<numGrids_ref, numThreads_ref>>>(
        vecGMCUDA, X_k, S_k, N_k, ref_nb);
    HANDLE_ERROR_KERNEL;
    if (regularizePrecisionMatrixConstants::lambdaMin < 0) {
      printf(
          "ERROR: before GMEMregularizeWkKernel: "
          "regularizePrecisionMatrixConstants were not set\n");
      exit(3);
    }
    GMEMregularizeWkKernel<<<numGrids_ref, numThreads_ref>>>(
        vecGMCUDA, ref_nb, W4DOF, regularizePrecisionMatrixConstants::lambdaMin,
        regularizePrecisionMatrixConstants::lambdaMax,
        regularizePrecisionMatrixConstants::maxExcentricity);
    HANDLE_ERROR_KERNEL;
    ...
    totalAlpha =
        calculateTotalAlpha(vecGMCUDA, totalAlphaTempCUDA,
                            finalSumVectorInHostD, ref_nb, numGrids_ref);
    HANDLE_ERROR_KERNEL;
    GMEMcheckDeadCellsKernel<<<numGrids_ref, numThreads_ref>>>(
        vecGMCUDA, N_k, ref_nb, totalAlpha);
    HANDLE_ERROR_KERNEL;
    ...
    totalAlpha =
        calculateTotalAlpha(vecGMCUDA, totalAlphaTempCUDA,
                            finalSumVectorInHostD, ref_nb, numGrids_ref);
    ll = addTotalLikelihood(imgDataCUDA, likelihoodVecCUDA,
                            finalSumVectorInHostF, vecGMCUDA, indCUDA,
                            queryCUDA, query_nb, ref_nb, totalAlpha);
    ...
    sizeof(GaussianMixtureModelCUDA) * ref_nb, cudaMemcpyDeviceToHost ) );
            ...
            ofstream outXML((debugPath + itoaFrame +"/debugEMGM_iter"+ itoa
    +".xml").c_str());

            GaussianMixtureModelCUDA::writeXMLheader(outXML);
            for(unsigned int ii=0;ii<ref_nb;ii++)
    vecGMtemp[ii].writeXML(outXML,ii);
            ...
            int *indHost=new int[maxGaussiansPerVoxel*query_nb];
            float2
    *rnkHost2=(float2*)malloc(maxGaussiansPerVoxel*query_nb*sizeof(float2));
            HANDLE_ERROR( cudaMemcpy( rnkHost, rnkCUDA, sizeof(float) * query_nb
    *maxGaussiansPerVoxel, cudaMemcpyDeviceToHost ) );
            HANDLE_ERROR( cudaMemcpy( indHost, indCUDA, sizeof(int) * query_nb
    *maxGaussiansPerVoxel, cudaMemcpyDeviceToHost ) );
            HANDLE_ERROR( cudaMemcpy( rnkHost2, rnkIndCUDA, sizeof(float2) *
    query_nb *maxGaussiansPerVoxel, cudaMemcpyDeviceToHost ) );

            FILE *pFile=fopen((debugPath + itoaFrame +"/rnk_iter"+ itoa
    +".txt").c_str(),"w");
            for(int ii=0;ii<query_nb;ii++)
            {
                    for(int jj=0;jj<maxGaussiansPerVoxel;jj++)
            ...
            fclose(pFile);
            pFile=fopen((debugPath + itoaFrame +"/ind_iter"+ itoa
    +".txt").c_str(),"w");
            for(int ii=0;ii<query_nb;ii++)
            {
                    for(int jj=0;jj<maxGaussiansPerVoxel;jj++)
            ...
            HANDLE_ERROR( cudaMemcpy( indHost, indCUDAtr, sizeof(int) * query_nb
    *maxGaussiansPerVoxel, cudaMemcpyDeviceToHost ) );
            pFile=fopen((debugPath + itoaFrame +"/indTr_iter"+ itoa
    +".txt").c_str(),"w");
            for(int ii=0;ii<query_nb*maxGaussiansPerVoxel;ii++)
            {
                            fprintf(pFile," %d",indHost[ii]);
            ...
            fprintf(pFile,";\n");
            fclose(pFile);
            pFile=fopen((debugPath + itoaFrame +"/rnkTr_iter"+ itoa
    +".txt").c_str(),"w");
            for(int ii=0;ii<query_nb*maxGaussiansPerVoxel;ii++)
            {
            ...
            pFile=fopen((debugPath + itoaFrame +"/csrRowPtr_iter"+ itoa
    +".txt").c_str(),"w");
            for(int ii=0;ii<ref_nb+1;ii++)
            {
                    fprintf(pFile," %d",csrRowPtrAHost[ii]);
  ...
  // NOT needed anymore since we use classifier for split score. TODO: implement
  // feature calculation in GPU
  ...
  printf(" done in %f secs for %d EM iterations (%f secs per iteration)\n",
         elapsedTime / 1000, numIterEM - 1,
         elapsedTime / ((numIterEM - 1) * 1000));
...
//=================================================================================
// main interface to run variational inference in GPU with supervoxels, so the
// same assignment is guaranteed for voxels withihn the same supervoxel
void GMEMvariationalInferenceCUDAWithSupervoxels(
    ...
    long long int query_nb, int ref_nb, unsigned short int numLabels,
  ...
  long long int numGrids_query =
      std::min((long long int)MAX_BLOCKS,
               (query_nb + numThreads_query - 1) / numThreads_query);
  long long int numThreads_labels =
      std::min((long long int)MAX_THREADS, (long long int)numLabels);
  long long int numGrids_labels = std::min(
      (long long int)MAX_BLOCKS,
      (long long int)((numLabels + numThreads_labels - 1) / numThreads_labels));

  int sizeLikelihoodVec = (int)pow(
      2.0f, (int)ceil(log2((float)numLabels)));  // we need it to be apower of 2
                                                 // in order to perform
                                                 // summation as a kernel (dot
                                                 // product example)
  long long int numThreads_likelihood =
      std::min((long long int)MAX_THREADS, (long long int)sizeLikelihoodVec);
  long long int numGrids_likelihood =
  ...
  */
  ...
  // HANDLE_ERROR( cudaMalloc( (void**)&likelihoodTempCUDA,
  // query_nb*sizeof(float) ) );
  HANDLE_ERROR(cudaMalloc((void **)&likelihoodVecCUDA,
                          sizeLikelihoodVec * sizeof(float)));
  HANDLE_ERROR(
      cudaMalloc((void **)&csrRowPtrACUDA, sizeof(int) * (ref_nb + 1)));
  HANDLE_ERROR(cudaMalloc((void **)&rnkIndCUDA,
                          sizeof(float2) * numLabels * maxGaussiansPerVoxel));
  ...
  while (fabs(ll - llOld) / fabs(llOld) > tolLikelihood &&
    ...
    knnCUDAinPlace(indCUDA, centroidLabelPositionCUDA, refTempCUDA, numLabels,
                   ref_nb);  // calculate the possible assignment using the
                             // centroids of each Gaussian
    ...
    GMEMcomputeRnkCUDAInplaceWithSupervoxels(rnkCUDA, indCUDA, queryCUDA,
                                             query_nb, numLabels, vecGMCUDA,
                                             labelListPtrCUDA);
    ...
    // use thrust library to transpose Rnk matrix
    // wrap raw pointer with a device_ptr
    GMEMcopyRnkBeforeSortKernel<<<numGrids_labels, numThreads_labels>>>(
        rnkCUDA, rnkIndCUDA, numLabels);
    HANDLE_ERROR_KERNEL;

    // amatf
    // HANDLE_ERROR(cudaMemcpy(indCUDAtr,indCUDA,sizeof(int)*maxGaussiansPerVoxel*query_nb,cudaMemcpyDeviceToDevice));//because
    // we need indCUDA later
    memCpyDeviceToDeviceKernel<<<numGrids_labels, numThreads_labels>>>(
        indCUDA, indCUDAtr, maxGaussiansPerVoxel * numLabels);
    HANDLE_ERROR_KERNEL;

    thrust::sort_by_key(thrust_indCUDAtr,
                        thrust_indCUDAtr + maxGaussiansPerVoxel * numLabels,
                        thrust_rnkIndCUDA);  // super fast radix sorting (166e6
                                             // elements/sec in my GPU)

    // set row compress index format
    GMEMsetcsrRowPtrA(csrRowPtrAHost, csrRowPtrACUDA, indCUDAtr, numLabels,
                      ref_nb);
    ...
    HANDLE_ERROR(cudaMemset(S_k, 0, sizeof(float) * ref_nb * 6));
    HANDLE_ERROR(cudaMemset(N_k, 0, sizeof(float) * ref_nb));
    GMEMcomputeXkNkSkKernelTrWithSupervoxels<<<ref_nb, MAX_THREADS>>>(
        imgDataCUDA, labelListPtrCUDA, indCUDAtr, queryCUDA, rnkIndCUDA,
        csrRowPtrACUDA, X_k, N_k, S_k, query_nb, ref_nb);
    HANDLE_ERROR_KERNEL;
    ...
    totalAlpha =
        calculateTotalAlpha(vecGMCUDA, totalAlphaTempCUDA,
                            finalSumVectorInHostD, ref_nb, numGrids_ref);
    ll = addTotalLikelihoodWithSupervoxels(
        imgDataCUDA, likelihoodVecCUDA, finalSumVectorInHostF, vecGMCUDA,
        indCUDA, queryCUDA, labelListPtrCUDA, query_nb, numLabels, totalAlpha);
  ...
  HANDLE_ERROR( cudaMemcpy( rnkHostAux, rnkCUDA, sizeof(pxi) *
  numLabels*maxGaussiansPerVoxel, cudaMemcpyDeviceToHost ) );
  HANDLE_ERROR( cudaMemcpy( indHostAux, indCUDA, sizeof(int) *
  numLabels*maxGaussiansPerVoxel, cudaMemcpyDeviceToHost ) );
  ...
  */
  ...
  pxi *rnkHostAux = new pxi[numLabels * maxGaussiansPerVoxel];
  int *indHostAux = new int[numLabels * maxGaussiansPerVoxel];
  HANDLE_ERROR(cudaMemcpy(rnkHostAux, rnkCUDA,
                          sizeof(pxi) * numLabels * maxGaussiansPerVoxel,
                          cudaMemcpyDeviceToHost));
  HANDLE_ERROR(cudaMemcpy(indHostAux, indCUDA,
                          sizeof(int) * numLabels * maxGaussiansPerVoxel,
                          cudaMemcpyDeviceToHost));
    ...
    for (unsigned int jj = 0; jj < numLabels; jj++) {
      if (rnkHostAux[pos] > 0.5)  // clear assignment
      {
        ...
        if (auxN < MAX_SUPERVOXELS_PER_GAUSSIAN) {
          vecGMtemp[indHostAux[pos]]
              .supervoxelIdx[vecGMtemp[indHostAux[pos]].supervoxelNum++] = jj;
        } else {
          // cout<<"WARNING: at GMEMvariationalInferenceCUDAWithSupervoxels:
          // number of supervoxels per Gaussian exceeds limit. Most likely it is
          // a Gaussian colecting background. You can increase
          // MAX_SUPERVOXELS_PER_GAUSSIAN (currently it is
          // "<<MAX_SUPERVOXELS_PER_GAUSSIAN<<") and recompile code. jj =
          // "<<indHostAux[pos]<<";number of supervoxels="<<vecGMtemp[
          // indHostAux[pos] ].supervoxelNum<<endl;
        }
      }
      pos++;
    }
  ...
  for(unsigned int jj = 0;jj<ref_nb;jj++)
          ...
          {
                  cout<<"ERROR: at GMEMvariationalInferenceCUDAWithSupervoxels:
  number of supervoxels per Gaussian exceeds limit. Increase
  MAX_SUPERVOXELS_PER_GAUSSIAN and recompile code. jj = "<<jj<<";number of
  supervoxels="<<vecGMtemp[jj].supervoxelNum<<endl;
                  exit(3);
          }
  */
  ...
  if (strcmp(debugPath.c_str(), "") != 0) {
    ...
    if (fid == NULL) {
      printf(
          "ERROR: opening file %s to save rnk values from variational "
          "inference\n",
          debugPath.c_str());
      exit(2);
    }
    ...
    fwrite(&maxGaussiansPerVoxel, sizeof(int), 1,
           fid);  // write maxGaussians per voxel
    fwrite(rnkHostAux, sizeof(pxi), numLabels * maxGaussiansPerVoxel,
           fid);  // writing rnk
    fwrite(indHostAux, sizeof(int), numLabels * maxGaussiansPerVoxel,
           fid);  // writing ind
    fclose(fid);
  }
...
void mainTestGMEMcomputeVariationalInference(long long int query_nb) {
  ...
  srand((unsigned int)time(NULL));
  for (int i = 0; i < query_nb * dimsImage; i++)
    query[i] = 10 * (float)rand() / (float)RAND_MAX;
  for (int i = 0; i < query_nb; i++)
    imgData[i] = (float)rand() / (float)RAND_MAX;
  ...
  for (int i = 0; i < ref_nb; i++) {
    ...
    for (int j = 0; j < dimsImage; j++)
      vecGM[i].m_o[j] = (float)rand() / (float)RAND_MAX;
  ...
  HANDLE_ERROR(cudaMalloc((void **)&rnkCUDA,
                          query_nb * maxGaussiansPerVoxel * sizeof(float)));
  HANDLE_ERROR(cudaMalloc((void **)&rnkCUDAtr,
                          query_nb * maxGaussiansPerVoxel * sizeof(float)));
  HANDLE_ERROR(cudaMalloc((void **)&indCUDA,
                          query_nb * maxGaussiansPerVoxel * sizeof(int)));
  HANDLE_ERROR(cudaMalloc((void **)&indCUDAtr,
                          query_nb * maxGaussiansPerVoxel * sizeof(int)));

  // copy initial elements
  HANDLE_ERROR(cudaMemcpy(imgDataCUDA, imgData, sizeof(float) * query_nb,
                          cudaMemcpyHostToDevice));
  HANDLE_ERROR(cudaMemcpy(queryCUDA, query,
                          sizeof(float) * query_nb * dimsImage,
                          cudaMemcpyHostToDevice));
  ...
  // main calculation
  GMEMvariationalInferenceCUDA(queryCUDA, imgDataCUDA, rnkCUDA, rnkCUDAtr,
                               indCUDA, indCUDAtr, vecGM, query_nb, ref_nb,
                               maxIterEM, tolLikelihoodEM, dev, 0, true);
...
void GMEMinitializeMemory(float **queryCUDA, float *query, float **imgDataCUDA,
  ...
  /*
  size_t
  memoryNeededInBytes=query_nb*(dimsImage+1+maxGaussiansPerVoxel*3)*sizeof(float);
  ...
  size_t memTotal,memAvailable;
  cuMemGetInfo(&memAvailable,&memTotal);
  printf( "Memory required: %lu;CUDA choosing device:  %d;memory
  available=%lu;total mem=%lu\n",
  memoryNeededInBytes,devCUDA,memAvailable,memTotal);
  cuCtxDetach (cuContext);
   */
  ...
  HANDLE_ERROR(cudaMalloc((void **)&(*rnkCUDA),
                          query_nb * maxGaussiansPerVoxel * sizeof(float)));
  // HANDLE_ERROR( cudaMalloc( (void**)&(*rnkCUDAtr),
  // query_nb*maxGaussiansPerVoxel*sizeof(float) ) ); Only needed if we use
  // Csparse
  HANDLE_ERROR(cudaMalloc((void **)&(*indCUDA),
                          query_nb * maxGaussiansPerVoxel * sizeof(int)));
  HANDLE_ERROR(cudaMalloc((void **)&(*indCUDAtr),
...
void GMEMinitializeMemoryWithSupervoxels(
    ...
    float **centroidLabelPositionCUDA, float *centroidLabelPosition,
  ...
  HANDLE_ERROR(cudaMalloc(
      (void **)&(*rnkCUDA),
      numLabels * maxGaussiansPerVoxel *
          sizeof(float)));  // numLabels<<query_nb -> we save a lot of memory
  // HANDLE_ERROR( cudaMalloc( (void**)&(*rnkCUDAtr),
  // query_nb*maxGaussiansPerVoxel*sizeof(float) ) ); Only needed if we use
  // Csparse
  HANDLE_ERROR(cudaMalloc((void **)&(*indCUDA),
                          numLabels * maxGaussiansPerVoxel * sizeof(int)));
  HANDLE_ERROR(cudaMalloc((void **)&(*indCUDAtr),
                          numLabels * maxGaussiansPerVoxel * sizeof(int)));
  HANDLE_ERROR(cudaMalloc((void **)&(*centroidLabelPositionCUDA),
                          numLabels * dimsImage * sizeof(float)));
  HANDLE_ERROR(cudaMalloc((void **)&(*labelListPtrCUDA),
                          (numLabels + 1) * sizeof(long long int)));
  ...
  HANDLE_ERROR(cudaMemcpy(*queryCUDA, query,
                          sizeof(float) * query_nb * dimsImage,
                          cudaMemcpyHostToDevice));
  HANDLE_ERROR(cudaMemcpy(*centroidLabelPositionCUDA, centroidLabelPosition,
                          sizeof(float) * numLabels * dimsImage,
                          cudaMemcpyHostToDevice));
  HANDLE_ERROR(cudaMemcpy(*labelListPtrCUDA, labelListPtrHOST,
                          sizeof(long long int) * (numLabels + 1),
                          cudaMemcpyHostToDevice));
  uploadScaleCUDA(scale);
}
...
ostream &GaussianMixtureModelCUDA::writeXML(ostream &os, int id) {
  ...
  ;
  os << "id=\"" << id << "\" lineage=\"" << 0 << "\" parent=\"" << 0
     << "\" dims=\"" << dimsImage << "\" splitScore=\"" << splitScore << "\"";
  ...
  // for dimsImage==3
  os << "\" W=\"" << W_k[0] << " " << W_k[1] << " " << W_k[2] << " " << W_k[1]
     << " " << W_k[3] << " " << W_k[4] << " " << W_k[2] << " " << W_k[4] << " "
     << W_k[5];

  os << "\"" << endl;

  // write priors values
  os << "nuPrior=\"" << nu_o << "\" betaPrior=\"" << beta_o
     << "\" alphaPrior=\"" << alpha_o << "\" distMRFPrior=\"" << 0.0 << "\"";
  os << " mPrior=\"";
  for (int ii = 0; ii < dimsImage; ii++) os << m_o[ii] << " ";
  os << "\" WPrior=\"" << W_o[0] << " " << W_o[1] << " " << W_o[2] << " "
     << W_o[1] << " " << W_o[3] << " " << W_o[4] << " " << W_o[2] << " "
     << W_o[4] << " " << W_o[5];
...
void findVoxelAssignmentBasedOnRnk(int *kAssignment, long long int query_nb,
  ...
  float *rnkHost = new float[maxGaussiansPerVoxel * query_nb];
  int *indHost = new int[maxGaussiansPerVoxel * query_nb];
  HANDLE_ERROR(cudaMemcpy(rnkHost, rnkCUDA,
                          sizeof(float) * query_nb * maxGaussiansPerVoxel,
                          cudaMemcpyDeviceToHost));
  ...
  for (long long int ii = 0; ii < query_nb; ii++) {
    ...
    if (auxNorm > 0.0) {
      if (maxRnk / auxNorm > 0.3)  // clear assignment
      {
        ...
        kAssignment[ii] = -1;  // no clear assignment
    } else
      kAssignment[ii] = -1;  // no assignment
  }
...
void copyScaleToDvice(float *scale) {
  HANDLE_ERROR(cudaMemcpyToSymbol(
      scaleGMEMCUDA, scale, dimsImage * sizeof(float)));  // constant memory
}

//======================================================
// copied from ostream& GaussianMixtureModel::writeXML(ostream& os) in order to
// be able to debug EM iterations withing the GPU
void writeXMLdebugCUDA(GaussianMixtureModelCUDA *vecGMtemp,
                       unsigned int numElem) {
  char logBuffer[32];
  sprintf(logBuffer, "%.4d",
          iterEMdebug++);  // increment iter in order to save next iteration
  string itoaLog = string(logBuffer);
  string filename(
      "/Users/amatf/TrackingNuclei/tmp/GMEMtracking3D_1320774859/XML_EMiter/"
      "XML_EMiter_frame127_TM" +
      itoaLog + ".xml");  // change this to save debug file
  ...
  for (unsigned int rr = 0; rr < numElem; rr++) {
    ...
    os << "<GaussianMixtureModel ";
    ;
    os << "id=\"" << rr << "\" lineage=\"" << 0 << "\" parent=\"" << 0
       << "\" dims=\"" << dimsImage << "\" splitScore=\"" << GM->splitScore
       << "\"";
    ...
    // write variables values
    os << "nu=\"" << GM->nu_k << "\" beta=\"" << GM->beta_k << "\" alpha=\""
       << GM->alpha_k << "\"";
    os << " m=\"";
    ...
    // write priors values
    os << "nuPrior=\"" << GM->nu_o << "\" betaPrior=\"" << GM->beta_o
       << "\" alphaPrior=\"" << GM->alpha_o << "\" distMRFPrior=\"" << 0
       << "\"";
    os << " mPrior=\"";
