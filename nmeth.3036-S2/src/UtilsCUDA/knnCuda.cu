...
__global__ void __launch_bounds__(MAX_THREADS) knnKernelNoConstantMemory(int *indCUDA,float *queryCUDA,float* anchorCUDA,int ref_nb,long long int query_nb)
{
	...
	if(tid==-1)
	{
		cuPrintf("tid=%d;queryAux=%f %f %f\n",tid,queryAux[0],queryAux[1],queryAux[2]);
	}
#endif
	int refIdx=-3;
	for(int jj=0;jj<maxGaussiansPerVoxel;jj++) minDist[jj]=1e32;//equivalent to infinity
	...
	for(int ii=0;ii<ref_nb;ii++)
	{
		...
		if(tid==-1)
		{
			cuPrintf("tid=%d;refIdx-3=%d;anchorCUDA=%f %f %f\n",tid,refIdx-3,anchorCUDA[refIdx-3],anchorCUDA[refIdx-2],anchorCUDA[refIdx-1]);
		}
#endif
		//insert element" minimize memory exchanges
		minDist[minPos]=dist;
...
__global__ void __launch_bounds__(MAX_THREADS) knnKernelSortedArray(int *indCUDA,float *queryCUDA,int ref_nb,long long int query_nb)
{
	...
	for(int ii=0;ii<ref_nb;ii++)
	{
		...
		if(tid==1)
		{
			cuPrintf("tid=%d;refIdx-3=%d;refCuda=%f %f %f\n",tid,refIdx-3,refCUDA[refIdx-3],refCUDA[refIdx-2],refCUDA[refIdx-1]);
		}
#endif
...
int mainTestKnnCuda(void)
{
	...
	/*
	memoryNeededInBytes=ref_nb   * dimsImage * sizeof(float);//make sure we can allocate texture memory
	if(((double)memoryNeededInBytes)/1048576.0 > MAX_TEXTURE1D_MEM)
	{
		printf("ERROR: number of query points exceeds maximum texture memory. Code is not ready for this\n");
		exit(2);
	}
	*/
#ifdef USE_CUDA_PRINTF
	...
	cudaChannelFormatDesc description = cudaCreateChannelDesc<float>();
	  ...
	  HANDLE_ERROR( cudaBindTexture(NULL,queryTexture, queryCUDA,dimsImage*query_nb*sizeof(float)) );
	  */
	...
	int numGrids=min(MAX_BLOCKS,(query_nb+numThreads-1)/numThreads);//TODO: play with these numbers to optimize
	...
	for(int ii=0;ii<iterations;ii++)
	{
		...
		HANDLE_ERROR(cudaMemcpy(indHost,indCUDA, query_nb*maxGaussiansPerVoxel*sizeof(int), cudaMemcpyDeviceToHost));
		printf("==================================\n");
...
long long 	int numGrids=std::min((long long int)MAX_BLOCKS,(query_nb+numThreads-1)/numThreads);
	...
	HANDLE_ERROR(cudaMemcpyToSymbol(refCUDA,refTemp,ref_nb* dimsImage * sizeof(float)));//constant memory	
...
//===================================================================================================
void knnCUDAinPlace(int *indCUDA,float *queryCUDA,float *refTemp,long long int query_nb,int ref_nb)
{
...
long long 	int numGrids=std::min((long long int)MAX_BLOCKS,(query_nb+numThreads-1)/numThreads);
	...
	if(MAX_REF_POINTS<ref_nb)//use global memory for anchorPoints (slower)
	{
		...
		HANDLE_ERROR(cudaMemcpyToSymbol(refCUDA,refTemp,ref_nb* dimsImage * sizeof(float),0,cudaMemcpyDeviceToDevice));//constant memory	
