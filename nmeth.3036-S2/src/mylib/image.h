...
*              the introduction of a Tiff data type, and the addition of
Advance_Tiff     *
*              and Tiff_EOF to the abstraction (cleaner than before). *
*           Dec 2009: Introduced arrays to generalize images.  Almost a complete
rewrite. *
*              Also introduced layers to capture an unlimited number of
channels.         *
* *
...
typedef struct {
  ...
  Array **layers;  //  layers[i] is the image of the i'th layer for i in
                   //  [0,num_layers-1]
} Layer_Bundle;
...
boolean Write_Images(string file_name, Layer_Bundle *images,
...
typedef struct {
  ...
  int num_width;   //  when reading.  suffix points into the malloc block
                   //  assigned
...
boolean Write_Series(Series_Bundle *files, Array *image,
                     Image_Compress compress);

Layer_Bundle *Read_All_Series(Series_Bundle *M(files),
                              Layer_Bundle *R(M(layers)));
...
Tiff *G(Open_Tiff)(string file_name, string mode);
