...
void For_Contour(Contour *tour, void *arg,
                 boolean handler(Indx_Type p, Indx_Type q, void *arg));

int Segment_Contour(Contour *tour, void *arg,
                    boolean handler(Indx_Type p, Indx_Type q, void *arg));
void For_Contour_Segment(Contour *tour, int seg, void *arg,
                         boolean handler(Indx_Type p, Indx_Type q, void *arg));
void Rev_Contour_Segment(Contour *tour, int seg, void *arg,
                         boolean handler(Indx_Type p, Indx_Type q, void *arg));

typedef struct {
  Size_Type rastlen;  // Length of the raster pair vector (always even)
  Indx_Type *raster;  // Pixels are [raster[i],raster[i+1]] for i in [0,rastlen)
                      // and i even
} Region;
...
Region *G(Record_Region)(APart *source, int share, boolean iscon2n,
                         Indx_Type leftmost, boolean with_holes, void *arg,
                         boolean (*test)(Indx_Type p, void *arg));

Region *G(Record_Basic)(APart *source, int share, boolean iscon2n,
                        Indx_Type leftmost, boolean with_holes,
                        Comparator cmprsn, Value level);
...
void For_Region(Region *reg, void *arg,
                void (*handler)(Indx_Type p, void *arg));
void For_Region_Outline(Region *reg, void *arg,
                        void (*handler)(Indx_Type p, void *arg));
void For_Region_Holes(Region *reg, void *arg,
                      void (*handler)(Indx_Type p, void *arg));
void For_Region_Exterior(Region *reg, void *arg,
                         void (*handler)(Indx_Type p, void *arg));

Region *G(Record_Level_Set)(Level_Tree *t, Level_Set *r, boolean with_holes,
                            int share);
