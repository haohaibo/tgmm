...
struct Multinormaldev {
  LLT<Matrix<double, dimsImage, dimsImage> >
      L_k;  // cholesky decomposition of covariance matrix
  ...
  Matrix<double, dimsImage, 1>
      xAux;  // in case we need to evaluate using double[dimsImage]
  ...
  Multinormaldev(Matrix<double, dimsImage, 1> &amu,
                 Matrix<double, dimsImage, dimsImage> &aS_k,
                 mylib::uint32 seed) {
    mu = amu;
    L_k = aS_k.llt();
    ...
    ZpLog = log(Zp);
    lambda_k = aS_k.inverse();  // we assume dimsImage is low, so analytical
                                // formulas apply
  };
  // initialization with precision matrix
  Multinormaldev(
      Matrix<double, dimsImage, 1> &amu,
      Matrix<double, dimsImage, dimsImage> &aS_kInverse, mylib::uint32 seed,
      bool dummy)  // bool just to differentitate for above constructor
  {
    mu = amu;
    L_k =
        aS_kInverse.inverse()
            .llt();  // we assume dimsImage is low, so analytical formulas apply
    normal = mylib::Normal_CDF(0.0, 1.0);
    mylib::Seed_CDF(normal, seed);
    Zp = pow(Pi2, dimsImage / 2.0) * L_k.matrixL().determinant();
    ZpLog = log(Zp);
  ...
  // destructor
  ~Multinormaldev() { mylib::Free_CDF(normal); };
  void resetParameters(Matrix<double, dimsImage, 1> &amu,
                       Matrix<double, dimsImage, dimsImage> &aS_k) {
    mu = amu;
    L_k = aS_k.llt();
    Zp = pow(Pi2, dimsImage / 2.0) * L_k.matrixL().determinant();
  ...
  double eval(const Matrix<double, dimsImage, 1>
                  &s)  // returns the probability of a given realization
  {
  ...
  double evalLog(const Matrix<double, dimsImage, 1>
                     &s)  // returns the log probability of a given realization
  {
    return -0.5 * (((s - mu).transpose() * lambda_k * (s - mu))(0)) - ZpLog;
  }
  double evalMahalanobisDistance(
      const Matrix<double, dimsImage, 1>
          &s)  // returns the log probability of a given realization
  {
