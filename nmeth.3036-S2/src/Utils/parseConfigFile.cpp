...
const int MAX_TOKENS_PER_LINE =
    2;  // right now we only expect configVariable:value
const char* DELIMITER = "=";

int configOptionsTrackingGaussianMixture::
    parseConfigFileTrackingGaussianMixture(const string& filename) {
  ifstream configFile(filename.c_str());
  if (!configFile.good()) {
    cout << "ERROR at parseConfigFileTrackingGaussianMixture: config filename "
  ...
  while (!configFile.eof()) {
    ...
    else if(strcmp("thrSignal",token[0])==0)
    {
      ...
      conn3D = atoi(token[1]);
    } else if (strcmp("regularizePrecisionMatrixConstants_lambdaMin",
                      token[0]) == 0) {
      lambdaMin = atof(token[1]);
    } else if (strcmp("regularizePrecisionMatrixConstants_lambdaMax",
                      token[0]) == 0) {
      lambdaMax = atof(token[1]);
    } else if (strcmp("regularizePrecisionMatrixConstants_maxExcentricity",
                      token[0]) == 0) {
      ...
      cout << "WARNING at parseConfigFileTrackingGaussianMixture: does not "
              "recognize config option "
           << token[0] << endl;
    }
  }
  ...
  if (strcmp(imgFilePattern.c_str(), "empty") == 0) {
    cout << "ERROR at parseConfigFileTrackingGaussianMixture: mandatory "
            "variable imgFilePattern was not set at config filename "
         << filename << " could not be opened" << endl;
    return 1;
  }
  ...
  if (strcmp(debugPathPrefix.c_str(), "empty") == 0) {
    cout << "ERROR at parseConfigFileTrackingGaussianMixture: mandatory "
            "variable debugPathPrefix was not set at config filename "
         << filename << " could not be opened" << endl;
    return 1;
  }
  ...
  if (backgroundThreshold < -1e31) {
    cout << "ERROR at parseConfigFileTrackingGaussianMixture: mandatory "
            "variable backgroundThreshold was not set at config filename "
         << filename << " could not be opened" << endl;
    return 1;
  }
...
void configOptionsTrackingGaussianMixture::
    printConfigFileTrackingGaussianMixture(ostream& outLog) {
  ...
  outLog << "thrCellDivisionPlaneDistance=" << thrCellDivisionPlaneDistance
         << endl;
  outLog << "thrPrcentile=" << thrPercentile << endl;
  // outLog<<"thrSignal="<<thrSignal<<endl;
  // outLog<<"Use Ilastik background/foreground
  // detection="<<useIlastikBackgroundDetector<<endl;
  ...
  outLog << "temporalWindowForLogicalRules (radius)="
         << temporalWindowRadiusForLogicalRules << endl;
  ...
  outLog << "regularizePrecisionMatrixConstants_lambdaMin = " << lambdaMin
         << endl;
  outLog << "regularizePrecisionMatrixConstants_lambdaMax = " << lambdaMax
         << endl;
  outLog << "regularizePrecisionMatrixConstants_maxExcentricity = "
         << maxExcentricity << endl;

  outLog << "Trim supervoxel minNucleiSize = " << minNucleiSize << endl;
  outLog << "Trim supervoxel maxNucleiSize =" << maxNucleiSize << endl;
  outLog << "Trim supervoxel maxPercentileTrimSV = " << maxPercentileTrimSV
         << endl;
