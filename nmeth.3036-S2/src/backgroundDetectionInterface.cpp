/*
 *
 * \brief Interface to backgroundDetection lib to find incoherent tracks that
...
int setProbBackgroundTracksAtTM(lineageHyperTree& lht, int TM,
  ...
  // features: have to match the values used during training in
  // mainBackgroundDetection.cpp::mainExtractFeatures()
  // nearest neighbors features ("social" features)
  unsigned int KmaxNumNNsupervoxel = 10;
  float KmaxDistKNNsupervoxel = 1e32;

  // set parameters
  backgroundDetectionFeatures::setTemporalWindowSize(
  ...
  unsigned long long int numFeatures =
      backgroundDetectionFeatures::getNumFeatures();
  unsigned long long int numSamples = 1;
  unsigned int debugC = 0;
  for (list<nucleus>::iterator iterN = lht.nucleiList[TM].begin();
       iterN != lht.nucleiList[TM].end(); ++iterN, debugC++) {
    if (f.calculateFeatures(iterN->treeNodePtr, devCUDA) ==
        0)  // we have good features
    {
      boostingTreeClassifier(&(f.f[0]), &(iterN->probBackground), classifier,
                             numSamples, numFeatures);
    } else {  // undetermined: we always initialize it to zero, so we do not
              // need to anything
      // iterN->probBackground = 0.0;
      // forward propagation
      if (iterN->treeNodePtr->parent != NULL) {
        iterN->probBackground =
            iterN->treeNodePtr->parent->data->probBackground;
      } else {  // at the beginning of the lineage
        iterN->probBackground =
            -1.0f;  // to indicate we did not calculate it here
      }
    }
  }
...
int applyProbBackgroundMinMaxRulePerBranch(const string& basenameTGMMresult,
  ...
  if (GetFileAttributes(outputFolder.c_str()) == INVALID_FILE_ATTRIBUTES) {
    ...
    if (error > 0) {
      cout << "ERROR (" << error << "): generating debug path " << outputFolder
           << endl;
  ...
  // I use list instead of vector because when it resizes dynamically it does
  // not copy all the binary trees objects. Thus, changing all the pointer
  // references.
  list<BinaryTree<float> > btBckg;  // for each element in a lineage we store
                                    // the background probability score

  GaussianMixtureModelRedux auxGM;
  TreeNode<float>* auxNode;
  vector<vector<TreeNode<float>*> > mapFrameBlob2bt(
      endFrame + 1);  // stores all the pointers between (frameId,blobId) ->
                      // binary tree, so I can resave later
  for (int frame = iniFrame; frame <= endFrame; frame++) {
    cout << "Background filter: Reading frame " << frame
         << " for forward-backward pass" << endl;
    // read XML file
    char buffer[256];
    ...
    for (int ii = 0; ii < n; ii++) {
      ...
      TreeNode<float>* nodeCh = new TreeNode<
          float>();  // by default everything is initialized to NULL
      ...
      if (auxGM.parentId < 0 || frame == iniFrame)  // new lineage
      {
        ...
        if (auxNode->left == NULL) {
          ...
          cout << "ERROR: at applyProbBackgroundMinMaxRulePerBranch: node "
                  "already has two children"
               << endl;
          return 1;
        }

        // we CANNOT propagate forward information at this point because we do
        // not have a complete lineage, so we do not know when branches start or
        // end
      }
    }
  }

  // forward-backward pass: find all the leaves and splitting points and
  // traverse that branch backward
  cout << "Background filter: forward-backward pass for " << btBckg.size()
       << " lineages" << endl;
  size_t countI = 0;
  for (list<BinaryTree<float> >::iterator iter = btBckg.begin();
       iter != btBckg.end(); ++iter, countI++) {
    ...
    while (q.empty() == false) {
      ...
      if (auxNode->parent != NULL &&
          auxNode->parent->getNumChildren() ==
              1)  // they are part of the same branch
      {
        auxNode->data = std::max(
            auxNode->data, auxNode->parent->data);  // propagate max forward
      }

      if (auxNode->getNumChildren() != 1)  // leave or branch->traverse that
                                           // branch backward to propagate
                                           // information
      {
        while (auxNode->parent != NULL &&
               auxNode->parent->getNumChildren() < 2) {
          auxNode->parent->data = std::max(
              auxNode->data, auxNode->parent->data);  // propagate max backwards
          // update backwards
  ...
  // now all the elements in a branch have the same probability (the
  // maximum)->delete all the branches that do not satisfy that criteria
  long long unsigned int totalBlobs = 0, totalBlobsBackground = 0;
  vector<int> mapParentId, mapParentIdOld;
  for (int frame = iniFrame; frame <= endFrame; frame++) {
    cout << "Background filter: updating frame " << frame
         << " after forward/backward pass" << endl;
    // read XML file
    char buffer[256];
    sprintf(buffer, "%.4d", frame);
    string itoa(buffer);
    ...
    if (outXML.is_open() == false) {
      cout << "ERROR: at applyProbBackgroundMinMaxRulePerBranch: opening "
              "output file "
           << XMLout << endl;
      // release memory
      for (list<BinaryTree<float> >::iterator iter = btBckg.begin();
           iter != btBckg.end(); ++iter)
        iter->clear();
      return 2;
    }
    ...
    for (int ii = 0; ii < n; ii++) {
      ...
      // if(1)
      //----------------------------------------------
      if (mapFrameBlob2bt[frame][ii]->data <
          thrBackground)  // this blob needs to be written
      {
        auxGM.id = countB;
        if (auxGM.parentId >= 0)
  ...
  cout << "Deleted " << totalBlobsBackground << " out of " << totalBlobs
       << " with background thr = " << thrBackground << endl;
...
int applyProbBackgroundAvgRulePerBranch(const string& basenameTGMMresult,
  ...
  vector<vector<TreeNode<float>*> > mapFrameBlob2bt(
      endFrame + 1);  // stores all the pointers between (frameId,blobId) ->
                      // binary tree, so I can resave later
  for (int frame = iniFrame; frame <= endFrame; frame++) {
    cout << "Background filter: Reading frame " << frame << " for forward pass"
         << endl;
    // read XML file
    char buffer[256];
    sprintf(buffer, "%.4d", frame);
  ...
  // backward pass: find all the leaves and splitting points and traverse that
  // branch backward
  cout << "Background filter: backward pass for " << btBckg.size()
       << " lineages" << endl;
  size_t countI = 0;
  for (list<BinaryTree<float> >::iterator iter = btBckg.begin();
       iter != btBckg.end(); ++iter, countI++) {
    ...
    while (q.empty() == false) {
      ...
      if (auxNode->parent != NULL &&
          auxNode->parent->getNumChildren() == 1)  // this is not the beginning
                                                   // of a branch->propagate
                                                   // average forward
      {
        auxNode->data += auxNode->parent->data;
      }  // else: it is a starting point, so leave the current prob

      if (auxNode->getNumChildren() !=
          1)  // leave or branch->traverse that branch backward
      {
...
//========================================================================================================
int applyProbBackgroundHysteresisRulePerBranch(const string& basenameTGMMresult,
                                               int iniFrame, int endFrame,
                                               const string& outputFolder,
                                               float thrLow, float thrHigh) {
#if defined(_WIN32) || defined(_WIN64)
  if (GetFileAttributes(outputFolder.c_str()) == INVALID_FILE_ATTRIBUTES) {
    string cmd = string("mkdir " + outputFolder);
    int error = system(cmd.c_str());
    ...
    while (q.empty() == false) {
      ...
      if (auxNode->parent != NULL &&
          auxNode->parent->getNumChildren() ==
              1)  // they are part of the same branch
      {
        if (auxNode->parent->data > thrHigh &&
            auxNode->data > thrLow)  // we are within the hysteresis process
          auxNode->data = std::max(
              auxNode->data, auxNode->parent->data);  // propagate max forward
      }
                                           ...
                                           // information
      {
        while (auxNode->parent != NULL &&
               auxNode->parent->getNumChildren() < 2) {
          if (auxNode->data > thrHigh &&
              auxNode->parent->data >
                  thrLow)  // we are within the hysteresis process
            auxNode->parent->data =
                std::max(auxNode->data,
                         auxNode->parent->data);  // propagate max backwards
          // update backwards
          auxNode = auxNode->parent;
        }
      }
  ...
  for (int frame = iniFrame; frame <= endFrame; frame++) {
    ...
    for (int ii = 0; ii < n; ii++) {
      ...
      //----------------------------------------------
      if (mapFrameBlob2bt[frame][ii]->data <
          thrHigh)  // this blob needs to be written
      {
        auxGM.id = countB;
        if (auxGM.parentId >= 0)
          auxGM.parentId = mapParentIdOld[auxGM.parentId];
  ...
  cout << "Deleted " << totalBlobsBackground << " out of " << totalBlobs
       << " with background thrHigh = " << thrHigh << "; thrLow=" << thrLow
       << endl;
  return 0;
}
