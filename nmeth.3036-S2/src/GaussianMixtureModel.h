...
static const int numParticles =
    1;  // number of particles to characterize q(mu,sigma) distribution
...
struct meanPrecisionSample {
  Matrix<double, dimsImage, 1>
      mu_k;  // expected value for the mean of teh Gaussian
  Matrix<double, dimsImage, dimsImage> lambda_k;  // expected value of the
                                                  // covariance of the Gaussian.
                                                  // It should be a symmetric
                                                  // matrix
  double w;                                       // weight

  static const double nuProposal;
  static const double betaProposal;  // needed for proposal distribution since
                                     // the one obtained from data is too tight
};
...
class GaussianMixtureModel {
 public:
  ...
  int parentId;
  bool fixed;  // fixed==true->we should not update parameters for this Gaussian
               // mixture
  ...
  Matrix<double, dimsImage, 1>
      m_k;        // expected value for the mean of teh Gaussian
  double beta_k;  // proportionality constant to relate covariance matrix of the
                  // mean of the Gaussian and and the expected value of teh
                  // covariance itself
  Matrix<double, dimsImage, dimsImage> W_k;  // expected value of the covariance
                                             // of the Gaussian. It should be a
                                             // symmetric matrix
  double nu_k;  // degrees of freedom in the covariance. It has to be greater or
                // equal than dimsImage
  double alpha_k;  // responsivities. Expected value of the number of elements
                   // that belong to his Gaussian in the mixture model
  ...
  // priors
  Matrix<double, dimsImage, 1>
      m_o;        // expected value for the mean of teh Gaussian
  double beta_o;  // proportionality constant to relate covariance matrix of the
                  // mean of the Gaussian and and the expected value of teh
                  // covariance itself
  Matrix<double, dimsImage, dimsImage> W_o;  // expected value of the covariance
                                             // of the Gaussian. It should be a
                                             // symmetric matrix
  double nu_o;  // degrees of freedom in the covariance. It has to be greater or
                // equal than dimsImage
  double alpha_o;  // responsivities. Expected value of the number of elements
                   // that belong to his Gaussian in the mixture model
  ...
  static float scale[dimsImage];  // anisotropy in resolution between different
                                  // dimensions
  ...
  double expectedMahalanobisDistance(
      Matrix<double, dimsImage, 1> &x_n);  // equation 10.64
  void updateGaussianParameters(
      const Matrix<double, dimsImage, 1> &x_k,
      const Matrix<double, dimsImage, dimsImage> &S_k);
  void regularizePrecisionMatrix(bool W4DOF);
  //------------------------------------------------------------------------
  // other methods
  void updateCenter() {
    for (int ii = 0; ii < dimsImage; ii++) center[ii] = m_k(ii);
  };
  double distEllipsoid(GaussianMixtureModel &p);  // distance between two
                                                  // Gaussians considering them
                                                  // as ellipsoids
  static double distEllipsoid(meanPrecisionSample &p1,
                              meanPrecisionSample &p2);  // distance between two
                                                         // Gaussians
                                                         // considering them as
                                                         // ellipsoids
  double muLambdaExpectation(
      pfn_muLambdaExpectation p,
      void *data);  // calculates expectation of a function
  ...
  void splitGaussian(
      ...
      int id_);  // splits a Gaussian into two in order to test cell division
  void splitGaussian(GaussianMixtureModel &GM, int id_, float *img,
                     int imSize[dimsImage]) {
    splitGaussian(&GM, id_, img, imSize);
  };  // splits Gaussian using k-means
  void splitGaussian(GaussianMixtureModel *GM, int id_, float *img,
                     int imSize[dimsImage]);  // splits Gaussian using k-means
  void updatePriors(
      double betaPercentageOfN_k, double nuPercentageOfN_k,
      double alphaPercentage,
      double alphaTotal);  // update priors with previous frame state
  void updatePriorsAfterSplitProposal();
  void copyPriors();
  void killMixture(void);
  double volumeGaussian();
  void pixels2units(float scaleOrig[dimsImage]);  // transform elemnts form
                                                  // pixels to metric units
  ...
  inline bool operator<(GaussianMixtureModel const &other) const {
  ...
  static const double
      ellipsoidVolumeConstant;  // needed to calculate ellipsoid volume
  static const double ellipseAreaConstant;  // needed to calculate ellipsoid
                                            // area
...
//=========================================================
inline double GaussianMixtureModel::expectedMahalanobisDistance(
    Matrix<double, dimsImage, 1> &x_n) {
  return (dimsImage / beta_k +
          nu_k * (x_n - m_k).transpose() * W_k * (x_n - m_k));
}

//============================================================
inline void GaussianMixtureModel::updateGaussianParameters(
    const Matrix<double, dimsImage, 1> &x_k,
    const Matrix<double, dimsImage, dimsImage> &S_k) {
  if (fixed == false) {
    beta_k = beta_o + N_k;
    m_k = (beta_o * m_o + N_k * x_k) / beta_k;
    if (W_o(dimsImage - 1, dimsImage - 1) <
    ...
    W_k = (W_o.inverse() + N_k * S_k +
           (beta_o * N_k / (beta_o + N_k)) * (x_k - m_o) *
               (x_k - m_o).transpose())
              .inverse();
    nu_k = nu_o + N_k;
    alpha_k = max(alpha_o + N_k,
                  0.0);  // alpha_o can be negative (improper Dirichlet prior)
  }
}
//==============================================================
// VIP:THIS FUNCTION HAS TO BE THE SAME AS
// GMEMupdateCUDA::GMEMregularizeWkKernel
// static const double lambdaMin=0.02;//aux=scaleSigma/(maxRadius*maxRadius)
// with scaleSigma=2.0 and maxRadius=10 (adjust with scale)
// static const double lambdaMax=0.2222;//aux=scaleSigma/(minRadius*minRadius)
...
inline void GaussianMixtureModel::regularizePrecisionMatrix(bool W4DOF) {
  if (regularizePrecisionMatrixConstants::lambdaMax < 0.0) {
    cout << "ERROR: GaussianMixtureModel::regularizePrecisionMatrix: "
  ...
  double auxMax =
      ...
      nu_k;  // aux=scaleSigma/(minRadius*minRadius*nu_k) with scaleSigma=2.0
  double auxMin =
      regularizePrecisionMatrixConstants::lambdaMin /
      nu_k;  // aux=scaleSigma/(minRadius*minRadius*nu_k) with scaleSigma=2.0
  ...
  for (int ii = 0; ii < dimsImage; ii++) {
    if (scale[ii] < 1e-3) {
      cout << "ERROR: GaussianMixtureModel::regularizePrecisionMatrix: scale "
              "is below zero. it shoudl be set properly before calling this "
              "function"
           << endl;
      exit(3);
    }
  ...
  // reconstruct W_k
  W_k = sigma1.eigenvectors() * d.asDiagonal() *
        sigma1.eigenvectors().transpose();
...
//===============================================================
inline double GaussianMixtureModel::muLambdaExpectation(
    pfn_muLambdaExpectation p, void *data) {
  double E = 0.0;
