...
int parseNucleiList2TGMM(std::vector<GaussianMixtureModel*>& vecGM,
                         lineageHyperTree& lht, int frame, bool regularizeW,
                         float thrDist2LargeDisplacement) {
  if (frame >= lht.getMaxTM()) return 0;
  ...
  if (N > vecGM.size())  // I need to allocate
  {
    ...
    for (size_t ii = N; ii < vecGM.size(); ii++) delete vecGM[ii];
    vecGM.resize(N);
  }

  // reset supervoxels id
  int count = 0;
  for (list<supervoxel>::iterator iterS = lht.supervoxelsList[frame].begin();
       iterS != lht.supervoxelsList[frame].end(); ++iterS, count++)
  ...
  count = 0;
  int countW;
  for (list<nucleus>::iterator iterN = listNucleiPtr->begin();
       iterN != listNucleiPtr->end(); ++iterN, ++count) {
    ...
    vecGM[count]->nu_o =
        dimsImage + 1;  // minimum degrees of freedom are dimsImage +1
    vecGM[count]->sigmaDist_o =
        0;  // to avoid complaint from Valgrind of uninitialized variable
    countW = 0;
    ...
    if (vecGM[count]->W_k(dimsImage - 1, dimsImage - 1) <
        1e-8)  // it means we have a 2D ellipsoid->just regularize W with any
               // value (we will ignore it in the putput anyway)
    {
      vecGM[count]->W_k(dimsImage - 1, dimsImage - 1) =
          0.5 * (vecGM[count]->W_k(0, 0) + vecGM[count]->W_k(1, 1));
    }

    vecGM[count]->W_k = (vecGM[count]->W_k.inverse()) / vecGM[count]->nu_k;

    if (regularizeW == true) vecGM[count]->regularizePrecisionMatrix(true);

    vecGM[count]->W_o =
        vecGM[count]->W_k * vecGM[count]->nu_k / vecGM[count]->nu_o;
    // VIP: we do not recover parent information. We just assume that parentId
    // is the index of the nucleiList[frame]
    vecGM[count]->parentId = count;

    vecGM[count]->lineageId = count;  // some pieces of the code need this info
                                      // to be different than -1;in this case
                                      // everything is from a different lineage

    // save supervoxel
    vecGM[count]->supervoxelIdx.resize(iterN->treeNode.getNumChildren());
    int countS = 0;
    for (vector<ChildrenTypeNucleus>::iterator
             iter = iterN->treeNode.getChildren().begin();
         iter != iterN->treeNode.getChildren().end(); ++iter, countS++) {
      vecGM[count]->supervoxelIdx[countS] = (int)((*iter)->tempWildcard);
    }

    // save background probability for this nucleus
    vecGM[count]->beta_o = iterN->probBackground;  // TODO: use its own
                                                   // variable, although here it
                                                   // was set to zero anyways

    // save split score based on confidence: link to CATMAID editing
    // vecGM[count]->splitScore = iterN->confidence;
    vecGM[count]->splitScore = lht.confidenceScoreForNucleus(
        iterN->treeNodePtr, thrDist2LargeDisplacement);  // TODO: calculate this
                                                         // before hand and save
                                                         // it in confidence

    // vecGM[count]->splitScore = iterN->debugVisualization;//uncomment this to
    // be able to visualize specific cases
  }

  // cout<<"=============DEBUGGING: confidence split score disconnected to study
  // incoherences========================="<<endl;
  // cout<<"=============REMINDER: confidence split score NOT disconnected to
  // study incoherences========================="<<endl;

  return 0;
}

//=============================================================================================================
int parseHierarchicalSegmentation2LineageHyperTree(hierarchicalSegmentation* hs,
  ...
  while (q.empty() == false) {
    ...
    if (aux->data.thrTau > hs->getMaxTau()) {
      ...
      // NOTE: we do not write out objects that are clearly segmented (ie,
      // isolated nodes that between minTau and maxTau have no connections in
      // the dendrogram)
      ...
      queue<TreeNode<ChildrenTypeLineage>*>
          qLHT;  // stores parent in LHT binary tree
      qHS.push(aux);
      qLHT.push(NULL);
      ...
      while (qHS.empty() == false) {
        ...
        ChildrenTypeNucleus iterS =
            ((++(lht.supervoxelsList[TM].rbegin()))
                 .base());  // iterator for the last element in the list
        ParentTypeSupervoxel iterN = lht.addNucleusFromSupervoxel(
            TM, iterS);  // creates a nucleus from a single supervoxel and
                         // updates the hypergraph properly. Returns an iterator
        ...
        if (iterN->treeNodePtr == NULL) {
          cout << "ERROR: at parseHierarchicalSegmentation2LineageHyperTree: "
                  "inerting node into binary tree"
               << endl;
          return 3;
        }
...
//=========================================================================================
//=============================================================================================================
int debugMergeSplitHStoLHT(hierarchicalSegmentation* hs, lineageHyperTree& lht,
                           imgVoxelType tau) {
  ...
  for (size_t ii = 0; ii < hs->currentSegmentatioSupervoxel.size(); ii++) {
    ...
    TreeNode<nodeHierarchicalSegmentation> *auxNode =
                                               hs->currentSegmentationNodes[ii],
                                           *rootMerge =
                                               hs->currentSegmentationNodes[ii];
    supervoxel auxSv;
    ...
    while (score > 0.0f) {
      auxSv = rootMergeSv;
      auxNode = rootMerge;
      score = hs->suggestMerge<unsigned short int>(auxNode, auxSv, &rootMerge,
                                                   rootMergeSv);
    }
    ...
    while (qHS.empty() == false) {
      ...
      ChildrenTypeNucleus iterS =
          ((++(lht.supervoxelsList[TM].rbegin()))
               .base());  // iterator for the last element in the list
      ParentTypeSupervoxel iterN = lht.addNucleusFromSupervoxel(
          TM, iterS);  // creates a nucleus from a single supervoxel and updates
                       // the hypergraph properly. Returns an iterator to the
                       // created element
      ...
      if( TM == 0 && lht.supervoxelsList[ TM ].size() == 2363 )
      {
              ...
              cout<<"ii = "<<ii<<";supervoxel = "<<(*iterS)<<";par tauThr =
      "<<auxNode->parent->data.thrTau<<endl;
              exit(3);
      }
      */
...
//=====================================================================
TreeNode<ChildrenTypeLineage>* addSupervoxelsPointersFromLineage(
    std::vector<vector<supervoxel*> >& svIniVec, int iniTM, int endTM,
...
void transposeStackUINT16(mylib::Array* img) {
  // cout<<"WARNING: transposing each slice to agree with Matlab
  // convention"<<endl;
  ...
  if (img->ndims != 3) {
    cout << "ERROR: at transposeStackUINT16: code only ready for 3D arrays"
         << endl;
    exit(3);
  }
...
//==============================================================
int extendDeadNucleiAtTMwithHS(lineageHyperTree& lht,
                               hierarchicalSegmentation* hsForward, int TM,
                               int& numExtensions, int& numDeaths) {
  numExtensions = 0;
  numDeaths = 0;

  if (TM < 0 || TM >= (int)(lht.getMaxTM())) return 0;
  ...
  for (list<nucleus>::iterator iterN = lht.nucleiList[TM].begin();
    ...
    if (iterN->treeNodePtr->getNumChildren() == 0) {
      numDeaths++;
      numExtensions +=
          extendDeadNucleiWithHS(lht, hsForward, iterN->treeNodePtr);
    }
  }
...
//=============================================================================================================================
int extendDeadNucleiWithHS(lineageHyperTree& lht,
                           hierarchicalSegmentation* hsForward,
  ...
  {
    iterNucNew = lht.addNucleusFromSupervoxel(
        rootDead->data->TM + 1,
  ...
  // Check in supervoxel cancidates in t+1 if splitting them should help. If it
  // does ->do it before calculating Hungarian algorithm

  float ddNoSplit;
  TreeNode<nodeHierarchicalSegmentation>* rootSplit[2];
  supervoxel rootSplitSv[2];
  float ddSplit[2];
  // items needed to know correspondence between hsForward->currentSegmentation
  // and lht.supervoxelsList[rootDead->data->TM + 1]
  ...
  for (vector<ChildrenTypeNucleus>::iterator iterNN = svListIterVec.begin();
       iterNN != svListIterVec.end(); ++iterNN, countNN++)
    (*iterNN)->tempWildcard = countNN;

  for (list<supervoxel>::iterator iter = svListT0.begin();
       iter != svListT0.end(); ++iter) {
    size_t sizeVec = iter->nearestNeighborsInTimeForward.size();
    for (size_t cc = 0; cc < sizeVec;
         cc++)  // we need to access using indexes because we push_back into
                // iter->nearestNeighborsInTimeForward if a split is done
    {
      SibilingTypeSupervoxel iterNeigh =
          iter->nearestNeighborsInTimeForward[cc];
      ddNoSplit = supervoxelFromNucleus.JaccardDistance(
          *iterNeigh);                // calculate distance without splitting
      float ddThr = ddNoSplit - 0.1;  // it has to be significantly better

      // propose split
      float prob = hsForward->suggestSplit<float>(
          (*iterNeigh).nodeHSptr, (*iterNeigh), rootSplit,
          rootSplitSv);  // probability of the split
      ...
      if (ddSplit[0] < ddThr ||
          ddSplit[1] < ddThr)  // incorporate split into solution space
      {
        // if matching is significantly better->proceed with to split
        // supervoxels and incorporate them in the lineage
        ...
        // lht->supervoxelsList[TM] was created as a copy of
        // hs->currentSegmentation for backwards compatibility (even if it is
        // redundant). So we just need tokeep that structure
        int aa = (int)((*iterNeigh).tempWildcard);
        hsForward->currentSegmentatioSupervoxel[aa] =
            rootSplitSv[0];  // root split has the correct nodeHSptr
        hsForward->currentSegmentatioSupervoxel.push_back(
            rootSplitSv[1]);  // this could dynamically reallocate memory and
                              // make all the pointers nodeHSptr->data.svPtr
                              // invalid (the crash would be obvious). So I
                              // reserve when creating a partition.

        hsForward->currentSegmentationNodes[aa] = rootSplit[0];
        hsForward->currentSegmentationNodes.push_back(rootSplit[1]);

        // update nucleus-sv info (remember: lht->supervoxelsList[TM] was
        // created as a copy of hs->currentSegmentation for backwards
        // compatibility);
        // I actually only need to change nucleus-supervoxel by adding children
        // to nucleus since it was a split;
        (*(svListIterVec[aa])) = rootSplitSv[0];
        (svListIterVec[aa])->tempWildcard = aa;
        lht.supervoxelsList[TMforward].push_back(rootSplitSv[1]);
        SibilingTypeSupervoxel iterSadded =
            (++(lht.supervoxelsList[TMforward].rbegin()))
                .base();  // get iterator to added element
        iterSadded->tempWildcard = lht.supervoxelsList[TMforward].size() - 1;
        (*iterNeigh).treeNode.getParent()->treeNode.addChild(iterSadded);

        // add new supervoxels in iter->NEARESTneighbors list for Hungarian
        // algorithm;
  ...
  for (iterS = rootDead->data->treeNode.getChildren().begin();
        ...
        false)  // the assigned element has no parent-> we can claim it directly
    {
      ...
      {
        lht.nucleiList[rootDead->data->TM + 1].push_back(
            nucleus(rootDead->data->TM + 1, assignmentId[count]->centroid));
        iterNucNew = (++(lht.nucleiList[rootDead->data->TM + 1].rbegin()))
...
//============================================================================
template int parseNucleiList2TGMM<float>(
    std::vector<GaussianMixtureModel*>& vecGM, lineageHyperTree& lht, int frame,
    bool regularizeW, float thrDist2LargeDisplacement);
template int parseNucleiList2TGMM<unsigned short int>(
    std::vector<GaussianMixtureModel*>& vecGM, lineageHyperTree& lht, int frame,
    bool regularizeW, float thrDist2LargeDisplacement);
template int parseNucleiList2TGMM<unsigned char>(
    std::vector<GaussianMixtureModel*>& vecGM, lineageHyperTree& lht, int frame,
    bool regularizeW, float thrDist2LargeDisplacement);
