...
//============================================================================
nucleus::~nucleus() { treeNodePtr = NULL; }
...
void nucleus::nucleusDefault(int time_, float* centroid_) {
  TM = time_;
  for (int ii = 0; ii < dimsImage; ii++) centroid[ii] = centroid_[ii];
...
bool nucleus::operator<(nucleus const& other) const {
  return (this->TM < other.TM);  // I could expand it so if they belong to the
                                 // same frame, use centroid
};
