...
//==================================================================
template <int D>
float localGeometricDescriptor<D>::distance(const localGeometricDescriptor &p,
                                            size_t k, float scale[D]) const {
  ...
  for (size_t ii = 0; ii < neighPts[0].size(); ii++) {
    ...
    for (size_t jj = 0; jj < p.neighPts[0].size();
         jj++)  // we xpect number of neighboring points to be small, so brute
                // force search should be OK
    {
      dAux = 0.0f;
      for (int kk = 0; kk < D; kk++)
        dAux += (p.neighPts[kk][jj] - neighPts[kk][ii]) *
                (p.neighPts[kk][jj] - neighPts[kk][ii]) * scale[kk] * scale[kk];
  ...
  dAux = 0;
  for (size_t ii = 0; ii < min(k, d.size()); ii++) dAux += d[ii];
...
//============================================================
template <int D>
void localGeometricDescriptor<D>::setNeighPts(
    const float p0[D], const vector<float> neighPts_[D]) {
