...
//===================================================================================
int lineageHyperTree::readListSupervoxelsFromTifWithWeightedCentroid(string filenameImage,string filenameLabels,int vecPosTM)
{
	//open image
...
int lineageHyperTree::parseTGMMframeResult(vector<GaussianMixtureModelRedux*> &vecGM, int vecPosTM)
{
	...
	for(list<nucleus>::iterator iter = listPtr->begin(); iter != listPtr->end(); count++,++iterL)
	{
		...
		if(vecRefLastTreeNode.empty() == false)//we have previous lineages
		{
			if(parent < 0)
			{
				cout<<"ERROR: at lineageHyperTree::parseTGMMframeResult: blob does not have a parent even if time point>0. Something must be wrong"<<endl;
				return 3;
			}
...
//---------------------------------------------------
void lineageHyperTree::debugPrintLineageForLocalLineageDisplayinMatlab(string imgPath, string imgLPath, string suffix, string imgRawPath)
{
	...
	for(unsigned int ii = 0; ii<maxTM;ii++)
	{
		...
		debugCanvasFromSegmentationCroppedRegion(ii, minXYZ, maxXYZ,imgBasename, imgRawPath);
	}
...
void lineageHyperTree::debugCanvasFromSegmentationCroppedRegion(unsigned int TM, uint64 minXYZ[dimsImage], uint64 maxXYZ[dimsImage],string imgBasename, string imgRawPath)
{
	...
	mylib::uint16* imgptr = (mylib::uint16*)(supervoxelsList[TM].begin()->dataPtr);
...
int lineageHyperTree::supervoxelNearestNeighborsInSpace(unsigned int TM,unsigned int KmaxNumNN, float KmaxDistKNN,int devCUDA)//you need to setup scale to calculate this properly
{
	...
	if(KmaxNumNN > maxKNN)
	{
		cout<<"ERROR: at supervoxelNearestNeighborsInSpace: maximum number of NN "<<KmaxNumNN<<" is superior to maxKNN "<<maxKNN<<". Please recompile knnCUDA.h code with a larger constant"<<endl;
		return 2;
	}
	...
	int err = knnCUDA_( ind, dist, xyz, xyz, query_nb, ref_nb, KmaxNumNN, supervoxel::getScale(), devCUDA);
	if(err>0)
		return err;
...
int lineageHyperTree::supervoxelNearestNeighborsInTimeForward(unsigned int TM,unsigned int KmaxNumNN, float KmaxDistKNN,int devCUDA)//you need to setup scale to calculate this properly
{
	...
	if(KmaxNumNN > maxKNN)
	{
		cout<<"ERROR: at supervoxelNearestNeighborsInTimeForward: maximum number of NN "<<KmaxNumNN<<" is superior to maxKNN "<<maxKNN<<". Please recompile knnCUDA.h code with a larger constant"<<endl;
		return 2;
	}
	...
	//calculate nearest neighbors
	int err = knnCUDA_( ind, dist, query_xyz, ref_xyz, query_nb, ref_nb,KmaxNumNN, supervoxel::getScale(), devCUDA);
	if(err>0)
		return err;
...
int lineageHyperTree::supervoxelNearestNeighborsInTimeBackward(unsigned int TM,unsigned int KmaxNumNN, float KmaxDistKNN,int devCUDA)//you need to setup scale to calculate this properly
{
	...
	if(KmaxNumNN > maxKNN)
	{
		cout<<"ERROR: at supervoxelNearestNeighborsInTimeBackward: maximum number of NN "<<KmaxNumNN<<" is superior to maxKNN "<<maxKNN<<". Please recompile knnCUDA.h code with a larger constant"<<endl;
		return 2;
	}
...
//===================================================================================
//=============================metrics===============================================
float lineageHyperTree::offsetJaccardDistance(TreeNode<ChildrenTypeLineage>* node)
{
	if( node->parent == NULL )
		return 0;
	if( node->data->treeNode.getNumChildren() == 0 ||  node->parent->data->treeNode.getNumChildren() == 0 )
		return 0;
	...
	if( node->parent->data->treeNode.getNumChildren() == 1)
	{
		...
		//merge all superovxels into one
		vector< SibilingTypeSupervoxel >::iterator iterS = node->parent->data->treeNode.getChildren().begin();
...
//=============================================================================================================================
int lineageHyperTree::mergeShortLivedDaughters (TreeNode<ChildrenTypeLineage>* rootSplit, int lengthTMthr)
{
...
//========================================================================
int lineageHyperTree::mergeShortLivedDaughtersAll(int lengthTMthr,int maxTM, int& numCorrections, int& numSplits)
{
	numCorrections = 0;
	numSplits = 0;
	TreeNode<ChildrenTypeLineage>* aux;	
...
int lineageHyperTree::breakCellDivisionBasedOnCellDivisionPlaneConstraint(int TM, double thrCellDivisionPlaneDistance ,int &numCorrections, int& numSplits)
{
	...
	for(list<nucleus>::iterator iterN = nucleiList[TM].begin(); iterN != nucleiList[TM].end(); ++iterN)	
	{
		...
		for(int ii = 0; ii<dimsImage; ii++)
		{
			p0 = 0.5 * (aux->left->data->centroid[ii] + aux->right->data->centroid[ii]) ; //midpoint
...
//========================================================================
int lineageHyperTree::mergeParallelLineagesAll(int conn3D, size_t minNeighboringVoxels, int &numMerges)
{
		...
		if(root1 != NULL)
		{
			...
			for(vector<ChildrenTypeNucleus>::iterator iterS2 = root1->data->treeNode.getChildren().begin(); iterS2 != root1->data->treeNode.getChildren().end(); ++iterS2)
			{
				for(vector<ChildrenTypeNucleus>::iterator iterS1 = (*iterS2)->nearestNeighborsInSpace.begin(); iterS1 != (*iterS2)->nearestNeighborsInSpace.end(); ++iterS1)
				{
					...
					if ( mergeParallelLineages(root2, root1, conn3D, neighOffset, minNeighboringVoxels) > 0 )//root1 is distroyed here
					{
						numMerges++;
						flag = true;
						break;
...
//========================================================================
int lineageHyperTree::mergeNonSeparatingDaughtersAll(int maxTM, size_t minNeighboringVoxels, int conn3D, int& numCorrections, int& numSplits)
{
	int64 boundarySize[dimsImage];
	int64* neighOffset = supervoxel::buildNeighboorhoodConnectivity(conn3D, boundarySize);
	numCorrections = 0;
	...
	for(list<lineage>::iterator iterL = lineagesList.begin(); iterL != lineagesList.end(); ++iterL)
	{
		...
		while(!q.empty())
		{
			...
			{				
				numSplits++;
				if ( mergeNonSeparatingDaughters(aux, conn3D, neighOffset, minNeighboringVoxels) > 0 )
					numCorrections++;	
			}
...
//========================================================================
int lineageHyperTree::mergeShortLivedAndCloseByDaughtersAll(int lengthTMthr,int maxTM, size_t minNeighboringVoxels, int conn3D, int& numCorrections, int& numSplits)
{
	int64 boundarySize[dimsImage];
	int64* neighOffset = supervoxel::buildNeighboorhoodConnectivity(conn3D, boundarySize);
	numCorrections = 0;
...
//================================================================================================================
int lineageHyperTree::mergeNonSeparatingDaughters(TreeNode<ChildrenTypeLineage>* rootSplit,int conn3D, int64* neighOffset)
{
...
//===================================================================================================
int lineageHyperTree::mergeNonSeparatingDaughters(TreeNode<ChildrenTypeLineage>* rootSplit,int conn3D, int64* neighOffset, size_t minNeighboringVoxels)//TODO: generate a function that once it has found minNeighboringVoxels it stops checking
{
...
//======================================================================================================
int lineageHyperTree::mergeParallelLineages(TreeNode<ChildrenTypeLineage>* root1, TreeNode<ChildrenTypeLineage>* root2,int conn3D, int64* neighOffset, size_t minNeighboringVoxels)
{
...
//=============================================================================================
int lineageHyperTree::mergeBranches(TreeNode<ChildrenTypeLineage>* root1 ,TreeNode<ChildrenTypeLineage>* root2)
{
	if(root1 == NULL || root2 == NULL) 
		return 0;
	if(root1 == root2)
...
int lineageHyperTree::calculateNucleiIntensityCentroid(SibilingTypeNucleus& iterN)
{
...
template<class imgTypeC>
void lineageHyperTree::calculateGMMparametersFromNuclei(list<nucleus>::iterator &iterN, float* m_k, float *N_k, float* S_k)
{
...
//================================================================================================================================
//se use this function sequentially, so we assume we only need to look at TM-1 to delete elements (call if sequentially if you want to really delete a range of TM). The most difficult part is when nuclei divide since we have to generate two different sublineages for each of the daughters
void lineageHyperTree::setFrameAsT_o(unsigned int TM)
{
	...
	if( supervoxelsList[TM-1].empty() == false)
	{
		if(supervoxelsList[TM-1].begin()->dataPtr != NULL)
			free( supervoxelsList[TM-1].begin()->dataPtr );//free because it was allocated using mylib
...
int lineageHyperTree::writeListSupervoxelsToBinaryFile(string filename, unsigned int TM)
{
...
//==============================================================================
//predefine templates
template void lineageHyperTree::calculateGMMparametersFromNuclei<float>(list<nucleus>::iterator &iterN, float* m_k, float *N_k, float* S_k);
template void lineageHyperTree::calculateGMMparametersFromNuclei<unsigned short int>(list<nucleus>::iterator &iterN, float* m_k, float *N_k, float* S_k);
template void lineageHyperTree::calculateGMMparametersFromNuclei<unsigned char>(list<nucleus>::iterator &iterN, float* m_k, float *N_k, float* S_k);

template int lineageHyperTree::calculateNucleiIntensityCentroid<float>(SibilingTypeNucleus& iterN);
template int lineageHyperTree::calculateNucleiIntensityCentroid<unsigned char>(SibilingTypeNucleus& iterN);
template int lineageHyperTree::calculateNucleiIntensityCentroid<unsigned short int>(SibilingTypeNucleus& iterN);
