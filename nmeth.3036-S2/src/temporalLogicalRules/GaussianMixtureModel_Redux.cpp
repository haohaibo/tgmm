...
//=============================================================
GaussianMixtureModelRedux::GaussianMixtureModelRedux(
    const GaussianMixtureModelRedux& p) {
  // m_k=p.m_k;
...
//===========================================================
GaussianMixtureModelRedux& GaussianMixtureModelRedux::operator=(
    const GaussianMixtureModelRedux& p) {
  if (this != &p) {
    // m_k=p.m_k;
    memcpy(m_k, p.m_k, sizeof(double) * dimsImage);
...
// files
GaussianMixtureModelRedux::GaussianMixtureModelRedux(XMLNode& xml,
                                                     int position) {
  N_k = 0.0;
...
ostream& GaussianMixtureModelRedux::writeXML(ostream& os) {
  ...
  count = 0;
  for (int ii = 0; ii < dimsImage; ii++)
    for (int jj = 0; jj < dimsImage; jj++) os << W_o[count++] << " ";
...
int readGaussianMixtureModelXMLfile(string filename,
  ...
  if (!fid.is_open()) {
    cout << "ERROR: at readGaussianMixtureModelXMLfile: file " << filename
         << " could not be openned" << endl;
    return 1;
  }
  fid.close();
