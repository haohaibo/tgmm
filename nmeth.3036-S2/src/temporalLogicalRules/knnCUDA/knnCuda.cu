...
__global__ void __launch_bounds__(MAX_THREADS) knnKernelNoConstantMemory(int *indCUDA,float *distCUDA,float *queryCUDA,float* anchorCUDA,int ref_nb,long long int query_nb, int KNN)
{
	...
	float minDist[maxKNN];//to mantain distance for each index: since K is very small instead of a priority queue we keep a sorted array
	int indAux[maxKNN];
...
int knnCUDA_(int *ind,float* dist, float *query,float *ref,long long int query_nb,int ref_nb, int KNN, float* scale, int devCUDA)
{
	...
	//int*   ind;                 // Pointer to index array: size query_nb*maxKNN. Again, order is GPU cache friendly.
	//float*   dist;              // Pointer to distance^2 array: size query_nb*maxKNN. Again, order is GPU cache friendly. If pointer is null, scaled euclidean distance to each nearest neighbor is not returned
