 ...
 * nuclei.h
 *
 * \brief Simple class containing information for a nuclei. It is better to keep
 * it separate and depending on the application the attributes you want to save
 * are different
 *
 */
...
class nucleus {
 public:
  ...
  TreeNode<ChildrenTypeLineage>* treeNodePtr;  // points at the TreeNode within
                                               // the lineage. NULL if it is not
                                               // associated with any lineage
  // vector< SibilingTypeNucleus > sibilings;//for nearest neighbors of other
  // nuclei. IT IS DONE THROUGH SUPERVOXELS
  float tempWilcard;  // temporary storage space for different purposes. IT IS
                      // NOT COPIED WITH THE ASSIGNMENT OPERATOR OR THE COPY
                      // CONSTRUCTOR!!!

  float debugVisualization;  // to mark specific nucleus in order to easily find
                             // them in the visualization

  float confidence;  // parameter to match form CATMAID (or any other estimate
                     // of how confident we are about this nucleus);
  float probBackground;  // score from background classifier. The higher, the
                         // more likely this nucleus is to be background
  ...
  int addSupervoxelToNucleus(
      const ChildrenTypeNucleus& ch)  // returns>0 if there was an error
  {
  ...
  int removeSupervoxelFromNucleus(ChildrenTypeNucleus& sv) {
    for (vector<ChildrenTypeNucleus>::iterator iter =
             treeNode.getChildren().begin();
         iter != treeNode.getChildren().end(); ++iter) {
      if (sv == (*iter)) {
  ...
  bool isDead() { return centroid[0] < -1e30f; }
...
//=================================================
inline float nucleus::Euclidean2Distance(const nucleus& s,
                                         float scale[dimsImage]) const {
  float dist = 0;
  float aux = 0;
