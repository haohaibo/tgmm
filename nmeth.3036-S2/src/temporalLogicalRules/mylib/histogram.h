...
Histogram *G(Make_Histogram)(Value_Kind kind, int nbins, Value binsize,
                             Value offset);
Histogram *Empty_Histogram(Histogram *R(M(h)));

Histogram *G(Histogram_Array)(AForm *a, int nbins, Value binsize, Value offset);
Histogram *G(Histogram_Region)(Array *a, Region *r, int nbins, Value binsize,
                               Value offset);
Histogram *G(Histogram_P_Vertex)(Array *a, Partition *p, int v, int nbins,
                                 Value binsize, Value offset);
...
Histogram *Histagain_Region(Histogram *R(M(h)), Array *a, Region *r,
                            boolean clip);
Histogram *Histagain_P_Vertex(Histogram *R(M(h)), Array *a, Partition *p, int v,
                              boolean clip);
Histogram *Histagain_Level_Set(Histogram *R(M(h)), Array *a, Level_Tree *t,
                               Level_Set *r, boolean clip);
...
#define ASCENDING_HGRAM \
  0x08  //  display in ascending order (descending by default)
...
#define BIN_MIDDLE \
  0x40  //  display the mid-value of a bin as opposed to its range

void Print_Histogram(Histogram *h, FILE *output, int indent, int flags,
                     Value binsize);
