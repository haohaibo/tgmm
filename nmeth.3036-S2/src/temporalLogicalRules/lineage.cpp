...
lineage::~lineage() { bt.clear(); }
lineage::lineage(const lineage& p) { bt = p.bt; }
...
//==================================================================
//--------------------statistics-----------------------------
int lineage::daughterLengthToDeath(
    TreeNode<ChildrenTypeLineage>* rootSplit) const {
  if (rootSplit == NULL) {
    cout << "ERROR: at lineage::daughterLengthToDeath: rootSplit node is null"
         << endl;
    exit(3);
  }
  if (rootSplit->left == NULL || rootSplit->right == NULL) {
    cout << "ERROR: at lineage::daughterLengthToDeath: rootSplit is not a split"
         << endl;
    exit(2);
  }
  ...
  aux = rootSplit->right;
  while (aux->getNumChildren() == 1 &&
         llAux2 < llAux1)  // if we overtake llAux1 it does not make sense
  {
    if (aux->left != NULL)
      aux = aux->left;
    else
  ...
  if (aux->getNumChildren() == 2) {
    ...
    if (llAux1 == 2147483647)
      ...
      return min(llAux1, llAux2);  // none of the daughters divided
  }
}
//--------------------------------------------------------------------
...
//--------------------statistics-----------------------------
int lineage::daughterLengthToDivision(TreeNode<ChildrenTypeLineage>* rootSplit,
                                      bool Left) const {
  if (rootSplit == NULL) {
    cout
        << "ERROR: at lineage::daughterLengthToDivision: rootSplit node is null"
        << endl;
    exit(3);
  }
  if (rootSplit->left == NULL || rootSplit->right == NULL) {
    cout << "ERROR: at lineage::daughterLengthToDivision: rootSplit is not a "
            "split"
         << endl;
    exit(2);
  }
