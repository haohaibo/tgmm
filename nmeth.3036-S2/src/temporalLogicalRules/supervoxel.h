...
public:
	...
	int weightedGaussianStatistics(double*m, double* W,float* intensity, bool regularizeW);//calculate mean (m) and precision matrix( W ) from supervoxel (so we have an equivalent Gaussian). double W[dimsImage * (1+dimsImage) /2] (symmetric matrix). We weight based on the intensity
	template<class imgTypeC>
	int weightedGaussianStatistics(bool regularizeW);//calculate mean (m) and precision matrix( W ) from supervoxel (so we have an equivalent Gaussian). double W[dimsImage * (1+dimsImage) /2] (symmetric matrix). We weight based on the intensity
	template<class imgTypeC>
	int weightedCentroid();//only the centroid (in case we do not need the other stats)
	template<class imgTypeC>
	int robustGaussianStatistics(double*m, double* W, int maxSize, float maxPercentile);//calculate mean (m) and precision matrix( W ) from supervoxel (so we have an equivalent Gaussian). double W[dimsImage * (1+dimsImage) /2] (symmetric matrix). We "prefilter" elements before calculating Gaussian statistics
	
	template<class imgTypeC>
	imgTypeC intensityThreshold(int maxSize, float maxPercentile, unsigned int *maxPosPixelIdxList, bool localBackgroundSubtracted);//calculates intensity threshold based on 3 criteria: maximum size, maxPrecentile and otzu's threshold. Returns also the position of the maximum in PixelIdxList
	template<class imgTypeC>
	imgTypeC trimSupervoxel(int maxSize, float maxPercentile, int conn3D, int64* neighOffset);//reduces the number of PixelIdxList based on intensityThreshold() and connectivity starting from the maximum (an attempt to segment nucleus after image partition with supervoxels)

	template<class imgTypeC>
	imgTypeC trimSupervoxel(){ return trimSupervoxel<imgTypeC>(maxSizeTrim, maxPrecentileTrim, conn3Dtrim, neighOffsetTrim);};//it uses static values within teh class supervoxel

	template<class imgTypeC>
	void removeZeros();//if we have already done local backgroudn subtraction, we just need to subtract zeros instead of trimming
...
//====================================================
inline void supervoxel::setTrimParameters(int maxSize, float maxPercentile, int conn3D)
{
	maxSizeTrim = maxSize;
