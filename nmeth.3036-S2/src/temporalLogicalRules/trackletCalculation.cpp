...
int calculateTrackletsWithSparseHungarianAlgorithm(
  ...
  for (list<supervoxel>::iterator iter = svListT0.begin();
    ...
    for (vector<SibilingTypeSupervoxel>::iterator iterNeigh =
             iter->nearestNeighborsInTimeForward.begin();
         iterNeigh != iter->nearestNeighborsInTimeForward.end(); ++iterNeigh) {
      (*iterNeigh)->tempWildcard =
          -1.0f;  // so we know which ones have not been assigned
      E++;
    }
  }

  // setup distance method
  float (supervoxel::*dd_func_ptr)(const supervoxel&)
      const;  // define pointer to function-member class
  ...
  switch (distanceMethod) {
    ...
    case 1:  // Euclidean
      ...
      thrCost =
          thrCost *
          thrCost;  // the distance is calculate without sqrt to save operations
      break;
    default:
      cout << "ERROR: at calculateTrackletsWithSparseHungarianAlgorithm: "
              "distanceMEthod not supported by the code"
           << endl;
      return 1;
  }

  // set workspace
  workspaceSparseHungarian* W =
      (workspaceSparseHungarian*)malloc(sizeof(workspaceSparseHungarian));
  workspaceSpraseHungarian_init(
      W, svListT0.size(), 0, E,
      thrCost);  // we need to update M after going trhough all the neighbors
  ...
  if (W->E == 0)  // degenerate case
  {
    for (int ii = 0; ii < W->N; ii++)
      assignmentIdAux[ii] =
          -1;  // garbage potential (no assignment) for all of them
  } else {     // regular case
...
int calculateTrackletsWithSparseHungarianAlgorithm(lineageHyperTree& lht,
  ...
  if (numThreads > 0) {
    cout << "WARNING: at calculateTrackletsWithSparseHungarianAlgorithm: code "
            "is not multi-threaded yet"
         << endl;  // TODO
  }

  // calculate assignment for each pair of time points. TODO: parallelize this
  // piece of code. Ishould be straight forward
  vector<vector<SibilingTypeSupervoxel> >
      assignmentId;  // assignmentId[ii] contains assignments from time jj->jj+1
  ...
  for (int ii = 0; ii < lht.getMaxTM() - 1; ii++) {
    ...
    int err = calculateTrackletsWithSparseHungarianAlgorithm(
        lht.supervoxelsList[ii], distanceMethod, thrCost, assignmentId[ii],
        &nullAssignment);
    if (err > 0) return err;
  }

  // delete all the lineages
  for (list<lineage>::iterator iterL = lht.lineagesList.begin();
       iterL != lht.lineagesList.end(); ++iterL) {
    iterL->bt.remove(
        iterL->bt.pointer_mainRoot());  // deletes all the lineage recursively
  }
  ...
  for (int ii = 0; ii < lht.getMaxTM(); ii++) {
    ...
    for (list<supervoxel>::iterator iterS = lht.supervoxelsList[ii].begin();
         iterS != lht.supervoxelsList[ii].end(); ++iterS, count++) {
      iterS->treeNode.deleteParent();
      iterS->tempWildcard = (float)count;
    }
  }

  // reconstruct nuclei and lineages from assignments (in this case one nuclei
  // correspond to one supervoxel)
  ...
  for (int ii = 0; ii < lht.getMaxTM() - 1; ii++) {
    ...
    for (list<supervoxel>::iterator iterS = lht.supervoxelsList[ii].begin();
         iterS != lht.supervoxelsList[ii].end(); ++iterS, countS++) {
      // if(iterS->tempWildcard < 0.0f || assignmentId[ii][countS]->centroid[0]
      // < -1e30f) //supervoxel has already been visited or it was not assigned
      if (iterS->tempWildcard < 0.0f)  // supervoxel has already been visited.
                                       // Thus, we allow tracklets of single
                                       // supervoxels
        continue;
      ...
      ParentTypeNucleus iterL = (++(lht.lineagesList.rbegin()))
                                    .base();  // iterator to last added lineage
      int posTM = ii, posID;
      while ((auxS)->centroid[0] > 0.0f)  // compare to null assignment
      {
        ...
        (auxS)->tempWildcard = -1.0f;  // signal that supervoxel has been
                                       // visited
  ...
  for (list<supervoxel>::iterator iterS = lht.supervoxelsList[posTM].begin();
       iterS != lht.supervoxelsList[posTM].end(); ++iterS) {
    if (iterS->tempWildcard < 0.0f)  // supervoxel has already been visited.
                                     // Thus, we allow tracklets of single
                                     // supervoxels
      continue;
...
int extendMatchingWithClearOneToOneAssignments(
  ...
  if (superVoxelB.size() != nearestNeighborVecBtoA.size()) {
    std::cout << "ERROR: "
                 "calculateSupervoxelMatchingOneToManyWithSparseHungarianAlgori"
                 "thm: size of nearest neighbor vector BtoA is not the same as "
                 "candidates list"
              << endl;
    return 2;
  }
  ...
  if (superVoxelA.size() != nearestNeighborVecAtoB.size()) {
    std::cout << "ERROR: "
                 "calculateSupervoxelMatchingOneToManyWithSparseHungarianAlgori"
                 "thm: size of nearest neighbor vector AtoB is not the same as "
                 "input list"
              << endl;
    return 2;
  }

  // generate a list of candidate elements that have been matched already (so we
  // do not duplicate)
  ...
  int ii = 0;
  for (list<supervoxel>::iterator iterS = superVoxelA.begin();
       iterS != superVoxelA.end(); ++iterS, ++ii)
    iterS->tempWildcard = ((float)ii);
  ii = 0;
  vector<list<supervoxel>::iterator> vecIterB(
      M);  // array of iterators to be able to access list
  for (list<supervoxel>::iterator iterS = superVoxelB.begin();
       iterS != superVoxelB.end(); ++iterS, ++ii) {
    iterS->tempWildcard = ((float)ii);
    vecIterB[ii] = iterS;
  }
  ...
  for (list<supervoxel>::iterator iterS = superVoxelA.begin();
    ...
    for (vector<SibilingTypeSupervoxel>::iterator iter =
             nearestNeighborVecAtoB[ii].begin();
         iter != nearestNeighborVecAtoB[ii].end(); ++iter) {
    ...
    if (idBestAB >= 0) {
      ...
      float costBestBA =
          costBestAB + 0.1f;  // to avoid floating point error in comparison

      for (vector<SibilingTypeSupervoxel>::iterator iter =
               nearestNeighborVecBtoA[idBestAB].begin();
           iter != nearestNeighborVecBtoA[idBestAB].end(); ++iter) {
  ...
  std::cout << "INFO: extendMatchingWithClearOneToOneAssignments: number of "
               "extended deaths "
            << numExtended << " out of " << numDeaths
            << " elements that are not assigned yet." << endl;
...
//==============================================================================================================
// very similar to extendMatchingWithClearOneToOneAssignments but in this case
// is a distance (->we try to MINIMIZE)
int extendMatchingWithClearOneToOneAssignmentsEuclidean(
  ...
  for (list<supervoxel>::iterator iter = superVoxelA.begin();
       iter != superVoxelA.end(); ++iter, ++ii) {
    for (int jj = 0; jj < assignmentId[ii].numAssignments; jj++) {
      assignedId[assignmentId[ii].assignmentId[jj]] = true;

      thrDist2 += iter->Euclidean2Distance(
          *(vecIterB[assignmentId[ii].assignmentId[jj]]));  // we need to
                                                            // calculate average
                                                            // displacement to
                                                            // set an adaptive
                                                            // threshold for
  ...
  thrDist2 *= 5.0f / ((float)thrDistN);  // threshold is K times teh average
                                         // displacement (we are looking for
                                         // things that do not intersect, so we
                                         // can be generous)
  ...
  for (list<supervoxel>::iterator iterS = superVoxelA.begin();
    ...
    // find the best candidate from A->B
    float costAux,
        costBestAB = thrDist2;  // it has to be better than the threshold
    int idBestAB = -1;
    for (vector<SibilingTypeSupervoxel>::iterator iter =
             nearestNeighborVecAtoB[ii].begin();
         iter != nearestNeighborVecAtoB[ii].end(); ++iter) {
    ...
    if (idBestAB >= 0) {
      list<supervoxel>::iterator iterSB = vecIterB[idBestAB];
      float costBestBA =
          costBestAB - 0.001f;  // to avoid floating point error in comparison

      for (vector<SibilingTypeSupervoxel>::iterator iter =
               nearestNeighborVecBtoA[idBestAB].begin();
           iter != nearestNeighborVecBtoA[idBestAB].end(); ++iter) {
        // if( assignmentId[ (int) ( (*iter)->tempWildcard ) ].numAssignments >
        // 0 ) continue; //uncommment this line to only check the ones that have
  ...
  std::cout << "INFO: extendMatchingWithClearOneToOneAssignmentsEuclidean: "
               "number of extended deaths "
            << numExtended << " out of " << numDeaths
            << " elements that are not assigned yet. ThrDist2 = " << thrDist2
            << endl;
...
int calculateSupervoxelMatchingOneToManyWithSparseHungarianAlgorithm(
  ...
  if (superVoxelB.size() != nearestNeighborVec.size()) {
    cout << "ERROR: "
            "calculateSupervoxelMatchingOneToManyWithSparseHungarianAlgorithm: "
            "size of nearest neighbor vector is not the same as candidates list"
         << endl;
    return 2;
  }

  // setup distance method
  float (supervoxel::*cost_func_ptr)(const supervoxel&)
      const;  // define pointer to function-member class
  ...
  switch (costMethod) {
    case 0:  // absolute number of intersecting points
      ...
      break;
    case 1:  // relative number of intersecting points with respect to candidate
             // size. Maximum value is 1.0
  ...
  // to keep the best assignment for each input (we need it later to try to
  // extend dead)
  ...
  // reset solution
  for (int ii = 0; ii < N; ii++) {
    assignmentId[ii].numAssignments =
        0;  // garbage potential (no assignment) for all of them
  ...
  // since we allow one-to-many, we do not need Hungarian. It is just a greedy
  // selection from the point of view of the candidates
  count = 0;
  int k, kAux;
  ...
  for (list<supervoxel>::iterator iter = superVoxelB.begin();
    ...
    k = -1;
    for (vector<SibilingTypeSupervoxel>::iterator iterNeigh =
             nearestNeighborVec[count].begin();
         iterNeigh != nearestNeighborVec[count].end(); ++iterNeigh) {
      costAux = ((*iter).*
                 cost_func_ptr)(*(*iterNeigh));  // call the selected distance
    ...
    if (k >= 0) {
      if (assignmentId[k].numAssignments < MAX_NUMBER_ASSINGMENTS_SHA) {
        assignmentId[k].assignmentCost[assignmentId[k].numAssignments] =
            costBest;
        assignmentId[k].assignmentId[assignmentId[k].numAssignments++] = count;
      } else {
        cout << "WARNING: at calculateTrackletsWithSparseHungarianAlgorithm: "
                "supervoxel "
             << k << " has exceeded the maximum number of possible assignments"
             << endl;
        // return 3;
      }
    }
  ...
  // we want to try to impose that every input nucleus has an assignment so we
  // run a greedy matching to see if some of the elements that have not been
  // assigned can "steal" an element from elements with multiple assignments
    ...
    if (stolenId[ii] >= 0 && assignmentId[stolenId[ii]].numAssignments > 1) {
      ...
      for (int jj = 0; jj < assignmentId[auxId2].numAssignments; jj++) {
        if (bestAssignmentId[auxId] == assignmentId[auxId2].assignmentId[jj]) {
          // delete element
          for (int kk = jj; kk < assignmentId[auxId2].numAssignments - 1;
               kk++) {
            assignmentId[auxId2].assignmentId[kk] =
                assignmentId[auxId2].assignmentId[kk + 1];
            assignmentId[auxId2].assignmentCost[kk] =
                assignmentId[auxId2].assignmentCost[kk + 1];
          }
      ...
      if (old - assignmentId[auxId2].numAssignments != 1) {
        cout << "ERROR: at calculateTrackletsWithSparseHungarianAlgorithm: I "
  ...
  cout << "INFO: "
          "calculateSupervoxelMatchingOneToManyWithSparseHungarianAlgorithm: "
          "number of extended deaths "
       << numExtended << " out of " << deadId.size()
       << " elements that are not assigned but have an edge." << endl;
  cout << "INFO: "
          "calculateSupervoxelMatchingOneToManyWithSparseHungarianAlgorithm: "
          "total of not assignments is "
       << numNotAssigned << " out of " << N << " input elements to be matched"
       << endl;
#else
  cout << "INFO: "
          "calculateSupervoxelMatchingOneToManyWithSparseHungarianAlgorithm: "
          "greedy algorithm to try to extend deaths not in use"
       << endl;
#endif
