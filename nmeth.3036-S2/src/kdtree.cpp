...
KDTree<T>::KDTree(const KDTree &p) {
  ...
  clear_nearestQueue();  // VIP: PRIORITY QUEUE JUST GETS EMPTIED!!!!NOT
                         // COPIED11 IT SHOULD BE A TEMPORARY STRUCTURE ANYWAYS
}
...
// In the worst case, up to N2, depending on specific sorting algorithm used by
// library implementation.
// So probably this part could be sped up
template <typename T>
void KDTree<T>::buildSubKDTree(vector<T *> &blobs, int start, int end,
                               int axis) {
...
template <typename T>
void KDTree<T>::findSubNearest(vector<T *> &blobs, const float *position,
                               const unsigned int count, const int start,
                               const int end, const int axis,
                               const float max_radius) {
  // Calculate current point: go down the tree from root
  int midpoint = (end - start) / 2 + start;

  // if radius is small, we stop looking down the tree very soon
  if (blobs[midpoint]->center[axis] - radius <=
      position[axis])  // Look in right subtree
    if (end >= midpoint + 1)
      findSubNearest(blobs, position, count, midpoint + 1, end,
                     (axis + 1) % dims, max_radius);

  if (blobs[midpoint]->center[axis] + radius >=
      position[axis])  // Look in left subtree
    if (start <= midpoint - 1)
      findSubNearest(blobs, position, count, start, midpoint - 1,
                     (axis + 1) % dims, max_radius);

  // Add this blob to the priority queue if close enough
  float dist = blobs[midpoint]->distBlobs(
      position);  // we do not account for different time frames

  T *qBlob;
  if (nearest_queue.size() >= count) {
    qBlob = nearest_queue.top();
    radius = qBlob->dist;
    if (dist < qBlob->dist) {  // Replace top blob: dist<max_radius is
                               // guaranteed since
                               // nearest_queue.top()->dist<max_radius
      nearest_queue.pop();
      radius = dist;
...
template <typename T>
void KDTree<T>::findNearest(vector<T *> &blobs, const float *position,
